...
#ifdef CFG_AUDIO_PLAYBACK_SUPPORT
    init_audio_task(TASK_SCENE_PLAYBACK_MSG);
    dsp_playback_init();
#endif
#ifdef CFG_AUDIO_LE_AUDIO_SUPPORT
    init_audio_task(TASK_SCENE_LEAUDIO_DECODE);
#endif
#ifdef CFG_IPC_SUPPORT
#if defined(CFG_HOST_READY_NOTIFY_SUPPORT)
    /* Send DSP audio ready to host */
    audio_ready_notify_host();
#endif
#endif
#endif
    PRINTF_I("%s done. Fireware compile time:%s %s\n", __FUNCTION__, __DATE__, __TIME__);
}
...
#ifdef CFG_AUDIO_LE_AUDIO_SUPPORT
    deinit_audio_task(TASK_SCENE_LEAUDIO_DECODE);
#endif
