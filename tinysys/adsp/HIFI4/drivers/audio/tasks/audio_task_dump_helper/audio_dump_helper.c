...
static void audio_dump_handler(const audio_dump_helper_t* helper, uint32_t dumped_size);

static void print_helper(audio_dump_helper_t* helper)
{
...
static uint32_t audio_dump_get_free_space(audio_dump_helper_t* helper)
{
    uint32_t ret;
...
static void audio_dump_copy_to_shm(audio_dump_helper_t* helper, char* addr,
                                   uint32_t size)
{
...
int audio_dump_feed_data(int dump_position, char* buf, int frames)
{
    ...
    while (size) {
        ...
        if (size) {// avail free not enough, not block va_process
            PRINTF_E("%s audio dump avail free not enough,drop:%d. once copy:%d\n", __func__, size, once_copy_size);
        }
        break;
    }
    err_count = 0;
...
static void audio_dump_trigger_start(audio_dump_helper_t* helper)
{
    helper->running = 1;
    g_dump_status |= (0x1 << helper->dump_position);
}

static void audio_dump_trigger_stop(audio_dump_helper_t* helper)
{
    if (helper->running==0) {
        PRINTF_E("[audio_dump_trigger_stop] not running, might already closed.\n");
        return;
    }
    helper->running = 0;
    g_dump_status &= ~(0x1 << helper->dump_position);
...
int audio_dump_handle_ipc_msg(ipi_msg_t *msg_ptr)
{
    ...
    if (ret > DSP_DATA_DUMP_NUM) {
        PRINTF_E("[audio_dump_handle_ipc_msg], dump_postion error:%d, ignore msg\n", ret);
        return -EINVAL;
    }
...
int audio_dump_helper_write(int dump_position, char* buf, int frames)
{
    int ret = 0;
    if (dump_position > DSP_DATA_DUMP_NUM) {
        PRINTF_E("[audio_dump_helper_write], dump_postion error:%d, ignore write \n", dump_position);
        return -EINVAL;
    }
...
static void audio_dump_lc3_copy_to_shm(audio_dump_helper_t* helper, char* addr,
                                   uint32_t size)
{
    char* base_addr = (char*)(helper->pshared_ring_buf->start_addr);
    uint32_t rd_offset = helper->pshared_ring_buf->ptr_to_appl_offset_bytes;
    ...
    while (size) {
        ...
        if (avail_size > size)
            avail_size = size;
        memcpy(base_addr + wr_offset, addr, avail_size);
        addr += avail_size;
        size -= avail_size;
        wr_offset += avail_size;
        if (wr_offset == data_buf_size) {
...
int audio_dump_feed_lc3_data(int dump_position, char* buf, int size)
{

   audio_dump_helper_t* helper;
    uint32_t once_copy_size, size_request;
    char *char_addr = buf;
    static int err_count = 0;
    ...
    while (size) {
        ...
        audio_dump_lc3_copy_to_shm(helper, char_addr, once_copy_size);
        char_addr += once_copy_size;
        size -= once_copy_size;
...
int audio_dump_lc3_helper_write(int dump_position, char* buf, int size)
{
    int ret = 0;
    if (g_dump_status & (0x1 << dump_position))
        ret = audio_dump_feed_lc3_data(dump_position, buf, size);
    return ret;
}
