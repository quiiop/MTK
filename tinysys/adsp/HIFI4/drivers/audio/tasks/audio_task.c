...
#endif
#ifdef CFG_AUDIO_LE_AUDIO_SUPPORT
...
static AUDIO_TASK g_audio_task_arr[TASK_SCENE_SIZE] = {
    ...
    [TASK_SCENE_VA] = {
        ...
        .ops = NULL,
#endif
        .thread_stack_dep = 8 * configMINIMAL_STACK_SIZE,
        .thread_priority = tskIDLE_PRIORITY,
    },
    [TASK_SCENE_RECORD] = {
        .thread_id = TASK_SCENE_RECORD,
        .thread_name = "aud_t_record",
        .ops = NULL,
        .thread_stack_dep = 4 * configMINIMAL_STACK_SIZE,
        .thread_priority = tskIDLE_PRIORITY,
    },
#ifdef CFG_AUDIO_PLAYBACK_SUPPORT
    [TASK_SCENE_PLAYBACK_MSG] = {
        .thread_id = TASK_SCENE_PLAYBACK_MSG,
        .thread_name = "aud_t_pb_msg",
        .ops = &g_aud_task_playback_msg_ops,
        .thread_stack_dep = 3 * configMINIMAL_STACK_SIZE,
        .thread_priority = tskIDLE_PRIORITY,
    },
#endif
...
#ifdef CFG_AUDIO_LE_AUDIO_SUPPORT
    [TASK_SCENE_LEAUDIO_DECODE] = {
        .thread_id = TASK_SCENE_LEAUDIO_DECODE,
        .thread_name = "aud_t_le_decode_msg",
        .ops = &g_aud_task_leaudio_decode_msg_ops,
        .thread_stack_dep = 3 * configMINIMAL_STACK_SIZE,
        .thread_priority = tskIDLE_PRIORITY,
    },
#endif
                       ...
                       int dir, int t_io_UID)
{
    ...
    if (dir == AUD_THREAD_DIR_OUT) {
        ...
        memset(&priv->input_io_ctrl[priv->input_io_ctrl_num-1], 0, sizeof(THREAD_IO_CTRL));
...
#ifdef CFG_ALIGNMENT_BYTES
    if (mem % CFG_ALIGNMENT_BYTES != 0)
        mem = (mem & (~(CFG_ALIGNMENT_BYTES - 1))) + CFG_ALIGNMENT_BYTES;
#else
    if (mem % 16 != 0)
...
static void shift_buffer(char *buf, int buf_size, int bytes_consumed)
{
    int i;
    if (bytes_consumed <= 0)
        return;
...
/* default 4 channels, 2ch mic + 2ch ref */
void reference_delay_control(char *proc_buf, int buffer_frames, unsigned int bitwidth)
{
    int i, j;
    int mic_ch = 2;
    int ref_ch = 2;
    static int hd; /* hd:1 32bits, hd:0 16bits */
    unsigned int delay_frames = 400; /* 48000Hz:1200, 16000Hz:400 */
    static unsigned int temp_bytes;
    static char *temp_buf = NULL;
    static unsigned int delay_frames_old = 0;

    if (delay_frames_old != delay_frames) {
        if (temp_buf) {
            MTK_vPortFree(temp_buf);
            temp_buf = NULL;
        }

        hd = (bitwidth > 16) ? 1 : 0; /* hd:1 32bits, hd:0 16bits */
        temp_bytes = (buffer_frames + delay_frames) * ref_ch * bitwidth / 8;
        temp_buf = (void *)(MTK_pvPortMalloc(temp_bytes, ADSP_MEM_NORMAL_CACHE));
        memset(temp_buf, 0, temp_bytes);
        delay_frames_old = delay_frames;
        PRINTF_D("Reference_delay_control init done, proc_buf = %p, temp_buf = %p, delay_frames = %d, hd = %d, buffer_frames = %d, temp_bytes = %d\n",
            proc_buf, temp_buf, delay_frames, hd, buffer_frames, temp_bytes);
    }
    /* 1. split */
    if (hd) {
        unsigned int *dst = (unsigned int *)temp_buf;
        unsigned int *src = (unsigned int *)proc_buf;
        for (i = 0; i < buffer_frames; i++) {
            for (j = 0; j < ref_ch; j++) {
                dst[(delay_frames + i) * ref_ch + j] = src[i*(mic_ch + ref_ch) + mic_ch + j];
            }
        }
    } else {
        unsigned short *dst = (unsigned short *)temp_buf;
        unsigned short *src = (unsigned short *)proc_buf;
        for (i = 0; i < buffer_frames; i++) {
            for (j = 0; j < ref_ch; j++) {
                dst[(delay_frames + i) * ref_ch + j] = src[i*(mic_ch + ref_ch) + mic_ch + j];
            }
    ...
    /* 2. copy back */
    if (hd) {
        unsigned int *dst = (unsigned int *)proc_buf;
        unsigned int *src = (unsigned int *)temp_buf;
        for (i = 0; i < buffer_frames; i++) {
            for (j = 0; j < ref_ch; j++)
                dst[i*(mic_ch + ref_ch) + mic_ch + j] = src[i * ref_ch + j];
        }
    } else {
        unsigned short *dst = (unsigned short *)proc_buf;
        unsigned short *src = (unsigned short *)temp_buf;
        for (i = 0; i < buffer_frames; i++) {
            for (j = 0; j < ref_ch; j++) {
                dst[i*(mic_ch + ref_ch) + mic_ch + j] = src[i * ref_ch + j];
            }
    ...
    /* 3. shift */
    shift_buffer(temp_buf, temp_bytes, (buffer_frames) * ref_ch * bitwidth / 8);
}
