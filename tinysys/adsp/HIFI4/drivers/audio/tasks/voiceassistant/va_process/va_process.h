...
{
    if (a.count == 0) {
        FreeRTOS_CLIPrintf("Zero count.\n");
        return;
    }
    FreeRTOS_CLIPrintf("count:%llu, cc:%llu, latest:%d, avg:%d, max:%d, min:%d\n",
             a.count,
             a.total_cc,
             a.stop-a.start,
...
enum {
    ...
    VA_WW_SUCESS,
    VA_WW_FAIL,
    VA_WW_LOCAL_CMD_TRIGGER_SUCCESS,
    VA_WW_LOCAL_CMD_COMMAND_SUCCESS,
    VA_WW_LOCAL_CMD_RECOGINIZE_FAIL,
    VA_WW_LOCAL_CMD_TIMEOUT_FAIL,
    VA_WW_LC_CONTINUE,
    VA_WW_LC_DONE,
};
