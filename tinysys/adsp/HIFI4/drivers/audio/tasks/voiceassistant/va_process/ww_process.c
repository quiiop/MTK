...
#include "cyberon_dspotter_adaptor.h"
#endif
...
NORMAL_SECTION_FUNC static int ww_init(struct va_proc_obj *obj, struct va_pcm_format in, struct va_pcm_format *out, int frames)
{
    ...
    if (algo_adaptor.init != NULL) {
        amz_ret = algo_adaptor.init(obj, in, NULL, frames);
        if (amz_ret){
    ...
    if (algo_adaptor_local_cmd.init != NULL) {
        local_cmd_ret = algo_adaptor_local_cmd.init(obj, in, NULL, frames);
        if (local_cmd_ret){
            if (algo_adaptor_local_cmd.uninit != NULL)
                algo_adaptor_local_cmd.uninit(obj);
            cyberon_dspotter_adaptor_unregister(&algo_adaptor_local_cmd);
        }
     }
     if (amz_ret !=0 && local_cmd_ret != 0 ){
         ret = -1;
         goto exit0;
     }
...
static int ww_set_params(struct va_proc_obj *obj, int cmd, void *data)
{
    ...
    if (algo_adaptor_local_cmd.set_params != NULL)
        algo_adaptor_local_cmd.set_params(obj, cmd, data);
...
static int ww_get_params(struct va_proc_obj *obj, int cmd, void *data)
{
    ...
    if (algo_adaptor_local_cmd.get_params != NULL)
        algo_adaptor_local_cmd.get_params(obj, cmd, data);
...
static int ww_reset(struct va_proc_obj *obj)
{
    ...
    if (algo_adaptor_local_cmd.reset != NULL)
        algo_adaptor_local_cmd.reset(obj);
...
static int ww_process(struct va_proc_obj *obj, char *inbuf, char *outbuf, int frames)
{
...
#ifdef DUMP_TO_HOST_SUPPORT
    audio_dump_helper_write(DSP_DUMP4, ww_proc_buf, frames);
#endif
    ...
    if (algo_adaptor.process != NULL)
        algo_adaptor.process(obj, ww_proc_buf, outbuf, frames);
#ifdef CFG_VA_WW_MCPS_CALC
        mcps_calc_after_proc(&ww_mcps);
#endif

#ifdef CFG_VA_WW_MCPS_CALC
        mcps_calc_before_proc(&lc_mcps);
#endif
    if (algo_adaptor_local_cmd.process != NULL)
        algo_adaptor_local_cmd.process(obj, ww_proc_buf, outbuf, frames);
#ifdef CFG_VA_WW_MCPS_CALC
            mcps_calc_after_proc(&lc_mcps);
#endif
    ...
    if (priv->detect_result == 1) {
        ...
        ret = VA_WW_FAIL;
        priv->detect_result = 0;
    } else if (priv->detect_result == 3) {
        ret = VA_WW_LOCAL_CMD_TRIGGER_SUCCESS;
        priv->detect_result = 0;
    } else if (priv->detect_result == 4) {
        ret = VA_WW_LOCAL_CMD_COMMAND_SUCCESS;
        priv->detect_result = 0;
    } else if (priv->detect_result == 5) {
        ret = VA_WW_LOCAL_CMD_RECOGINIZE_FAIL;
        priv->detect_result = 0;
    } else if(priv->detect_result == 6){
        ret = VA_WW_LOCAL_CMD_TIMEOUT_FAIL;
        priv->detect_result = 0;
    } else {
        ret = VA_WW_CONTINUE;
    }
...
#endif
#ifdef CFG_DSPOTTER_SUPPORT
    cyberon_dspotter_adaptor_register(&algo_adaptor_local_cmd);
#endif
    PRINTF_D("%s [time:%llu] wakeword create success\n", __func__, read_systimer_stamp_ns());
    return 0;
}
...
NORMAL_SECTION_FUNC int ww_proc_destroy(struct va_proc_obj *obj)
{
    ...
    memset(&algo_adaptor_local_cmd, 0, sizeof(algo_adaptor_local_cmd));
    PRINTF_D("%s [time:%llu] wakeword destroy success\n", __func__, read_systimer_stamp_ns());
    return 0;
}
