...
NORMAL_SECTION_FUNC static void local_cmd_ap_notify(struct va_proc_mgr *mgr)
{
    ipi_msg_t ipi_msg_notify;
    struct dsp_ipc_va_notify info;
    struct local_cmd_result lc_info;

    PRINTF_D("[local_cmd_ap_notify] enter\n");

    mgr->proc_list[VA_PROC_WW]->get_params(mgr->proc_list[VA_PROC_WW], CMD_GET_DSPOTTER_RESULT, &lc_info);

    if (lc_info.cmd_state == 0) {
        info.type = VA_NOTIFY_LOCAL_CMD_TRIGGER_PASS;
    } else if (lc_info.cmd_state == 1) {
        info.type = VA_NOTIFY_LOCAL_CMD_COMMAND_PASS;
    }
    else if (lc_info.cmd_state == -1){
        info.type = VA_NOTIFY_LOCAL_CMD_COMMAND_TIMEOUT;
    }
    info.local_cmd_idx = lc_info.cmd_idx;
    info.local_cmd_mapID = lc_info.cmd_mapID;
    PRINTF_D("[local_cmd_ap_notify] type = %d, cmd_idx = %d\n", info.type, lc_info.cmd_idx);
...
NORMAL_SECTION_FUNC static void va_process_ap_notify(struct va_proc_mgr *mgr, struct ww_result *ww_info, int start_pos)
{
    ...
    if (ww_info == NULL) {
        ...
        PRINTF_D("%s [time:%llu] VAD OK to voice upload\n", __func__, read_systimer_stamp_ns());
    } else {
        info.type = VA_NOTIFY_WAKEWORD_PASS;
        strcpy(info.wakeword, ww_info->wakeword);
        /* TODO: need reset CPU share buffer */
        PRINTF_D("%s [time:%llu] WW OK to voice upload\n", __func__, read_systimer_stamp_ns());
        PRINTF_D("wakeword: %s, begin:%llu, end:%llu, cur:%llu, confidence:%d, start:%d, hw ofs:%d\n",
            ww_info->wakeword,
            ww_info->begin_idx,
            ww_info->end_idx,
            ww_info->cur_idx,
...
static int va_process_for_state_change(struct va_proc_mgr *mgr, int from, int to, int event)
{
    ...
    if ((to == VA_STAT_WAKEWORD) && ((ww_always_on && from == VA_STAT_VAD) || (!ww_always_on))) {
        PRINTF_D("%s [time:%llu] wakeword reset\n", __func__, read_systimer_stamp_ns());
        if (va_process_is_enable(mgr, VA_PROC_WW))
    ...
    if (from == VA_STAT_VAD && to == VA_STAT_VOICEUPLOAD && event != VA_EVT_FORCE_VOICE_UPLOAD) {
            ...
            if (va_process_is_enable(mgr, VA_PROC_PREPROC)) {
                ...
                setting.conf = ww_info.confidence/10;
...
NORMAL_SECTION_FUNC int va_process_init(struct va_proc_mgr *mgr, struct va_pcm_format in, struct va_pcm_format *out, int frames)
{
    ...
    if (mgr->proc_list[VA_PROC_VAD] != NULL) {
        ret = mgr->proc_list[VA_PROC_VAD]->init(mgr->proc_list[VA_PROC_VAD], in, NULL, frames);
        if (ret) {
            PRINTF_E("%s VAD init error, ret = %d\n", __func__, ret);
            goto exit0;
        }
    ...
    if (mgr->proc_list[VA_PROC_PREPROC]) {
        ret = mgr->proc_list[VA_PROC_PREPROC]->init(mgr->proc_list[VA_PROC_PREPROC], in, out, frames);
        if (ret) {
            PRINTF_E("%s PREPROC init error, ret = %d\n", __func__, ret);
            goto exit1;
        }
        PRINTF_D("%s preproc output format ch(%d), bitwidth(%d), rate(%d)\n",
                 __func__, out->chnum, out->bitwidth, out->rate);
        if (out->chnum != CFG_VA_VOICE_UPLOAD_CH_NUM || out->bitwidth != CFG_VA_VOICE_UPLOAD_BITWIDTH) {
    ...
    */
    if ((out->chnum != CFG_VA_VOICE_UPLOAD_CH_NUM) ||
        ...
        ret = mgr->proc_list[VA_PROC_CHDEMUX]->init(mgr->proc_list[VA_PROC_CHDEMUX], input, out, frames);
        if (ret) {
            PRINTF_E("%s VA_PROC_CHDEMUX init error, ret = %d", __func__, ret);
            goto exit2;
        }
        PRINTF_D("%s chdemux output format ch(%d), bitwidth(%d), rate(%d)\n",
                 __func__, out->chnum, out->bitwidth, out->rate);
        need_out = 1;
    }
    ...
    if (mgr->proc_list[VA_PROC_WW]) {
        memcpy(&input, out, sizeof(struct va_pcm_format));
        ret = mgr->proc_list[VA_PROC_WW]->init(mgr->proc_list[VA_PROC_WW], input, NULL, frames);
        if (ret) {
            PRINTF_E("%s VA_PROC_WW init error, ret = %d\n", __func__, ret);
            goto exit3;
        }
    ...
    PRINTF_D("%s [time:%llu] end\n", __func__, read_systimer_stamp_ns());
    return 0;
exit3:

    if (mgr->proc_list[VA_PROC_CHDEMUX]) {
        mgr->proc_list[VA_PROC_CHDEMUX]->uninit(mgr->proc_list[VA_PROC_CHDEMUX]);
    }

exit2:
    if (mgr->proc_list[VA_PROC_PREPROC]) {
        mgr->proc_list[VA_PROC_PREPROC]->uninit(mgr->proc_list[VA_PROC_PREPROC]);
    }
    if (mgr->preproc) {
        va_proc_buf_destroy(mgr->preproc, mgr->priv);
    }
  //  if (mgr->proc_list[VA_PROC_CHDEMUX]) {
...
exit1:
    if (mgr->proc_list[VA_PROC_VAD]) {
        mgr->proc_list[VA_PROC_VAD]->uninit(mgr->proc_list[VA_PROC_VAD]);
    }
    if (mgr->in) {
        va_proc_buf_destroy(mgr->in, mgr->priv);
    }
...
static inline int va_vad_ww_process_ret_to_event(int ret_val, int state)
{
    ...
    if (state == VA_STAT_VAD) {
        if (ret_val==VA_VAD_SUCESS)
            event = VA_EVT_VAD_SUCCESS;
        else if (ret_val==VA_VAD_FAIL)
            event = VA_EVT_VAD_FAIL;
        else if (ret_val==VA_VAD_TOONOISY)
            event = VA_EVT_VAD_TOONOISY;
        else
            event = VA_EVT_VAD_FAIL;
    }
    ...
    if (state == VA_STAT_WAKEWORD) {
        if (ret_val == VA_WW_CONTINUE)
            ...
            event = VA_WW_FAIL;
#endif
        else if (ret_val == VA_WW_LOCAL_CMD_TRIGGER_SUCCESS)
            event = VA_EVT_LOCAL_CMD_TRIGGER_SUCCESS;
        else if (ret_val == VA_WW_LOCAL_CMD_COMMAND_SUCCESS)
            event = VA_EVT_LOCAL_CMD_COMMAND_SUCCESS;
        else if (ret_val == VA_WW_LOCAL_CMD_RECOGINIZE_FAIL)
            event = VA_EVT_LOCAL_CMD_RECOGINIZE_FAIL;
        else if (ret_val == VA_WW_LOCAL_CMD_TIMEOUT_FAIL)
            event = VA_EVT_LOCAL_CMD_TIMEOUT_FAIL;
        else
            event = VA_WW_FAIL;
    }
...
int va_process(struct va_proc_mgr *mgr, char *buf, int frames)
{
    ...
    is_need_proc = (buf != NULL) && va_process_is_enable(mgr, VA_PROC_VAD) &&
    ...
    if (state == VA_STAT_WAKEWORD || state == VA_STAT_VOICEUPLOAD) {
        /* common process */
        int obtain_frames;
        int is_in_avail = (int)(va_process_is_enable(mgr, VA_PROC_WW) ||
                          va_process_is_enable(mgr, VA_PROC_PREPROC) ||
                          va_process_is_enable(mgr, VA_PROC_CHDEMUX));
        ...
        if (va_process_is_enable(mgr, VA_PROC_CHDEMUX)) {
            in = out;
    ...
    is_need_proc = va_process_is_enable(mgr, VA_PROC_WW) &&
    ...
    if (is_need_proc) {
        ...
        if (ret == VA_WW_LOCAL_CMD_TRIGGER_SUCCESS || ret == VA_WW_LOCAL_CMD_COMMAND_SUCCESS || ret == VA_WW_LOCAL_CMD_TIMEOUT_FAIL)
            local_cmd_ap_notify(mgr);
        event = va_vad_ww_process_ret_to_event(ret, state);
    }
    ...
    if (state == VA_STAT_WAKEWORD) {
        // if local command was triggered, should ignore ww time out
        static int ww_tout_ignore = 0;
        if (event == VA_EVT_LOCAL_CMD_TRIGGER_SUCCESS) {
            ww_tout_ignore = 1;
        }
        //if local command was done command recognize, should back to vad state
        if (event == VA_EVT_LOCAL_CMD_RECOGINIZE_FAIL || event == VA_EVT_LOCAL_CMD_TIMEOUT_FAIL) {
            ww_tout_ignore = 0;
        }
        int ww_tout = (va_state_get_ww_timeout()) && (!ww_tout_ignore);
        /* wakeword process */
...
NORMAL_SECTION_FUNC int va_process_create(struct va_proc_mgr *mgr, int type)
{
    ...
    PRINTF_D("%s process(%d) is create, setting(%x)\n", __func__, type, mgr->va_setting);
    return 0;
}
...
NORMAL_SECTION_FUNC int va_process_destroy(struct va_proc_mgr *mgr, int type)
{
    ...
    PRINTF_D("%s process(%d) is destroy, setting(%x)\n", __func__, type, mgr->va_setting);
    return ret;
}
