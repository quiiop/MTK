...
#define __ffp_ram0_mem_size__ (344832)
#define __ffp_ram1_mem_size__ (16384)
...
static int effp_adaptor_init(struct va_proc_obj *obj, struct va_pcm_format in, struct va_pcm_format *out, int frames)
{
    ...
    EFFP_MemSpecifier_setSize(&supplyMem, effp_mem_id_tcm_temp, __ffp_ram1_mem_size__);
    ...
    if (require_sram_size) {
        mem_setting->sram_addr =
            MTK_pvPortMalloc(__ffp_ram0_mem_size__ + 8, get_adsp_heap_type(ADSP_MEM_NORMAL_CACHE));
    ...
    if (require_tcm_size) {
        mem_setting->tcm_addr =
            MTK_pvPortMalloc(__ffp_ram1_mem_size__ + 8, get_adsp_heap_type(ADSP_MEM_LP_CACHE));
