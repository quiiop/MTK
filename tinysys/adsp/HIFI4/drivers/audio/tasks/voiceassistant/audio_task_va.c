...
static int va_voice_upload_done_flag = 0;
...
static void va_irq_callback(int io_path_handler)
{
    ...
    if (ret != pdPASS) {
        PRINTF_E("[record_irq_callback] irq queue full.\n");
        configASSERT(0);
        return;
    }
	    ...
	    if (count++ > 1)
            ...
            for (i = 0; i < out_port_num; ++i) {
                ...
                do {
                    ...
                    if (ret != 0) {
                        // If the ap does not read data, set VA_VOICE_UPLOAD_DONE.
                        if (va_voice_upload_done_flag == 0) {
                            va_process_voice_upload_set(va_mgr, VA_SET, VA_VOICE_UPLOAD_DONE);
                            break;
                        }
    ...
    THREAD_IO_CTRL* ctrl, int mem_type, uint32_t specific_addr, uint32_t size_bytes, int ring_buf_type)
{
    ...
    uint32_t request_bytes = size_bytes + CFG_ALIGNMENT_BYTES;
#else
    uint32_t request_bytes = size_bytes;
#endif
    ...
    case AFE_MEM_TYPE_LP:
        ...
        if (buf == NULL) {
            buf =
                (void *)(MTK_pvPortMalloc(request_bytes, get_adsp_heap_type(ADSP_MEM_LP_NCACHE)));
            final_mem_type = ADSP_MEM_LP_NCACHE;
        }
        break;
    default:
        buf =
        (void *)(MTK_pvPortMalloc(request_bytes, get_adsp_heap_type(ADSP_MEM_NORMAL_NCACHE)));
        final_mem_type = ADSP_MEM_NORMAL_NCACHE;
        break;
    }
    PRINTF_D("%s, io ctrl start_addr:%x, type:%d\n", __func__, (uint32_t)buf, final_mem_type);
    if (buf == NULL)
        configASSERT(buf != NULL);

#ifdef CFG_ALIGNMENT_BYTES
    ctrl->buffer_start = buf;
    if ((final_mem_type == ADSP_MEM_LP_NCACHE) || (final_mem_type == ADSP_MEM_NORMAL_NCACHE)) {
        buf = (void *)((((uint32_t)buf + request_bytes) & (~(CFG_ALIGNMENT_BYTES - 1))) - size_bytes);
        PRINTF_D("%s, adjusted io ctrl start_addr:%x, size = %x\n", __func__, (uint32_t)buf, size_bytes);
    }
#endif
...
NORMAL_SECTION_FUNC void va_host_hw_params(AUDIO_TASK* this, ipi_msg_t* msg_ptr) // without static for ice debug
{
    ...
    AUD_DRV_LOG_D("dai_id:0x%x periods:%d, period size:%d, srate:%d, ch:%d, bits:%d\n",
    ...
    if ((dai_id == 3) && (is_va_type == 1) &&(is_hostless == 0))
        va_voice_upload_done_flag = 1;
    ...
    if (ctrl->stream_enabled <= 0) {
        ...
        if (is_va_type) {
            ...
            ret =  va_process_init(va_mgr, in, &out, ipc_hw_params.period_size);
            if (ret){
			    goto exit0;
	    }
        ...
        if (is_hostless) {
            ...
            ctrl->period_count = size_bytes / ctrl->period_size;
        } else {
    ...
    if (is_va_type && !is_hostless && (va_state_get() != VA_STAT_VOICEUPLOAD))
...
{
#ifdef AUDIO_LOG_DEBUG
    AUD_DRV_LOG_D("+%s : msg_ptr->msg_id %d, %s\n", __func__,msg_ptr->msg_id, msg_string[msg_ptr->msg_id]);
#endif
