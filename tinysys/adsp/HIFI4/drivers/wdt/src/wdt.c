			...
			size_t xWriteBufferLen, const char *pcCommandString )
{
	...
	if (1 != FreeRTOS_CLIGetNumberOfParameters(pcCommandString)) {
		PRINTF_D("parameter timeout is null, set to 0 by default\n");
		timeout = 0;
	} else {
		timeout = atoi(FreeRTOS_CLIGetParameter(pcCommandString, 1, &timeout));
		PRINTF_D("set watchdog timeout to %d s\n", timeout);
	}

	mtk_wdt_set_time_out_value(timeout);
	mtk_wdt_enable();
	vTaskSuspend(kick_wdt_handler);
	return 0;
}
