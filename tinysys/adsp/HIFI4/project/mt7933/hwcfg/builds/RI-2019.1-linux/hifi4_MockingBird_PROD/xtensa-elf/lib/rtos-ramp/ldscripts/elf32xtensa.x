...
SECTIONS
{
  ...
  } >sram7_seg :packed_phdr

  .ResetVector.literal : AT(LOADADDR(.DoubleExceptionVector.text) + (ADDR(.ResetVector.literal) - ADDR(.DoubleExceptionVector.text))) ALIGN(4)
  {
  ...
  PROVIDE(__stack = 0xb0000000);
  _heap_sentry = 0xb0000000;
  .rom_store_end LOADADDR(.ResetVector.literal) + SIZEOF(.ResetVector.literal) : 
  { 
    _rom_store_end = .;
  } >sram8_seg
  .debug  0 :  { *(.debug) }
