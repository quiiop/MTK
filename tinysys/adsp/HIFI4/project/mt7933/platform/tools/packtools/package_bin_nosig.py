...
#IMG_BIN_INF_1.bin <-> "hifi4_sram.bin"
def genImgBinInf1():
    ...
    ImgInf_file1.write(struct.pack("I",getFileSize(hifi4dsp_sram_bin)))#IMG_BIN_SZ (=IMG_SZ)
    ImgInf_file1.write(struct.pack("I",getFileSize(hifi4dsp_sram_bin)))#IMG_SZ
    ImgInf_file1.write(struct.pack("I",hifi4dsp_sram_bin_load_base))#LD_ADR, for test boot at SRAM
    ImgInf_file1.close()
...
def genImgBinInf3():
    ...
    ImgInf_file3.write(struct.pack("I",(0 + getFileSize(hifi4dsp_sram_bin))))#Total_HEAD_len+IMG_BIN1's size
...
def genImgBinInfTB():
    ...
    total_img_bin_inf_sz = getFileSize(python_path + "IMG_BIN_INF_1.bin") + getFileSize(python_path + "IMG_BIN_INF_3.bin")
...
hifi4dsp_sram_bin = sys.argv[3]
hifi4dsp_dram_bin  = sys.argv[4]
bin_section_number = 2
hifi4dsp_sram_bin_load_base = int(sys.argv[5], 16)
...
# example - MT8512DSP : 0x40060001
# running = CPU-reservd-addr(for CPU loading binary).
#
hifi4dsp_dram_bin_load_base  = int(sys.argv[6], 16)
hifi4dsp_bootup_vector_base  = int(sys.argv[7], 16)
...
if core_number == 2:
    ...
    exit(1)
if os.path.exists(hifi4dsp_sram_bin):
    pass
else:
    print "Maybe build %s error!" %(hifi4dsp_sram_bin)
    exit(1)
...
#
total_dsp_bin_file_size = getFileSize(hifi4dsp_sram_bin) + getFileSize(hifi4dsp_dram_bin)
...
#
merge(hifi4dsp_sram_bin, python_path + "LOAD_NOSIG.image")
