...
struct hw_res_mgr {
    ...
    int32_t enable[DSP_HW_RES_NUM];
    int32_t request[DSP_HW_RES_NUM][DSP_HW_USER_NUM];
...
static int32_t dsp_hw_res_highest_request(int type)
{
    int i;
    int32_t highest;
...
int dsp_hw_res_enable(int type, int enable)
{
    ...
    if (g_hw_res_mgr.enable[type] == (int32_t)enable)
        return 0;

    g_hw_res_mgr.enable[type] = (int32_t)enable;

    PRINTF_D("hw_mgr [time:%llu], type:%d, en:%d\n", read_systimer_stamp_ns(), type, enable);

    /* for dsp2spm int, no need to force update */
    if (type == DSP_HW_RES_SPM_INT)
        return 0;
...
int dsp_hw_res_request(int type, int user, int request)
{
    ...
    if (g_hw_res_mgr.request[type][user] == (int32_t)request)
        return 0;

    g_hw_res_mgr.request[type][user] = (int32_t)request;
    PRINTF_D("hw_mgr [time:%llu], type:%d, user:%d, req:%d\n", read_systimer_stamp_ns(), type, user, request);
