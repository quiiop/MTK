...
#ifdef NO_USE_UART
void uart_init(unsigned int addr, unsigned int baudrate, unsigned int uart_clk)
{
}
#else
...
NORMAL_SECTION_FUNC void uart_init(unsigned int addr, unsigned int baudrate, unsigned int uart_clk)
{
    int highspeed, quot;
    ...
    if (baudrate < 115200) {
        ...
        highspeed = 3;
        quot = DIV_EXACTLY(uart_clk, 256 * baudrate) + 1;
    }

    sample = DIV_EXACTLY(uart_clk, quot * baudrate);
...
#ifdef CFG_CLI_SUPPORT
    DRV_WriteReg32(UART_IER(addr), UART_IER_ALLOFF);
    request_irq(LX_UART_IRQ_B, vUARTInterruptHandler, "UART0");
    DRV_WriteReg32(UART_IER(addr), UART_IER_ERBFI);
#endif

//    PRINTF_D("dump baudrate config, baud:%d, HIGHSPEED:%d, DLL:%d, DLH:%d, sample_count:%d, sample_point:%d, fcr:%d\n",
//        baudrate, highspeed, dll, dlh, sample_count, sample_point, DRV_Reg32(UART_FCR_RD(addr)));
}
