...
unsigned int aud_drv_pcm_get_mem_start(int handler)
{
    struct dsp_path_data* dsp_data = aud_drv_get_path_data();

    return get_hw_mem_start_with_path_data(&dsp_data[handler]);
}

unsigned int aud_drv_pcm_get_mem_cur(int handler)
{
    struct dsp_path_data* dsp_data = aud_drv_get_path_data();

    return get_hw_mem_cur_with_path_data(&dsp_data[handler]);
}
