...
/**
 * @brief Sending non-mesh advertising data with specified time.
 * @param[in] len is the length of this adv data.
 * @param[in] buf is the adv data.
 * @param[in] ms is the total amount of time of advertising the data.
 ...
 */
bt_mesh_status_t bt_mesh_bearer_adv_send_by_time(uint8_t len, uint8_t * buf, uint16_t ms, bt_hci_cmd_le_set_advertising_parameters_t *params);
 ...
 * @param[in] params is the advertising parameter of the scan response data.
 * @return
 * #BT_MESH_SUCCESS, the adv data is successfully sent. \n
 * #BT_MESH_ERROR_OOM, not enough memory for sending the adv data. \n
 */
bt_mesh_status_t bt_mesh_bearer_adv_set_scan_response_data(bt_hci_cmd_le_set_scan_response_data_t *params);
