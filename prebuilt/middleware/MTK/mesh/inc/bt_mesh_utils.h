...
/**< The max network number for mesh filter, fw cannot support more than 4 network for mesh filter*/
#define MESH_FILTER_MAX_NETWORK_NUMBER 4
...
/**
 * @defgroup Bluetooth_mesh_filter_struct Struct
 * @{
 */

/** @brief The mesh filter param structure. */
...
typedef struct
{
    ...
    uint16_t mesh_message_net_index[MESH_FILTER_MAX_NETWORK_NUMBER];    /**< network indexes for receive mesh message*/
    ...
    uint16_t secure_beacon_net_index[MESH_FILTER_MAX_NETWORK_NUMBER];    /**< network indexes for receive secure network beacon*/
