...
typedef enum {
    BT_MESH_ON_OFF_STATUS_ENABLE_START,
    BT_MESH_ON_OFF_STATUS_ENABLE,
    BT_MESH_ON_OFF_STATUS_DISABLE_START,
    BT_MESH_ON_OFF_STATUS_DISABLE
}bt_mesh_on_off_status;
...
/**
 * @brief notify mesh coredump done.
 * @param[in] state is the parameters of the coredump.
 * @return
 * @note when BT_PANIC_COREDUMP_DONE is received, then set state to true.
 */
void bt_mesh_set_fw_assert_state( bool state );

/**
 * @brief get fw assert state.
 * @return
 * #true, if coredump done
 * #false, if not happened
 * @note when sending hci cmd at coredumped, then get state to judge.
 */
bool bt_mesh_get_fw_assert_state( void );
