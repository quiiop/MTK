...
from .ecdsa import ECDSA384R1, ECDSA384R1Public
from .ecdsa import ECDSA521R1, ECDSA521R1Public
...
def load(path, passwd=None):
    ...
    if isinstance(pk, RSAPrivateKey):
        ...
        if pk.curve.name == 'secp256r1' and pk.key_size == 256:
            return ECDSA256P1(pk)
        elif pk.curve.name == 'secp384r1' and pk.key_size == 384:
            return ECDSA384R1(pk)
        elif pk.curve.name == 'secp521r1' and pk.key_size == 521:
            return ECDSA521R1(pk)
        else:
            raise Exception("Unsupported EC curve: " + pk.curve.name + ", size: " + pk.key_size)
    elif isinstance(pk, EllipticCurvePublicKey):
        if pk.curve.name == 'secp256r1' and pk.key_size == 256:
            return ECDSA256P1Public(pk)
        elif pk.curve.name == 'secp384r1' and pk.key_size == 384:
            return ECDSA384R1Public(pk)
        elif pk.curve.name == 'secp521r1' and pk.key_size == 521:
            return ECDSA521R1Public(pk)
        else:
            raise Exception("Unsupported EC curve: " + pk.curve.name + ", size: " + pk.key_size)
