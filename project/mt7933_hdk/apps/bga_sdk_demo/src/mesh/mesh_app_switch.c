...
#define MESH_CONFIGURE_TIMEOUT 500 // ms
...
#define MAX_NVDM_BLOCK_SIZE (38*MESH_PROVISIONER_NODES_MAX) //sizeof(switch_mesh_provisioned_node_data_t) = 38
...
static int msg_generic_on_off_set(uint16_t dst_addr, uint8_t ttl,
...
// device provisioning
uint16_t g_appkey_index = 0x0000;
...
uint16_t g_start_temp_addr = 0;
uint16_t g_current_element_num = 0;
static uint8_t g_prov_current_uuid[BT_MESH_UUID_SIZE];

static bt_mesh_prov_provisioner_params_t g_start_params = {0};
...
static uint8_t gCheckOnOff = 0;
static uint8_t gCheckTtl = 0;
static uint8_t gCheckCount = 0;
static uint16_t gCheckAddr = 0;
...
static uint32_t gProvisioningTick_start = 0;
static uint32_t gProvisioningTick_end = 0;
static uint32_t gConfigTick_end = 0;
...
static uint32_t get_time_diff_in_ms(uint32_t start, uint32_t end)
{
    uint32_t diff;
    if (start > end) {
...
static void switch_load_prov_addr(void)
{
    ...
    nvdm_status_t status;
    uint32_t size = MAX_NVDM_BLOCK_SIZE;
    uint16_t used_addr = 0;

    status = nvdm_read_data_item("MESH_APP", "node_list", temp_nvdm_buffer, &size);
    if (NVDM_STATUS_OK != status) {
        LOG_I(mesh_app, "Failed to read node info, status %x\n", status);
        g_prov_current_addr = bt_mesh_model_get_element_count() + 1;
        if (g_prov_current_addr  > 0x7FFF) {
            LOG_E(mesh_app, "load porvisioned addresses error!\n");
            g_prov_current_addr = 0x0000;
        }
        return;
    }
    ...
    for (i = 0; i < size / sizeof(switch_mesh_provisioned_node_data_t); i++) {
        ...
        LOG_I(mesh_app, "[%d:%d] 0x%04x, element_nu:%d", i, node_entry->state, node_entry->address, node_entry->elements_num);
        used_addr += node_entry->elements_num;
    ...
    g_prov_current_addr = bt_mesh_model_get_element_count() + used_addr + 1;
    if (g_prov_current_addr > 0x7FFF) {
        LOG_E(mesh_app, "load porvisioned addresses error!\n");
        g_prov_current_addr = 0x0000;
    }
...
static void switch_store_prov_addr(uint16_t prov_current_addr, uint8_t *p_device_key, uint8_t *p_uuid, uint16_t elements_num)
{
    nvdm_status_t status;
    uint32_t size = MAX_NVDM_BLOCK_SIZE;
    ...
    node_entry->elements_num = elements_num;
                                  ...
                                  uint8_t delay, bool reliable)
{
    ...
    return (ret == BT_MESH_SUCCESS) ? 0 : -1;
}
                ...
                if (gSwitchState == SWITCH_SETTING_STATE_ADD_APPKEY_DONE || gSwitchState == SWITCH_SETTING_STATE_BIND_MODEL) {
                    if (event->data.model_app_status.status == BT_MESH_ACCESS_MSG_STATUS_SUCCESS) {
                        ...
                        if (entry != NULL) {
                            ...
                            gConfigTick_end = get_current_time_in_ms();
                            LOG_I(mesh_app, "All model binding is done\n");
                            LOG_I(mesh_app, "Config done! config time: %d ms, total time %d ms",
                                  get_time_diff_in_ms(gProvisioningTick_end, gConfigTick_end),
                                  get_time_diff_in_ms(gProvisioningTick_start, gConfigTick_end));
                        }
                    } else {
                        gSwitchState = SWITCH_SETTING_STATE_IDLE;
                ...
                if (!bt_mesh_utils_is_local_address(msg->meta_data.src_addr)) {
                    if (gIsChecking && (evt->present_onoff == gCheckOnOff)) {
                        if (evt->present_onoff == gCheckOnOff) {
                            ...
                            msg_generic_on_off_set(gCheckAddr, gCheckTtl, gCheckOnOff, gCheckTid, 0, 0, true);
...
static void switch_prov_init(void)
{
    // start
    g_start_params.start.algorithm = BT_MESH_PROV_START_ALGORITHM_FIPS_P256_ELLIPTIC_CURVE;
    g_start_params.start.public_key = BT_MESH_PROV_START_PUBLIC_KEY_NO_OOB;
    ...
    g_prov_current_netidx = g_start_params.data.netkey_index;
    g_start_params.data.address = g_prov_current_addr;

    gProvisioningTick_start = 0;
    gProvisioningTick_end = 0;
    gConfigTick_end = 0;
}
...
bt_mesh_status_t bt_mesh_app_event_callback(bt_mesh_event_id evt_id, bool status, bt_mesh_evt_t *evt_data)
{
    switch (evt_id) {
        case BT_MESH_EVT_INIT_DONE: {
                ...
                LOG_I(mesh_app, "==============================\n\n");
                bt_mesh_enable();
                if (0 == bt_mesh_model_get_element_address(0)) {
                    bt_mesh_model_set_element_addr(0x0001);
                    ...
                    bt_mesh_model_bind_model_app(g_model_handle_health_server, g_appkey_index);
                    bt_mesh_model_bind_model_app(g_model_handle_health_client, g_appkey_index);

                    /* binding generic onoff client model */
                    bt_mesh_model_bind_model_app(g_model_handle_onoff_client, g_appkey_index);

                    /* binding generic level client model */
                    bt_mesh_model_bind_model_app(g_model_handle_level_client, g_appkey_index);

#ifdef MESH_APP_PERFORMANCE_TEST
                    /* binding vendor model */
                    bt_mesh_model_bind_model_app(g_model_handle_vendor, g_appkey_index);
#endif /* #ifdef MESH_APP_PERFORMANCE_TEST */

                    /* binding lighting client models */
                    bt_mesh_model_bind_model_app(g_model_handle_light_lightness_client, g_appkey_index);
                    bt_mesh_model_bind_model_app(g_model_handle_light_hsl_client, g_appkey_index);
                    bt_mesh_model_bind_model_app(g_model_handle_light_ctl_client, g_appkey_index);

                    /* binding scene/time/scheduler client models */
                    bt_mesh_model_bind_model_app(g_model_handle_scene, g_appkey_index);
                    bt_mesh_model_bind_model_app(g_model_handle_time, g_appkey_index);
                    bt_mesh_model_bind_model_app(g_model_handle_scheduler, g_appkey_index);

                    bt_mesh_config_set_iv_index(0, 0);
                }
        ...
        case BT_MESH_EVT_PROV_CAPABILITIES: {
                ...
                g_start_temp_addr = g_start_params.data.address + p->cap.number_of_elements;
                g_current_element_num = p->cap.number_of_elements;
                if (g_start_temp_addr > 0x7FFF) {
        ...
        case BT_MESH_EVT_PROV_DONE: {
                gProvisioningTick_end = get_current_time_in_ms();
                uint32_t prov_time = get_time_diff_in_ms(gProvisioningTick_start, gProvisioningTick_end);
                ...
                if (status) {
                    ...
                    switch_store_prov_addr(p->address, p->device_key, g_prov_current_uuid, g_current_element_num);
                    g_start_params.data.address = g_start_temp_addr;
                } else {
                    if (g_provision_by_gatt) {
                        bt_mesh_bearer_gatt_disconnect();
                    }
...
bt_status_t cmd_delete(void *no, uint16_t argc, char **argv)
{
    if (argc < 2) {
        ...
        g_start_params.data.address = bt_mesh_model_get_element_count() + 1;
        return BT_STATUS_SUCCESS;
    }
...
static bt_status_t cmd_prov_invite(void *no, uint16_t argc, char **argv)
{
    ...
    if ((gSwitchState == SWITCH_SETTING_STATE_IDLE) || (gSwitchState == SWITCH_SETTING_STATE_DONE)) {
        ...
        bt_mesh_app_util_str2u8HexNumArray(argv[1], g_prov_current_uuid);

        // invite
        if (argc < 3) {
            params.attention_duration = 5;
        } else {
            bt_mesh_app_util_str2u8HexNum(argv[2], &params.attention_duration);
        }
        gProvisioningTick_start = get_current_time_in_ms();
        g_prov_current_addr = g_start_params.data.address;
        bt_mesh_provision_invite_provisioning(g_prov_current_uuid, &params);
        gSwitchState = SWITCH_SETTING_STATE_PROVISIONING;
        LOG_I(mesh_app, "start to provision:%");
    } else {
        LOG_I(mesh_app, "already in process, state %d\n", gSwitchState);
    }
    return BT_STATUS_SUCCESS;
...
static bt_status_t cmd_prov_start(void *no, uint16_t argc, char **argv)
{
    ...
    if (gSwitchState == SWITCH_SETTING_STATE_PROVISIONING) {
        ...
        /*Sync with capy. STATIC_OOB -> NO_OOB.Some devices will refuse to provision if we not use "no oob" authen method.*/
...
static bt_status_t cmd_prov_init(void *no, uint16_t argc, char **argv)
{
    ...
    if (argc < 1) {
        ...
        uint16_t addr;
        bt_mesh_app_util_str2u16HexNum(argv[1], &addr);
        if (addr < g_start_params.data.address) {
            LOG_E(mesh_app, "address:0x%04x has been used. current start address:0x%04x",
                  addr, g_start_params.data.address);
            return BT_STATUS_FAIL;
        }
        g_start_params.data.address = addr;
...
static bt_status_t cmd_msg_onoff(void *no, uint16_t argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "set")) {
        ...
        if (addrlen == (BT_MESH_UUID_SIZE * 2)) {
            ...
            bt_mesh_app_util_str2u16HexNum(argv[2], &dst_addr);
            ret = msg_generic_on_off_set(dst_addr, ttl, onoff, gTid++, 0, 0, reliable);
            if (!ret && reliable) {
                gIsChecking = true;
                gCheckTid = gTid;
                gCheckOnOff = onoff;
