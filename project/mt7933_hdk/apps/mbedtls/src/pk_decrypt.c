...
#include <string.h>
#endif /* #if defined(MBEDTLS_BIGNUM_C) && defined(MBEDTLS_PK_PARSE_C) && \ */
...
#else /* #if !defined(MBEDTLS_BIGNUM_C) || !defined(MBEDTLS_PK_PARSE_C) ||  \ */
int pk_decrypt_main(char *enc_buf, size_t enc_len, char *key_buf, size_t key_len)
{
    int ret = 0;
    size_t olen = 0;
    ...
    if ((ret = mbedtls_pk_parse_key(&pk, (const unsigned char *)key_buf, key_len, (const unsigned char *)"", 0, mbedtls_ctr_drbg_random, &ctr_drbg)) != 0) {
