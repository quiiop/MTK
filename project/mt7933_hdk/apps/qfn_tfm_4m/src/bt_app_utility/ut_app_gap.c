...
bt_status_t bt_app_gap_io_callback(void *input, void *output)
{
    ...
    /* Usage: advanced power_on [public address] [random address].
       Note:  Set N if you doesn't need it. */
        ...
        if (strlen(cmd) >= 18) {
            ...
            BT_LOGW("APP", "usage: advanced po [pub addr/N] [rand addr/N]");
    ...
    /* Usage: random_addr [random address].
       Note:  [random address] should be existed. */
        ...
        if (strlen(cmd) >= 16) {
            ...
            BT_LOGW("APP", "usage: gap random_addr [random addr]");
            BT_COLOR_SET(BT_COLOR_WHITE);
        }
    }
       ...
       Note:  default use #lt_addr_type and #lt_addr */
        ...
        if (strlen(cmd) >= 11) {
            ...
            if (addr_type != 0 && addr_type != 1) {
                ...
                BT_LOGW("APP", "gap wl add [0:pub / 1:rand] [bt addr]");
                BT_COLOR_SET(BT_COLOR_WHITE);
            } else {
            ...
            BT_LOGW("APP", "usage: gap wl add [0:pub / 1:rand] [bt addr]");
            BT_COLOR_SET(BT_COLOR_WHITE);
        }
    }
       ...
       Note:  default use #lt_addr_type and #lt_addr */
        ...
        if (strlen(cmd) >= 14) {
            ...
            if (addr_type != 0 && addr_type != 1) {
                BT_COLOR_SET(BT_COLOR_RED);
                //BT_LOGW("APP", "please input the correct address type");
                BT_LOGW("APP", "usage: gap wl remove [0:pub / 1:rand] [bt addr]");
                BT_COLOR_SET(BT_COLOR_WHITE);
            } else {
                const char *addr_str = cmd + 16;
                uint8_t addr[6];
     ...
     */
        ...
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "min_interval > 0xFFFF");
        ...
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "max_interval > 0xFFFF");
            return BT_STATUS_FAIL;
        }
    ...
    */
        ...
        if (strlen(cmd) >= sizeof("gap start_multi_adv xx xx")) {
            ...
            if (ulcn > 0xFF) {
                BT_LOGE("APP", "instance should be 0x00 ~ 0xFF");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 0x7F) {
                BT_LOGE("APP", "tx_power should be 0x00 ~ 0x7F");
                return BT_STATUS_FAIL;
            }
    ...
    */
        ...
        if (ulcn > 1) {
            BT_LOGE("APP", "le_scan_type should be 0 ~ 1");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0x4000 || ulcn < 0x4) {
            BT_LOGE("APP", "le_scan_interval should be 0x0004 ~ 0x4000");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0x4000 || ulcn < 0x4) {
            BT_LOGE("APP", "le_scan_window should be 0x0004 ~ 0x4000");
            return BT_STATUS_FAIL;
        }
        ...
        ulcn = strtoul(cmd + 29, NULL, 10);
        if (ulcn > 0xFF) {
            BT_LOGE("APP", "scanning_filter_policy should be 0x00 ~ 0xFF");
            return BT_STATUS_FAIL;
        }
        ...
        ulcn = strtoul(cmd + 31, NULL, 10);
        if (ulcn > 3) {
            BT_LOGE("APP", "filter duplicates should be 00 ~ 02");
            return BT_STATUS_FAIL;
        }
        ...
        if (strlen(cmd) > 33) {
            ...
            if (is_instant_display > 1) {
                BT_LOGE("APP", "instant display report should be 0 or 1");
                return BT_STATUS_FAIL;
            }
        }
    ...
    */
    ...
    /* Usage:   disconnect <handle in hex>
       Example: disconnect 0200 */
        ...
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "conn_hdl should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
    ...
    /* Usage: update data length <handle in hex> <tx octets in hex> <tx time in hex>.
       Example: update data length 0200 0030 0500*/
        ...
        ulcn = strtoul(cmd + 23, NULL, 16);
        if (ulcn > 0x0EFF) {
            BT_LOGE("APP", "conn_hdl should be 0x0000 ~ 0x0EFF");
            return BT_STATUS_FAIL;
        }
    ...
    /* Usage: tx_power <handle in hex> <tx power level in integer>.
       ...
       Example: tx_power 0200 1*/
        ...
        if (ulcn > 7) {
            BT_LOGE("APP", "tx_power_level should be 0 ~ 7");
            return BT_STATUS_FAIL;
        }
        ...
        if (tx_power_t.connection_handle > 0x0f00 ||
            ...
            BT_LOGW("APP", "The range of connn hdl: 0x0000-0x0EFF, tx power level: 0-7");
    ...
    /* Usage:   gap bond <handle in hex> [io capability] [oob data flag] [auth req]
       ...
       Example: gap bond 0200 3 0 1 0 0*/
        ...
        if (ulcn > 5) {
            BT_LOGE("APP", "io_capability should be 0 ~ 5");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 1) {
            BT_LOGE("APP", "oob_data_flag should be 0 ~ 1");
            return BT_STATUS_FAIL;
        }
        ...
        ulcn = strtoul(cmd + 18, NULL, 10);
        if (ulcn > 0xFF) {
            BT_LOGE("APP", "auth_req should be 0x00 ~ 0xFF");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0xFF) {
            BT_LOGE("APP", "initiator_key_distribution should be 0x00 ~ 0xFF");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0xFF) {
            BT_LOGE("APP", "responder_key_distribution should be 0x00 ~ 0xFF");
            return BT_STATUS_FAIL;
        }
        ...
        ulcn = strtoul(handle, NULL, 16);
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "handle should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
#ifdef MTK_BLE_DM_SUPPORT
