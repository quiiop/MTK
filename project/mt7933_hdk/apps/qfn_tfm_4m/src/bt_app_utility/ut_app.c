...
bt_status_t bt_app_io_callback(void *input, void *output)
{
    ...
    if (bt_app_cmd_in_protected_list(cmd)) {
        if (bt_app_power_state_get() != BT_APP_POWER_STATE_POWER_ON) {
            BT_LOGE("APP", "BT is not in power ON state(st = %d).", bt_app_power_state_get());
            return BT_STATUS_SUCCESS;
        }
    }
    ...
    /* Usage: advanced po [public address] [random address].
       Note:  Set N if you doesn't need it. */
        ...
        if (strlen(cmd) >= 12) {
            ...
            BT_LOGW("APP", "usage: ble advanced po [pub addr/N] [rand addr/N]");
            BT_COLOR_SET(BT_COLOR_WHITE);
        }
        ...
        if (strlen(cmd) >= 34) {
            ...
            if (addr_type != 0 && addr_type != 1) {
                BT_COLOR_SET(BT_COLOR_RED);
                BT_LOGW("APP", "add resolving_list [0:pub_id / 1:rand_id] [addr] [irk]");
                BT_COLOR_SET(BT_COLOR_WHITE);
            } else {
                uint8_t addr[6];
                bt_hci_cmd_le_add_device_to_resolving_list_t dev;
            ...
            BT_COLOR_SET(BT_COLOR_RED);
            BT_LOGW("APP", "add resolving_list [2:pub_id / 3:rand_id] [addr] [irk]");
            BT_COLOR_SET(BT_COLOR_WHITE);
        }
    }
    /* update peer Central Address Resolution supporting */
        ...
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "hdl should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
    ...
    /* Usage: random address [random address].
       Note:  [random address] should be existed. */
        ...
        if (strlen(cmd) >= 15) {
            ...
            BT_LOGW("APP", "usage: ble random address [random addr]");
            BT_COLOR_SET(BT_COLOR_WHITE);
        }
    }
        ...
        if (app_bonded_info != NULL) {
            ...
            BT_LOGE("APP", "idx[%d] error, use \"list bond\" to check", idx);
        }
       ...
       Note:  default use #lt_addr_type and #lt_addr */
        ...
        if (strlen(cmd) >= 7) {
            ...
            if (addr_type != 0 && addr_type != 1) {
                ...
                BT_LOGW("APP", "usage: ble wl add [0:pub / 1:rand] [addr]");
                BT_COLOR_SET(BT_COLOR_WHITE);
            } else {
    ...
    */
        ...
        if (ulcn > 1) {
            BT_LOGE("APP", "scan_type should be 0 or 1");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 3) {
            BT_LOGE("APP", "own_addr_type should be 0 ~ 3");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 4) {
            BT_LOGE("APP", "policy should be 0 ~ 4");
            return BT_STATUS_FAIL;
        }
        policy = (uint8_t)ulcn;
        ...
        BT_LOGI("APP", "Scan_Type[%d] Own_Addr_Type[%d] Filter_Policy[%d]\n", scan_type, own_address_type, policy);
        BT_COLOR_SET(BT_COLOR_WHITE);
    ...
    */
        ...
        if (ulcn > 4) {
            BT_LOGE("APP", "adv_type should be 0 ~ 4");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 3) {
            BT_LOGE("APP", "peer_addr_type should be 0 ~ 3");
            return BT_STATUS_FAIL;
        }
     ...
     */
        ...
        ulcn = strtoul(cmd + 19, NULL, 10);
        if (ulcn > 3) {
            BT_LOGE("APP", "own_address_type should be 0 ~ 3");
            return BT_STATUS_FAIL;
        }
        own_address_type = (uint8_t)ulcn;
        ...
        if (ulcn > 3) {
            BT_LOGE("APP", "peer_address_type should be 0 ~ 3");
            return BT_STATUS_FAIL;
        }
        ...
        BT_LOGI("APP", "Initiator_Filter_Policy[%d] Own_Addr_Type[%d] Peer_Addr_Type[%d]",
    ...
    /* Usage:   disconnect <handle in hex>
       Example: disconnect 0200 */
        ...
        BT_LOGI("APP", "conn_handle(0x%04x)", disconnect_para.connection_handle);
    ...
    /* Usage: update data length <handle in hex> <tx octets in hex> <tx time in hex>.
       Example: update data length 0200 0030 0500*/
        ...
        if (ulcn > 0x0F00) {
            BT_LOGE("APP", "handle should be 0x0000 ~ 0x0F00");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0x00FB || ulcn < 0x001B) {
            BT_LOGE("APP", "tx_octets should be 0x001B ~ 0x00FB");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0x0848 || ulcn < 0x0148) {
            BT_LOGE("APP", "tx_time should be 0x0148 ~ 0x0848");
            return BT_STATUS_FAIL;
        }
        ...
        if (data_length.connection_handle > 0x0f00 ||
            ...
            BT_LOGW("APP", "The range of conn hdl: 0x0000-0x0EFF, tx octets: 0x001B-0x00FB, tx time : 0x0148-0x0848");
        } else {
    ...
    /* Usage:   bond <handle in hex>
       Example: bond 0200 */
        ...
        ulcn = strtoul(handle, NULL, 16);
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "bond error, hdl should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0xFF) {
            BT_LOGE("APP", "during should be 0x00 ~ 0xFF");
            return BT_STATUS_FAIL;
        }
        during = (uint8_t)ulcn;
        ...
        if (ulcn > 0xFF) {
            BT_LOGE("APP", "count should be 0x00 ~ 0xFF");
            return BT_STATUS_FAIL;
        }
        count = (uint8_t)ulcn;
...
bt_status_t bt_ut_app_event_callback(bt_msg_type_t msg, bt_status_t status, void *buff)
{
    if (msg != BT_MEMORY_FREE_GARBAGE_IND &&
        ...
        BT_LOGI("APP", "evt_cb: status(0x%04x), msg = 0x%x", status, msg); // this log is used by CI system test, do not shrink
        BT_COLOR_SET(BT_COLOR_WHITE);
    }
    ...
    switch (msg) {
        ...
        case BT_GAP_LE_CONNECT_IND: {
                ...
                BT_LOGI("APP", "connection handle=0x%04x", connection_ind->connection_handle); // this log is used by CI system test, do not shrink
                ...
                if (status == BT_STATUS_SUCCESS) {
                    ...
                    BT_LOGI("APP", "ACL connected, so start bonding, hdl = 0x%x", handle);
        ...
        case BT_GAP_LE_READ_RSSI_CNF: {
                ...
                BT_LOGI("APP", "conn hdl=0x%04x", rssi->handle);
                if (rssi->rssi == 127) {
        ...
        case BT_GATTC_READ_USING_CHARC_UUID: {
                if (bt_app_wait_peer_central_address_resolution_rsp == true) {
                    ...
                    BT_COLOR_SET(BT_COLOR_RED);
                    BT_LOGI("APP", "Read Peer Central Addr Resolution char");
                    BT_COLOR_SET(BT_COLOR_BLUE);
                    ...
                    if (rsp.att_rsp->opcode == BT_ATT_OPCODE_READ_BY_TYPE_RESPONSE) {
                        ...
                        if (status == BT_STATUS_SUCCESS && rsp.att_rsp == NULL) {
                            BT_LOGI("APP", "Read Peer Central Addr Resolution char FINISHED!!");
                            BT_COLOR_SET(BT_COLOR_WHITE);
                            break;
                        }
                        ...
                        if (rsp.att_rsp->length - 2 == 1) {
                            ...
                            BT_LOGI("APP", "Peer Central Addr Resolution Supporting= %d", Peer_CAR_supporting);
                        }

                    } else if (rsp.att_rsp->opcode == 0x1) {
                        bt_gattc_error_rsp_t error_rsp = *((bt_gattc_error_rsp_t *)buff);
                        BT_LOGI("APP", "Can not find Peer Central Addr Resolution");
                        BT_LOGI("APP", "err_opcode=0x%02x, err_code=0x%02x", error_rsp.att_rsp->error_opcode, error_rsp.att_rsp->error_code);
                    } else {
                        BT_LOGI("APP", "Error: Can not handle feedback");
                    }
                    BT_COLOR_SET(BT_COLOR_WHITE);
                    return BT_STATUS_SUCCESS;
                }
