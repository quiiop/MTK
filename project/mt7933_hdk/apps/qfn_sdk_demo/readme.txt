 ...
 * @{
 * @addtogroup mt793x_hdk_qfn_sdk_demo qfn_sdk_demo
 * @{

@par Overview
  - Project Demonstration
    - This project demonstrates the Wi-Fi/BT connectivity with low power feature 
  - Application features
    - Act as a Wi-Fi station to connect to a Wi-Fi network.
    - Act as a Wi-Fi AP to accept connection to the MT7931 HDK using a
...
@par Run the examples
  ...
  - Below are 5 examples to demonstrate the Wi-Fi station, Wi-Fi access
    point modes of the 793x HDK, BT GATT mode and BT Mesh mode.
  ...
  - Example 1. Wi-Fi station mode (SEC mode).
    ...
    - Once the information is collected, use the following commands to configure the MT7931. The example below assumes either WPA PSK or WPA2 PSK is used for authentication, TKIP or AES for encryption, 'MTK_SOFT_AP'
    ...
    - After calling reload CLI by CM33_UART, if everything is correct, similar messages will be shown in the console to notify your HDK has received an IP address.
        \code
        DHCP got IP:192.168.1.100
        \endcode

    - ping: It assumes IP 19.168.1.100 is received. If the IP address is fetched and the network is operating, the MT7931 can ping other devices on the network with the following command in the console.
        \code
        ping 192.168.1.100
  ...
  - Example 2. Wi-Fi station mode (Open mode).
    - Find your Wi-Fi access point settings: Before connecting to a Wi-Fi access point, the following information needs to be collected:
        -# The SSID of your Wi-Fi access point.
    - Once the information is collected, use the following commands to configure the MT7931. The example below assumes, 'MTK_SOFT_AP' is used for the SSID. 
        \code
        wifi init
        wifi config set ssid 0 MTK_SOFT_AP
  ...
  - Example 3. Wi-Fi access point mode
    - Provide the Wi-Fi access point settings:
      ...
      -# Encryption Type
      -# Password
    - Once the information is collected, use the following commands to configure the MT7931. This example assumes WPA2 PSK is used for authentication, AES for encryption,  'MTK_SOFT_AP' for the SSID,
