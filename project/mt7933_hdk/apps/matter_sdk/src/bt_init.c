...
static void bt_preread_local_address(bt_bd_addr_t addr)
{
    ...
    if (memcmp(addr, &tempaddr, sizeof(bt_bd_addr_t)) == 0) {
        ...
        if (NVDM_STATUS_OK == status) {
            ...
            LOG_I(common, "[BT]Read address from NVDM [%02X:%02X:%02X:%02X:%02X:%02X]", addr[0],
                  addr[1], addr[2], addr[3], addr[4], addr[5]);
            return;
        } else {
            LOG_I(common, "[BT]Failed to Read from NVDM:%d.", status);
            ret = hal_trng_init();
    ...
    LOG_I(common, "[BT]address to write:%s len:%d", buffer, strlen((char *)buffer));
    ...
    if (NVDM_STATUS_OK != status) {
        ...
        LOG_I(common, "[BT]Successfully store address to NVDM [%02X:%02X:%02X:%02X:%02X:%02X]", addr[0],
              addr[1], addr[2], addr[3], addr[4], addr[5]);
    }
}
...
void bt_mm_init()
{
    LOG_I(common, "bt init TX_Buf:%d, RX_Buf:%d", BT_TX_BUF_SIZE, BT_RX_BUF_SIZE);
...
bt_boot_mode_t bt_get_init_mode(void)
{
    ...
    if ((NVDM_STATUS_OK != status) ||
        (init_mode == BT_BOOT_INIT_MODE_NONE) || (init_mode >= BT_BOOT_INIT_MODE_MAX)) {
