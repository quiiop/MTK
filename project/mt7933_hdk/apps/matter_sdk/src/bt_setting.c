                                          ...
                                          bt_setting_value_t set_value)
{
    ...
    if (value_type == BT_SETTING_VALUE_TYPE_BOOL) {
        ...
        result = nvdm_write_data_item("BT_DEBUG", itemname, NVDM_DATA_ITEM_TYPE_RAW_DATA, &value, size);
        ...
        uint32_t value_32 = set_value.value_u32;
        size = sizeof(set_value.value_u32);

        result = nvdm_write_data_item("BT_DEBUG", itemname, NVDM_DATA_ITEM_TYPE_RAW_DATA,
                                      (const uint8_t *)(&value_32), size);
        if (result != NVDM_STATUS_OK) {
            BT_LOGE("BT", "write the value fail, result =%d", result);
            return false;
        } else
            BT_LOGI("APP", "save value = %d, itemname = %s size = %d result = %d", value_32, itemname, size, result);
        ...
        BT_LOGE("BT", "current don't enalbe the module of needing these types, reverse types");
        return false;
    }
...
bool bt_setting_onoff_set(bool enable)
{
    ...
    if (g_setting_table[BT_SETTING_ON_OFF].value.value_bool == enable) {
        BT_LOGI("BT", "the value is same as platform, don't need set");
        return false;
    }
...
bt_setting_value_t bt_setting_value_get(bt_setting_key_t setting_idx)
{
    if (setting_idx < 0 || setting_idx >= BT_SETTING_KEY_MAX) {
        ...
        BT_LOGI("BT", " the index is invalid, return 0xFFFFFFFF");
        return value;
    }
...
bool bt_setting_value_set(uint8_t setting_idx, bt_setting_value_t set_value)
{
    if (!bt_setting_onoff_get()) {
        BT_LOGI("BT", " setting is off");
        return true;
    }

    if (setting_idx >= BT_SETTING_KEY_MAX) {
        BT_LOGI("BT", " setting_idx >= BT_SETTING_KEY_MAX, wrong parameter!");
        return false;
    }
    ...
    switch (g_setting_table[setting_idx].value_type) {
        case BT_SETTING_VALUE_TYPE_BOOL:
            if (g_setting_table[setting_idx].value.value_bool == set_value.value_bool) {
                BT_LOGI("BT", "the value is same as platform, don't need save");
                return false;
            }
        ...
        default:
            BT_LOGI("BT", "current don't enalbe the module of needing these types, reverse types");
            return false;
    }
...
void bt_setting_dump(void)
{
    ...
    for (i = 0; i < BT_SETTING_KEY_MAX; i++) {
        if (g_setting_table[i].value_type == BT_SETTING_VALUE_TYPE_BOOL)
            BT_LOGI("BT", " module name = %s, value is %s",
            ...
            BT_LOGI("BT", " module name = %s, value is %d",
...
void bt_setting_init(void)
{
    ...
    result = nvdm_read_data_item("BT_DEBUG", (const char *)g_setting_table[BT_SETTING_ON_OFF].itemname, &value, &size);
    if (result != NVDM_STATUS_OK)
    ...
    if (setting_on_off) {
        for (i = BT_SETTING_ON_OFF + 1; i < BT_SETTING_KEY_MAX; i++) {
            if (g_setting_table[i].value_type == BT_SETTING_VALUE_TYPE_BOOL) {
                ...
                result = nvdm_read_data_item("BT_DEBUG", (const char *)g_setting_table[i].itemname, &value, &size);
                if (result != NVDM_STATUS_OK)
                    BT_LOGI("BT", "read the value fail, result = %d", result);
                else {
                    BT_LOGI("BT", "read the value = %d, size = %d", value, size);
                ...
                result = nvdm_read_data_item("BT_DEBUG", g_setting_table[i].itemname, &value, &size);
                if (result != NVDM_STATUS_OK)
                    BT_LOGE("BT", "read the value fail, result = %d", result);
                else {
                    BT_LOGI("BT", "read the value = %d, size = %d", value, size);
                ...
                result = nvdm_read_data_item("BT_DEBUG", g_setting_table[i].itemname, (uint8_t *)(&value_u32), &size);
                if (result != NVDM_STATUS_OK)
                    BT_LOGE("BT", "read the value fail, result = %d", result);
                else {
                    BT_LOGI("BT", "read the value = %d, size = %d", value, size);
                ...
                result = nvdm_read_data_item("BT_DEBUG", g_setting_table[i].itemname,
        ...
        BT_LOGI("BT", "setting is off, currently use default value");
