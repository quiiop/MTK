...
BT_GATTS_NEW_CHARC_VALUE_UINT32_WRITABLE(gatt_service_changed, CLI_BT_SIG_UUID_SERVICE_CHANGED,
                                         0x2, 0x050F0001); //characteristic value = 0x[050F/*end handle*/0001/*start handle*/]
...
extern const bt_gatts_service_t chipOBle_gatts_service;

//server collects all service. Please make sure order services by handle number.
const bt_gatts_service_t *bt_gatt_server[] = {
    &bt_if_gap_service,       //0x0001-0x000D
    &bt_if_gatt_service_ro,   //0x0011-0x0013
    &chipOBle_gatts_service,
    NULL
};

//When GATTS get req from remote client, GATTS will call bt_get_gatt_server_by_handle() to get application's gatt service DB.
