...
void test_init_task(void *pvParameters)
{
	...
	hal_pinmux_set_function(HAL_GPIO_31, 0);
	hal_gpio_init(HAL_GPIO_33);
	hal_pinmux_set_function(HAL_GPIO_33, 0);
	hal_gpio_init(HAL_GPIO_36);
	hal_pinmux_set_function(HAL_GPIO_36, 0);
	
	hal_gpio_set_direction(HAL_GPIO_31, 1);
	hal_gpio_set_direction(HAL_GPIO_33, 1);
	hal_gpio_set_direction(HAL_GPIO_36, 1);
	hal_gpio_set_output(HAL_GPIO_31, 1);
	hal_gpio_set_output(HAL_GPIO_33, 1);
	//hal_gpio_set_output(HAL_GPIO_36, 0);
	hal_gpio_set_output(HAL_GPIO_36, 1);
	
	while(1)
	{
		printf("Turn on LED for 1 seconds!!\r\n\r\n");	
		vTaskDelay(1000/portTICK_RATE_MS);
		hal_gpio_set_output(HAL_GPIO_31, 0);
		hal_gpio_set_output(HAL_GPIO_33, 0);
		//hal_gpio_set_output(HAL_GPIO_36, 1);
		printf("Turn off LED for 1 seconds!!\r\n\r\n");	
		vTaskDelay(1000/portTICK_RATE_MS);
		hal_gpio_set_output(HAL_GPIO_31, 1);
		hal_gpio_set_output(HAL_GPIO_33, 1);
		//hal_gpio_set_output(HAL_GPIO_36, 1);
	}
	//printf("Turn off LED and Relay!!\r\n\r\n");	
	//hal_gpio_set_output(HAL_GPIO_31, 0);
	//hal_gpio_set_output(HAL_GPIO_33, 0);
	//hal_gpio_set_output(HAL_GPIO_36, 0);
	//vTaskDelay(1000/portTICK_RATE_MS);
...
int test_init_task_create(void)
{
    if (xTaskCreate(test_init_task,
                    test_init_TASK_NAME,
                    test_init_TASK_STACKSIZE / sizeof(portSTACK_TYPE),
                    NULL,
                    test_init_TASK_PRIO,
                    NULL) != pdPASS) {
        LOG_E(common, "xTaskCreate fail");
