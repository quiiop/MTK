...
static const bt_cm_config_t s_cm_config = {
    .max_connection_num = 1,
    .connection_takeover = true,
    .request_role = BT_CM_ROLE_UNKNOWN,
    .request_role_retry_times = 0xFF,
    .page_timeout = 0x2000, /* 0x2000 * 0.625 msec = 5.12 sec */
    .power_on_reconnect_profile = BT_CM_PROFILE_SERVICE_MASK(BT_CM_PROFILE_SERVICE_A2DP_SINK),
    .power_on_reconnect_duration = 0,
    .link_loss_reconnect_profile = BT_CM_PROFILE_SERVICE_MASK(BT_CM_PROFILE_SERVICE_A2DP_SINK),
...
#ifdef AIR_LE_AUDIO_ENABLE
static void bt_set_csis_sirk(uint8_t *data, uint8_t data_len)
{
    bt_key_t sirk = {0xf3, 0x0b, 0x65, 0x1f, 0x26, 0x84, 0xd7, 0xe1, 0x5f, 0x25, 0xd9, 0x48, 0x60, 0x53, 0xa4, 0x97};
#ifdef MTK_NVDM_ENABLE
    ...
    /*
     * SIRK already exist. BT LE Audio MW already read it. So do not generate it again.
     * If user want to change SIRK, call ble_csis_write_nvkey_sirk directly.
     */
    nvkey_status = nvkey_read_data(NVKEYID_BT_LE_AUDIO_CSIS_DATA, (uint8_t *)&data_nvkey, &size);
    if (NVKEY_STATUS_OK == nvkey_status) {
        goto exit;
    }
    LOG_I(common, "[BT] read SIRK from NVDM fail %d", nvkey_status);
#endif /* #ifdef MTK_NVDM_ENABLE */

    memcpy(sirk, data, data_len > sizeof(sirk) ? sizeof(sirk) : data_len);
    // Set SIRK to BT LE Audio MW
    ble_csis_set_sirk(sirk);

#ifdef MTK_NVDM_ENABLE
    // Store SIRK to NVDM
    memcpy(&data_nvkey.sirk, sirk, sizeof(bt_key_t));
    nvkey_status = nvkey_write_data(NVKEYID_BT_LE_AUDIO_CSIS_DATA, (uint8_t *)&data_nvkey, BLE_CSIS_NVKEY_DATA_LEN);
    if (NVKEY_STATUS_OK != nvkey_status) {
        LOG_E(common, "[BT]Failed to store SIRK. %d\n", nvkey_status);
    }
#endif /* #ifdef MTK_NVDM_ENABLE */

exit:
    LOG_I(common, "[BT]SIRK [%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x,...]\n",
          data_nvkey.sirk[15], data_nvkey.sirk[14], data_nvkey.sirk[13], data_nvkey.sirk[12],
          data_nvkey.sirk[11], data_nvkey.sirk[10], data_nvkey.sirk[9], data_nvkey.sirk[8]);
}
#endif /* #ifdef AIR_LE_AUDIO_ENABLE */
...
#endif /* #ifdef BT_LE_AUDIO_ENABLE */
    bt_memory_init_control_block(BT_MEMORY_CONTROL_BLOCK_EDR_CONNECTION, connection_cb_buf, BT_CONNECTION_BUF_SIZE);
...
void bt_create_task(void)
{
    ...
    bt_cm_init(&s_cm_config);

    bt_sink_feature_config_t config;
    config.features = BT_SINK_CONFIGURABLE_FEATURE_NONE;
...
#ifdef AIR_LE_AUDIO_ENABLE
    /*
     * This is just a sample code to set a random SIRK.
     * In real project, please set your own key to prevent security issue.
    */
    bt_set_csis_sirk(g_local_random_addr, 6);
#endif /* #ifdef AIR_LE_AUDIO_ENABLE */
