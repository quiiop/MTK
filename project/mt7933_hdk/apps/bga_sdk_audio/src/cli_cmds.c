...
#ifdef MTK_FATFS_ENABLE
static uint8_t _cmd_aud_dump(uint8_t len, char *param[])
{
    uint8_t val = 0;
    if (len != 1) {
        cli_puts("len error!\n");
    ...
    if (val == 1) {
        FRESULT f_ret;
        f_ret = f_open(&g_fid, "SD:/audio_dump", FA_CREATE_ALWAYS | FA_WRITE);
        if (f_ret) {
            cli_puts("open audio dump file failed. D:/audio_dump");
        } else {
            g_audump_inited = 1;
            cli_puts("open dump file success. SD:/audio_dump");
        }
    } else if (val == 0) {
        cli_puts("close dump file success. SD:/audio_dump");
        f_close(&g_fid);
        g_audump_inited = 0;
    }
...
extern cmd_t ssusb_driver_cli[];
#endif /* #ifdef MTK_SSUSB_HOST_ENABLE */
#ifdef MTK_FATFS_ENABLE
extern cmd_t ff_cli[];
#endif /* #ifdef MTK_FATFS_ENABLE */
#ifdef MTK_AUDIO_MP3_ENABLED
extern cmd_t mp3_cli[];
#endif /* #ifdef MTK_AUDIO_MP3_ENABLED */
#if defined(HAL_AUDIO_MODULE_ENABLED)
extern cmd_t cmd_hal_audio[];
...
#ifdef MTK_FATFS_ENABLE
    { "audump", "audio dump",                   _cmd_aud_dump,      NULL },
#endif /* #ifdef MTK_FATFS_ENABLE */
    PSRAM_CMDS
#ifdef BRINGUP_BT_ENABLE
...
#ifdef MTK_AUDIO_MP3_ENABLED
    { "mp3", "mp3 cli cmd", NULL, &mp3_cli[0] },
#endif /* #ifdef MTK_AUDIO_MP3_ENABLED */
#endif /* #ifdef MTK_MT7933_AUDIO_DRIVER_ENABLE */
#ifdef MTK_SSUSB_HOST_ENABLE
    { "ssusb", "ssusb driver cli cmd", NULL, &ssusb_driver_cli[0] },
#endif /* #ifdef MTK_SSUSB_HOST_ENABLE */
#ifdef MTK_FATFS_ENABLE
    { "ff", "ff cli cmd", NULL, &ff_cli[0] },
#endif /* #ifdef MTK_FATFS_ENABLE */
#if defined(HAL_AUDIO_MODULE_ENABLED)
    { "hal_aud", "hal auido deubg", NULL, &cmd_hal_audio[0]},
#endif /* #if defined(HAL_AUDIO_MODULE_ENABLED) */
    { NULL, NULL, NULL, NULL }
};
