...
#ifdef MTK_BTUT_SPP_SUPPORT
#include "btut_spp_main.h"
#endif /* #ifdef MTK_BTUT_SPP_SUPPORT */
...
#ifdef __MTK_BT_SINK_SUPPORT__
    {"sink",    bt_app_sink_io_callback},
#endif /* #ifdef __MTK_BT_SINK_SUPPORT__ */
#ifdef AIR_LE_AUDIO_ENABLE
    {"lea",     bt_app_lea_io_callback},
#endif /* #ifdef AIR_LE_AUDIO_ENABLE */
#ifdef MTK_BTUT_SPP_SUPPORT
    {"spp",     btut_spp_io_callback},
#endif /* #ifdef MTK_BTUT_SPP_SUPPORT */
};
...
bt_status_t bt_vendor_cmd_send_lc3_cbk(uint32_t is_timeout, uint32_t timer_id, uint32_t data, const void *arg)
{
    BT_LOGI("APP", "Trigger lc3, %s", is_timeout ? "Cmd timeout" : "Success");
    return BT_STATUS_SUCCESS;
}

//A common API to get addr from cmd string
bt_status_t bt_app_get_param_addr(bt_bd_addr_t *addr, char *text, const char *cmd)
{
    uint32_t cmd_offset;
    uint32_t cmd_str_len = 0;

    cmd_offset = strlen(text);
    cmd_str_len = strlen(cmd);
    if ((cmd_str_len - cmd_offset)  < 12) {
        //At least need 12 bytes for input addr (ex: 112233445566)
        BT_LOGE("APP", "Input param is wrong!!");
        return BT_STATUS_FAIL;
    }
    cmd += cmd_offset;
    if (copy_str_to_addr((uint8_t *)addr, cmd))
        BT_LOGI("APP", "Input addr = 0x%x:%x:%x:%x:%x:%x",
                (*addr)[5], (*addr)[4], (*addr)[3], (*addr)[2], (*addr)[1], (*addr)[0]);
    else {
        BT_LOGE("APP", "Please input complete address");
        return BT_STATUS_FAIL;
    }
    return BT_STATUS_SUCCESS;
}
...
bt_status_t bt_app_io_callback(void *input, void *output)
{
    ...
    /* Usage: set pts_addr [pts address]*/
        ...
        power_setting[7] = (uint8_t)strtoul(cmd + 29, NULL, 16); //Diff mode. "0" means 3dB diff between BR and EDR. "1" means 0 diff mode.

        bt_hci_send_vendor_cmd_ex(0xFC2C, power_setting, 8, 0, bt_vendor_cmd_set_tx_power_cbk);
    } else if (UT_APP_CMP("send lc3")) {
        char data[] = "LC3";
        bt_hci_send_vendor_cmd_ex(0x0C13, (uint8_t *)data, sizeof(data), 0, bt_vendor_cmd_send_lc3_cbk);
    }
    ...
    */
    ...
    /* Usage:   bond <handle in hex>
       Example: bond 0200 */
        ...
        /* bt disconnect [addr]
           e.g. bt disconnect 11-22-33-44-55-66 */
        bt_bd_addr_t addr;
        copy_str_to_addr(&addr[0], cmd + strlen("bt disconnect "));
        bt_gap_connection_handle_t conn_handle = bt_gap_get_handle_by_address(&addr);
        bt_gap_disconnect(conn_handle);
    } else if (UT_APP_CMP("bt cancel name")) {
        /* bt bt cancel name [addr]
           e.g. bt cancel name 11-22-33-44-55-66 */
        bt_bd_addr_t addr;
        ...
        /* bt bt remote name [addr]
           e.g. bt remote name 11-22-33-44-55-66 */
        bt_bd_addr_t addr;
...
#if 0
        /* bt connect [addr]
           e.g. bt connect 11-22-33-44-55-66 */
        bt_bd_addr_t addr;
        copy_str_to_addr(&addr[0], cmd + strlen("bt connect "));
        bt_gap_connect(&addr);
#endif /* #if 0 */
    } else if (UT_APP_CMP("bt set role")) {
        /* bt set role [addr] [role]
           e.g. bt set role 11-22-33-44-55-66 1 */
        bt_role_t role;
        bt_bd_addr_t addr;
        copy_str_to_addr(&addr[0], cmd + strlen("bt set role "));
        bt_gap_connection_handle_t conn_handle = bt_gap_get_handle_by_address(&addr);
        role = (bt_role_t)strtoul(cmd + strlen("bt set role xx-xx-xx-xx-xx-xx "), NULL, 16);
        bt_gap_set_role(conn_handle, role);
    } else if (UT_APP_CMP("bt get role")) {
        /* bt set role [addr]
           e.g. bt get role 11-22-33-44-55-66 */
        bt_bd_addr_t addr;
        copy_str_to_addr(&addr[0], cmd + strlen("bt get role "));
        bt_gap_connection_handle_t conn_handle = bt_gap_get_handle_by_address(&addr);
        bt_gap_get_role(conn_handle);
    } else if (UT_APP_CMP("bt read rssi")) {
        /* bt read rssi [addr]
           e.g. bt read rssi 11-22-33-44-55-66 */
        bt_bd_addr_t addr;
        copy_str_to_addr(&addr[0], cmd + strlen("bt read rssi "));
        bt_gap_connection_handle_t conn_handle = bt_gap_get_handle_by_address(&addr);
...
bt_status_t bt_ut_app_event_callback(bt_msg_type_t msg, bt_status_t status, void *buff)
{
    ...
    switch (msg) {
        ...
        case BT_GAP_LE_CONNECT_IND: {
                ...
                // Automatically cancel the connection we created or the enable RPA procedure for the receiving connection would fail
                if (connection_ind->role == BT_ROLE_SLAVE && bt_app_connecting) {
                    bt_gap_le_cancel_connection();
                } else
                    bt_app_connecting = false;
        ...
        case BT_GAP_LE_BONDING_COMPLETE_IND:
            ...
            BT_COLOR_SET(BT_COLOR_WHITE);
            if (status == BT_STATUS_SUCCESS) {
#if !defined(MTK_BLE_DM_SUPPORT) || defined(AIR_LE_AUDIO_ENABLE)
                const bt_gap_le_bonding_complete_ind_t *ind = (bt_gap_le_bonding_complete_ind_t *)buff;
#endif /* #if !defined(MTK_BLE_DM_SUPPORT) || defined(AIR_LE_AUDIO_ENABLE) */

#ifndef MTK_BLE_DM_SUPPORT
                /* If support BLE DM, below get bonded_info will always be NULL,
                   so we use 'ifndef MTK_BLE_DM_SUPPORT' to make sure this part will not be run.*/
...
#ifdef AIR_LE_AUDIO_ENABLE
                bt_gattc_discovery_status_t disc_status;
                disc_status = bt_gattc_discovery_start(BT_GATTC_DISCOVERY_USER_LE_AUDIO,
                                                       ind->handle, false);
                BT_LOGI("APP", "start discovery, status %d", disc_status);
#endif /* #ifdef AIR_LE_AUDIO_ENABLE */
