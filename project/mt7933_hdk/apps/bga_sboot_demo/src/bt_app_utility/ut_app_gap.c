...
bt_status_t bt_app_gap_io_callback(void *input, void *output)
{
    ...
    */
        ...
        bt_gap_le_config_extended_advertising(0x0B, NULL, &ext_adv_para, &ext_data, &ext_scan);
    }

    else if (UT_APP_CMP("gap enable_ext_adv")) {
        bt_hci_le_set_ext_advertising_enable_t ext_adv_enable = {
            .enable = BT_HCI_ENABLE,
            .duration = 3000, // 3000 * 10ms = 30sec
            .max_ext_advertising_evts = 0,
        };
        bt_gap_le_enable_extended_advertising(0x0B, &ext_adv_enable);
    }
    ...
    */
        ...
        if (strlen(cmd) >= 18) {
            ...
            if (ulcn > 0x4000 || ulcn < 0x4) {
                BT_LOGE("APP", "gap advanced_conn, le_scan_interval should be 0x0004 ~ 0x4000");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 0x4000 || ulcn < 0x4) {
                BT_LOGE("APP", "gap advanced_conn, le_scan_window should be 0x0004 ~ 0x4000");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 0xFF) {
                BT_LOGE("APP", "gap advanced_conn, initiator_filter_policy should be 0x00 ~ 0xFF");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 3) {
                BT_LOGE("APP", "gap advanced_conn, peer_address.type should be 0 ~ 3");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 3) {
                BT_LOGE("APP", "gap advanced_conn, own_address_type should be 0 ~ 3");
                return BT_STATUS_FAIL;
            }
            connect_para.own_address_type = (uint8_t)ulcn;

            ulcn = strtoul(cmd + 52, NULL, 16);
            if (ulcn > 0x190 || ulcn < 0x6) {
                BT_LOGE("APP", "gap advanced_conn, conn_interval_min should be 0x0006 ~ 0x0190");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 0x190 || ulcn < 0x6) {
                BT_LOGE("APP", "gap advanced_conn, conn_interval_max should be 0x0006 ~ 0x0190");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 0x1F3) {
                BT_LOGE("APP", "gap advanced_conn, conn_latency should should be 0x0000 ~ 0x01F3");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 0xC80 || ulcn < 0xA) {
                BT_LOGE("APP", "gap advanced_conn, supervision_timeout should be 0x000A ~ 0x0C80");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 0xFFFF) {
                BT_LOGE("APP", "gap advanced_conn, minimum_ce_length should be 0x0000 ~ 0xFFFF");
                return BT_STATUS_FAIL;
            }
            ...
            if (ulcn > 0xFFFF) {
                BT_LOGE("APP", "gap advanced_conn, maximum_ce_length should be 0x0000 ~ 0xFFFF");
                return BT_STATUS_FAIL;
            }
    ...
    /* Usage:   disconnect <handle in hex>
       Example: disconnect 0200 */
        ...
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "gap update_conn, connection_handle should 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0x190 || ulcn < 0x6) {
            BT_LOGE("APP", "gap update_conn, conn_interval_min should be 0x0006 ~ 0x0190");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0x190 || ulcn < 0x6) {
            BT_LOGE("APP", "gap update_conn, conn_interval_max should be 0x0006 ~ 0x0190");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0x1F3) {
            BT_LOGE("APP", "gap update_conn, conn_latency should be 0x0000 ~ 0x01F3");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0xC80 || ulcn < 0xA) {
            BT_LOGE("APP", "gap update_conn, supervision_timeout should be 0x000A ~ 0x0C80");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "gap update_conn, minimum_ce_length should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
        ...
        if (ulcn > 0xFFFF) {
            BT_LOGE("APP", "gap update_conn, maximum_ce_length should be 0x0000 ~ 0xFFFF");
            return BT_STATUS_FAIL;
        }
