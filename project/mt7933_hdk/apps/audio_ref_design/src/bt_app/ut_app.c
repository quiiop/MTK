...
void print_advertising_report(void *p)
{
    ...
    while (count < report->data_length) {
        if (count >= BT_HCI_LE_ADVERTISING_DATA_LENGTH_MAXIMUM - 2) {
            BT_LOGW("%s, count = %u, larger than report length", __func__, count);
            break;
        }
...
void ut_print_help(void)
{
    printf("Command line usage example: (ble h), (ble gap power_off)\n"
           ...
           "  start_scan [scan_type] [interval] [window] [own addr type] [scan filter policy] [filter duplicate]\n"
           "                                    -Start scan\n"
           "  stop_scan                         -Stop scan\n"
...
bt_status_t bt_app_io_callback(void *input, void *output)
{
    ...
    if (bt_app_cmd_in_protected_list(cmd)) {
        BT_LOGI("APP", "\"%s\" in cli cmd protected list", cmd);
        if (bt_app_power_state_get() != BT_APP_POWER_STATE_POWER_ON) {
            BT_LOGE("APP", "bt not power on, can not do this cmd!!!");
            return BT_STATUS_SUCCESS;
        }
    }
    ...
    */
        ...
        if ((adv_para.advertising_type == ADVERTISING_TYPE_DIRECT_IND_HIGH_DUTY) || (adv_para.advertising_type == ADVERTISING_TYPE_DIRECT_IND_LOW_DUTY)) {
    ...
    /* GATTC signed write wo rsp 0201 xxxx value. */
        ...
        if (bt_get_init_mode() == BT_BOOT_INIT_MODE_GATTS) {
            ...
            BT_LOGI("APP", "[DTP]not in gatts mode! Mode:0x%x \n", bt_get_init_mode());
        }
    }
#endif /* #ifdef MTK_BLE_SMTCN_ENABLE */
...
static void cb_gatts_request_read(uint16_t usConnId, uint32_t ulUlTransId,                                                                                      BTBdaddr_t *pxBda, uint16_t usAttrHandle, uint16_t usOffset);
