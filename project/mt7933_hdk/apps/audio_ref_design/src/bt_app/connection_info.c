...
{
    uint8_t i = 0;
    memset(connection_cb, 0, sizeof(app_bt_connection_cb_t));
    memset(bonded_info_req, 0, sizeof(app_bt_bonded_info_t));
...
void delete_connection_info(void *buff)
{
    ...
    for (i = 0; i < BT_CONNECTION_MAX ; i++) {
        if (disconnect_complete->connection_handle == connection_cb[i].connection_handle) {
            connection_cb[i].connection_handle = 0;//clear connection info.
...
{
    BT_LOGD("APP", "    bd_addr: %s", bt_debug_bd_addr2str3(bonded_info->bt_addr.addr));
    BT_LOGD("APP", "    identity_addr: %s", bt_debug_bd_addr2str3(bonded_info->info.identity_addr.address.addr));
