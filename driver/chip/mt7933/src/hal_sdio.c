...
error:
    ...
    log_hal_info("sdio error status = %d \r\n", status);
...
hal_sdio_status_t hal_sdio_init(hal_sdio_port_t sdio_port, hal_sdio_config_t *sdio_config)
{
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
    sleep_management_register_suspend_callback(SLEEP_BACKUP_RESTORE_MSDC, sdio_backup_all, NULL);
    sleep_management_register_resume_callback(SLEEP_BACKUP_RESTORE_MSDC, sdio_restore_all, NULL);
#endif /* #ifdef HAL_SLEEP_MANAGER_ENABLED */

#ifdef MSDC_USE_INT
    msdc_nvic_set(true);
#else /* #ifdef MSDC_USE_INT */
    msdc_nvic_set(false);
#endif /* #ifdef MSDC_USE_INT */

    msdc_reset();

    SRAM_PWR_ON_SDCTL;
    SRAM_PWR_ON_MSDC_ECC;
    hal_sleep_manager_lock_sleep(SLEEP_LOCK_MSDC);
    ...
    msdc_set_output_clock(MSDC_INIT_CLOCK); /*setting bus clock to 240KHz.*/
...
hal_sdio_status_t hal_sdio_deinit(hal_sdio_port_t sdio_port)
{
    ...
    SRAM_PWR_DOWN_SDCTL;
    hal_sleep_manager_unlock_sleep(SLEEP_LOCK_MSDC);
...
hal_sdio_status_t hal_sdio_execute_command53(hal_sdio_port_t sdio_port, hal_sdio_command53_config_t *command53_config)
{
    ...
    if (command53_config->direction) {
        ...
        while (word_count) {
            interrupte_status = MSDC_REG->MSDC_INT;
            if (interrupte_status & MSDC_INT_DATTMO || interrupte_status & MSDC_INT_DATCRCERR) {
                log_hal_info("SDIO write got int_status:0x%08x\n", interrupte_status);
        ...
        while (word_count) {
            interrupte_status = MSDC_REG->MSDC_INT;
            if (interrupte_status & MSDC_INT_DATTMO || interrupte_status & MSDC_INT_DATCRCERR) {
                log_hal_info("SDIO read got int_status:0x%08x\n", interrupte_status);
                log_hal_info("SDIO left:%d\n", word_count << 2);
                log_hal_info("SDIO reg IOCON:0x%08x\n", MSDC_REG->MSDC_IOCON);
                log_hal_info("SDIO reg PB0:0x%08x\n", MSDC_REG->PATCH_BIT0);
                log_hal_info("SDIO read int:0x%08x\n", MSDC_REG->MSDC_INT);
                log_hal_info("SDIO read crc:0x%08x\n", MSDC_REG->SDC_DATCRC_STS);
                status =  ERROR_DATA_CRC_ERROR;
                break;
            }
...
hal_sdio_status_t hal_sdio_execute_command53_dma(hal_sdio_port_t sdio_port, hal_sdio_command53_config_t *command53_config)
{
    ...
    if (NULL == msdc_config.msdc_sdio_dma_interrupt_callback) {
        log_hal_info("sdio dma interrupt callback is null\r\n");
        return HAL_SDIO_STATUS_ERROR;
    }
