                                    ...
                                    u32 frequency, u16 duty_cycle, u8 enable)
{
    ...
    if (enable == 0) {
        ...
        log_hal_debug("_mtk_mhal_pwm_group_set frequency == %d.\n",
                      frequency);
        ...
        mtk_hdl_pwm_enable_clk(base, index);
        mtk_hdl_pwm_group_global_kick_enable(base, index);
        mtk_hdl_pwm_group_global_kick(base);
        mtk_hdl_pwm_group_kick(base, index);
        mtk_hdl_pwm_group_global_kick_disable(base, index);
...
int mtk_mhal_pwm_clock_select(struct mtk_pwm_controller *ctlr)
{
    ...
    if (ctlr->group_clock >= PWM_CLOCK_NUM) {
        log_hal_error("invalid source clock: %d.\n", ctlr->group_clock);
                            ...
                            pwm_channels pwm_num)
{
    ...
    if (!_mhal_pwm_is_channel_valid(pwm_num)) {
        log_hal_error("Invalid channel: %d.\n", pwm_num);
        return -PWM_EPARAMETER;
    }
                      ...
                      u32 channel_bit_map)
{
    ...
    if (ctlr->group_number >= MAX_GROUP_NUM) {
        log_hal_error("group number should be 0~%d.\n", MAX_GROUP_NUM - 1);
        return -PWM_EPARAMETER;
    }

    if (ctlr->group_clock >= PWM_CLOCK_NUM) {
        log_hal_error("source clock:should be 0~%d.\n", PWM_CLOCK_NUM - 1);
                               ...
                               pwm_channels pwm_num)
{
    ...
    if (ctlr->data->frequency == 0) {
        log_hal_error("frequency cannot be %d.\n",
                      ctlr->data->frequency);
        return -PWM_EPARAMETER;
    }
    ...
    if (calculated > MAX_PWM_ON_OFF_RES)
        return -PWM_EPARAMETER;
    ...
    ctlr->data->current_frequency[pwm_num] = ctlr->data->frequency;

    ctlr->data->total_count = calculated;
    log_hal_debug("mtk mhal_pwm_set_frequency ,calculated == %d.\n",
                  calculated);
                                ...
                                pwm_channels pwm_num)
{
    ...
    if (ctlr->data->duty_cycle > PWM_DUTY_CYCLE_BASE) {
        log_hal_error("duty_cycle cannot be greater than %d.\n",
    ...
    log_hal_debug("mtk mhal_pwm_set_duty_cycle ,frequency == %d.\n",
                  frequency);
    ...
    if (frequency == 0) {
        ...
        pwm_enable = 1;
    log_hal_debug("mtk mhal_pwm_set_duty_cycle.\n");

    _mhal_pwm_calc_total_count(ctlr->group_clock,
                               ctlr->data->frequency, &total_count);
    ...
    if (((total_count * ctlr->data->duty_cycle) / PWM_DUTY_CYCLE_BASE > MAX_PWM_ON) ||
        ((total_count - total_count / PWM_DUTY_CYCLE_BASE * ctlr->data->duty_cycle)) > MAX_PWM_OFF) {
        log_hal_error("on: 0x%x.\n", (total_count * ctlr->data->duty_cycle) / PWM_DUTY_CYCLE_BASE);
        log_hal_error("off: 0x%x.\n", (total_count - total_count / PWM_DUTY_CYCLE_BASE * ctlr->data->duty_cycle));
        log_hal_error("mhal_pwm_set_duty_cycle failed.\n");
        return -PWM_EPARAMETER;
    }

    ctlr->data->current_duty_cycle[pwm_num] = ctlr->data->duty_cycle;
                       ...
                       pwm_channels pwm_num)
{
    ...
    log_hal_debug("frequency is =%d,duty =%d.\n", frequency, duty_cycle);
    _mtk_mhal_pwm_group_set(ctlr->base,
                            pwm_num,
                            ctlr->data->current_frequency[pwm_num],
                            ctlr->data->current_duty_cycle[pwm_num],
                            pwm_enable
                           );
                               ...
                               pwm_channels pwm_num)
{
    ...
    ctlr->data->current_frequency[pwm_num] = working_frequency;
                                ...
                                pwm_channels pwm_num)
{
    ...
    ctlr->data->current_duty_cycle[pwm_num] = working_duty_cycle;
                                ...
                                pwm_channels pwm_num)
{
    ...
    if ((ctlr->data->global_kick_enable != 0) &&
        ...
        log_hal_error("Invalid global_kick_enable: %d.\n",
    ...
    if ((ctlr->data->io_ctrl_sel != 0) &&
        ...
        log_hal_error("Invalid io_ctrl_sel: %d.\n",
    ...
    if ((ctlr->data->polarity_set != 0) &&
        ...
        log_hal_error("Invalid polarity_set: %d.\n",
                                        ...
                                        pwm_channels pwm_num)
{
    ...
    log_hal_debug("ctlr->data->stage %d.\n", ctlr->data->stage);
    log_hal_debug("ctlr->data->frequency %d.\n", ctlr->data->frequency);
    log_hal_debug("ctlr->data->duty_cycle %d.\n", ctlr->data->duty_cycle);
                                         ...
                                         pwm_channels pwm_num)
{
    ...
    if ((ctlr->data->replay_mode != 0) &&
        ...
        log_hal_error("Invalid replay_mode: %d.\n", pwm_num);
        return -PWM_EPARAMETER;
    }
    /*
    ...
    */
    if ((ctlr->data->s0_stay_cycle & 0xF000) ||
        ...
        log_hal_error("Invalid s0_stay_cycle: %d.\n",
                      ctlr->data->s0_stay_cycle);
        log_hal_error("Invalid s1_stay_cycle: %d.\n",
                      ctlr->data->s0_stay_cycle);
        return -PWM_EPARAMETER;
    }

    log_hal_debug("ctlr->data->s0_stay_cycle %d.\n",
                  ctlr->data->s0_stay_cycle);
    log_hal_debug("ctlr->data->s1_stay_cycle %d.\n",
                  ctlr->data->s1_stay_cycle);
    log_hal_debug("ctlr->data->replay_mode %d.\n",
                       ...
                       pwm_channels pwm_num)
{
    ...
    if (_mhal_pwm_is_dp_mode_valid(ctlr->data->mode)) {
        log_hal_error("Invalid differential mode select: %d.\n",
                      ctlr->data->mode);
        return -PWM_EPARAMETER;
    }
