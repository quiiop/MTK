...
/** @brief Defines the PWM IP status */
typedef enum {
    PWM_DEINIT = 0,
    /**< PWM is Uninit*/
    PWM_INIT = 1
...
struct mtk_com_pwm_data {
    ...
    u32  current_duty_cycle[PWM_MAX_NUM];
    /** PWM hardware current's output frequency */
    u32  current_frequency[PWM_MAX_NUM];
    ...
    pwm_running_status running_status[PWM_MAX_NUM];
};
...
struct mtk_pwm_controller {
    ...
    void __iomem *base;
    /** PWM controller group clock base address */
    void __iomem *top_clk_base;
    /** PWM controller group number */
    char group_number;
    /** Clock source of this PWM controller */
    pwm_clks group_clock;
    /** PWM in init or un-init status */
    pwm_crtl_status crtl_status;
