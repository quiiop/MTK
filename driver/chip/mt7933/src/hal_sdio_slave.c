...
{
    if (true == sdio_slave_check_fw_own()) {
        return HAL_SDIO_SLAVE_STATUS_NOT_READY;
    }
...
{
    return sdio_slave_check_fw_own();
}

void hal_sdio_slave_set_rx_queue_stop(hal_sdio_slave_rx_queue_id_t queue_id)
{
    sdio_slave_set_rx_queue_stop((sdio_slave_rx_queue_id_t) queue_id);
}
...
{
    sdio_slave_status_t status = sdio_slave_receive((sdio_slave_tx_queue_id_t)queue_id, (uint32_t)buffer, size);
