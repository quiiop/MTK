...
#define SDCTL_TOP_CMD_IRQn 138
#define SDIO_SLV_IRQn SDCTL_TOP_FW_IRQn
#define SDIO_CMD_AWAKE SDCTL_TOP_CMD_IRQn


sdio_slave_private_t sdio_private;
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN sdio_slave_gpd_header_t sdio_slave_rx_gpd[SDIO_SLAVE_RX_QUEUE_MAX];
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN sdio_slave_gpd_header_t sdio_slave_tx_gpd[SDIO_SLAVE_TX_QUEUE_MAX];
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN sdio_slave_gpd_header_t sdio_slave_gpd_null;
...
void sdio_slave_read_mailbox(uint32_t mailbox_number, uint32_t *mailbox_value)
{
    if (0 == mailbox_number) {
        ...
        *mailbox_value = SDIO_SLAVE_REG->H2DRM1R;
        ...
        log_hal_info("error, read mailbox_number %d invalide.\r\n", mailbox_number);
    }
}
...
void sdio_slave_write_mailbox(uint32_t mailbox_number, uint32_t mailbox_value)
{
    if (0 == mailbox_number) {
        ...
        log_hal_info("error, write mailbox_number %d invalide.\r\n", mailbox_number);
    }
}
...
void sdio_slave_set_device_to_host_interrupt(uint32_t interrupt_number)
{
    if (24 <= interrupt_number) {
        log_hal_info("error, read mailbox_number invalide.\r\n");
    } else {
...
void sdio_sw_reset(void)
{
    HAL_REG_32(NORMAL_SW_RST) &= ~(BIT(SDIO_SW_RST_OFFSET));
    HAL_REG_32(NORMAL_SW_RST) |= BIT(SDIO_SW_RST_OFFSET);
}
...
uint32_t error_for_timeout(uint16_t mask)
{
    if ((mask & (HWFISR_RD_TIMEOUT_INT_MASK | HWFISR_WR_TIMEOUT_INT_MASK)) ==
        (HWFISR_RD_TIMEOUT_INT_MASK | HWFISR_WR_TIMEOUT_INT_MASK)) {
        return HAL_SDIO_SLAVE_EVENT_ERROR_TIMEOUT_RXTX;
    }

    if (mask & HWFISR_RD_TIMEOUT_INT_MASK) {
        return HAL_SDIO_SLAVE_EVENT_ERROR_TIMEOUT_TX;
    }

    if (mask & HWFISR_WR_TIMEOUT_INT_MASK) {
        return HAL_SDIO_SLAVE_EVENT_ERROR_TIMEOUT_RX;
    }

    return HAL_SDIO_SLAVE_EVENT_ERROR_NONE;
}
...
void sdio_slave_isr(hal_nvic_irq_t irq_number)
{
    ...
    uint32_t i;
    uint32_t error_cb_param = HAL_SDIO_SLAVE_EVENT_ERROR_NONE;
    ...
    if (interrupt_status.fw_interrupte_status_mask & HWFISR_DRV_SET_FW_OWN_MASK) {
        if (NULL != sdio_private.sdio_property.sdio_slave_callback) {
            sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_FW_OWN, (void *)TRUE, sdio_private.sdio_property.sdio_slave_callback_user_data);
        }
        // TODO:: sleep handle.
    }
    ...
    if (interrupt_status.fw_interrupte_status_mask & HWFISR_DRV_CLR_FW_OWN_MASK) {
        ...
        if (NULL != sdio_private.sdio_property.sdio_slave_callback) {
            sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_FW_OWN, (void *)FALSE, sdio_private.sdio_property.sdio_slave_callback_user_data);
        }
    }

    if (interrupt_status.fw_interrupte_status_mask & (HWFISR_RD_TIMEOUT_INT_MASK | HWFISR_WR_TIMEOUT_INT_MASK)) {
        log_hal_info("Timeout error, fw_interrupte_status_mask = %x", interrupt_status.fw_interrupte_status_mask);
        if (NULL != sdio_private.sdio_property.sdio_slave_callback) {
            error_cb_param = error_for_timeout(interrupt_status.fw_interrupte_status_mask);
            sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_ERROR, (void *)&error_cb_param, sdio_private.sdio_property.sdio_slave_callback_user_data);
        }
    }

    if (interrupt_status.tx0_interrupte_status_mask & 0xffffff00) {
        log_hal_info("Tx0 error, tx0_interrupte_status_mask = %x", interrupt_status.tx0_interrupte_status_mask);
        if (NULL != sdio_private.sdio_property.sdio_slave_callback) {
            error_cb_param = HAL_SDIO_SLAVE_EVENT_ERROR_TX0;
            sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_ERROR, (void *)&error_cb_param, sdio_private.sdio_property.sdio_slave_callback_user_data);
        }
    }

    if (interrupt_status.rx0_interrupte_status_mask & 0xffffff00) {
        log_hal_info("Rx0 error, rx0_interrupte_status_mask = %x", interrupt_status.rx0_interrupte_status_mask);
        if (NULL != sdio_private.sdio_property.sdio_slave_callback) {
            error_cb_param = HAL_SDIO_SLAVE_EVENT_ERROR_RX0;
            sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_ERROR, (void *)&error_cb_param, sdio_private.sdio_property.sdio_slave_callback_user_data);
        }
    }

    if (interrupt_status.rx1_interrupte_status_mask & 0x0f00) {
        log_hal_info("Rx1 error, rx1_interrupte_status_mask = %x", interrupt_status.rx1_interrupte_status_mask);
        if (NULL != sdio_private.sdio_property.sdio_slave_callback) {
            error_cb_param = HAL_SDIO_SLAVE_EVENT_ERROR_RX1;
            sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_ERROR, (void *)&error_cb_param, sdio_private.sdio_property.sdio_slave_callback_user_data);
        }
    }
    ...
    if (interrupt_status.global_interrupt_status_mask & HGFISR_CRC_ERROR_INT_MASK) {
        log_hal_info("data crc error.");
        if (NULL != sdio_private.sdio_property.sdio_slave_callback) {
            error_cb_param = HAL_SDIO_SLAVE_EVENT_ERROR_CRC;
            sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_ERROR, (void *)&error_cb_param, sdio_private.sdio_property.sdio_slave_callback_user_data);
        }
    }
...
void sdio_slave_interrupt_init(void)
{
    ...
    hal_nvic_irq_set_type(SDIO_SLV_IRQn, HAL_NVIC_IRQ_TYPE_LEVEL_HIGH);
    ...
    for (i = SDIO_SLAVE_TXQ_START; i < (uint32_t)(sdio_private.sdio_property.txq_number + SDIO_SLAVE_TXQ_START); i++) {
    ...
    for (i = SDIO_SLAVE_TXQ_START; i < (uint32_t)(sdio_private.sdio_hw_property.tx_port_number + SDIO_SLAVE_TXQ_START); i++) {
...
sdio_slave_status_t sdio_slave_send(sdio_slave_rx_queue_id_t queue_id, uint32_t data_address, uint32_t data_length)
{
    ...
    if (queue_id >= SDIO_SLAVE_RX_QUEUE_MAX) {
        log_hal_info("rx queue id error.\r\n");
        return SDIO_SLAVE_STATUS_INVALID_PARAMETER;
    }

    if (data_address & 0x03) {
        log_hal_info("rx data address 4-byte align error.\r\n");
        return SDIO_SLAVE_STATUS_INVALID_PARAMETER;
    }

    if (data_length > SDIO_SLAVE_MAX_PACKET_LENGTH) {
        log_hal_info("rx data length error.\r\n");
        return SDIO_SLAVE_STATUS_INVALID_PARAMETER;
    }
...
sdio_slave_status_t sdio_slave_receive(sdio_slave_tx_queue_id_t queue_id, uint32_t data_address, uint32_t data_length)
{
    ...
    if (queue_id >= SDIO_SLAVE_TX_QUEUE_MAX) {
        log_hal_info("tx queue id error.\r\n");
        return SDIO_SLAVE_STATUS_INVALID_PARAMETER;
    }

    if (data_address & 0x03) {
        log_hal_info("tx data address 4-byte align error.\r\n");
        return SDIO_SLAVE_STATUS_INVALID_PARAMETER;
    }

    if (data_length > SDIO_SLAVE_MAX_PACKET_LENGTH) {
        log_hal_info("tx data length error.\r\n");
        return SDIO_SLAVE_STATUS_INVALID_PARAMETER;
    }
