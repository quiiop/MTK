...
#if defined CORE_CM4
    if (id != HAL_CORE_CM4) {
        if (((id == HAL_CORE_DSP0) && (status == HAL_CORE_OFF)) || ((id == HAL_CORE_DSP1) && (status == HAL_CORE_OFF))) {
...
void hal_dsp_core_reset(hal_core_id_t id, uint32_t reset_vector)
{
    ...
    *(((hal_core_id_t *)HW_SYSRAM_PRIVATE_MEMORY_CORE_STATUS_START) + id) = HAL_CORE_INIT;
    ...
    switch (id) {
        ...
        /*DSP1 set reset vector and reset */
        case HAL_CORE_DSP1:
            MCU_CFG_PRI->DSP1CFG_BOOT_VECTOR = reset_vector;
            MCU_CFG_PRI->DSP1CFG_BOOT_VECTOR_SELECT = 1;
            MCU_CFG_PRI->DSP1CFG_STALL = 1;
            WDT_REGISTER->WDT_RST1_UNION.WDT_RST1 = 0x011C011B;
            WDT_REGISTER->WDT_RST1_UNION.WDT_RST1 = 0x001C001B;
            MCU_CFG_PRI->DSP1CFG_STALL = 0;
            break;
        default:
            break;
    }
