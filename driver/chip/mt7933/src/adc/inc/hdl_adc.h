...
#define adc_err(fmt, arg...)\
    do {\
        log_hal_error("[%s]: ", __func__);\
        log_hal_error(fmt, ##arg);\
    } while (0)

#ifndef DISABLE_ADC_DEBUG
#define adc_debug(fmt, arg...)\
    do {\
        log_hal_debug("[%s]: ", __func__);\
        log_hal_debug(fmt, ##arg);\
    } while (0)
#else /* #ifndef DISABLE_ADC_DEBUG */
#define adc_debug(fmt, arg...)
#endif /* #ifndef DISABLE_ADC_DEBUG */
 ...
 */
#define ADC_CR_SW_RST_MASK                          BIT(5)
#define ADC_CR_SW_CLK_MASK                          BIT(30)
