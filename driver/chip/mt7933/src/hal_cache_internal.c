...
static ATTR_RWDATA_IN_TCM_SEC_SHM CACHE_CONFIG_Type g_cache_config;
...
ATTR_TEXT_IN_TCM void cache_status_save(void)
{
    ...
    g_cache_config.cache_con.w = CACHE->CACHE_CON;
    g_cache_config.cache_region_en = CACHE->CACHE_REGION_EN;

    /*
     * save current cache region settings
     */

    for (region = HAL_CACHE_REGION_0; region < HAL_CACHE_REGION_MAX; region ++) {
        CACHE_ENTRY_Type *entry = &g_cache_config.cache_entry[region];
        entry->cache_entry_n.w = CACHE->CACHE_ENTRY_N[region];
        entry->cache_end_entry_n.w = CACHE->CACHE_END_ENTRY_N[region];
    }
...
ATTR_TEXT_IN_TCM void cache_status_restore(void)
{
    ...
    cache_region_en = g_cache_config.cache_region_en;
    ...
    for (region = HAL_CACHE_REGION_0; region < HAL_CACHE_REGION_MAX; region ++) {
        /*
         * If the region is enabled, restore the previous setting of the
         * corresponding region
         */
        if (cache_region_en & 1) {
            CACHE->CACHE_ENTRY_N[region] = g_cache_config.cache_entry[region].cache_entry_n.w;
            CACHE->CACHE_END_ENTRY_N[region] = g_cache_config.cache_entry[region].cache_end_entry_n.w;
        }
        cache_region_en = cache_region_en >> 1;
    }
    CACHE->CACHE_REGION_EN = g_cache_config.cache_region_en;
    CACHE->CACHE_CON = g_cache_config.cache_con.w;
}
