...
mtk_sdio_status_t sdio_wait_command_ready(void)
{
    ...
    if (count >= MSDC_TIMEOUT_PERIOD_COMMAND) {
        //log_hal_info("wait sdio command ready timeout, MSDC_INT = %x, interrupt_status = %x, loop_count = %d.\r\n", MSDC_REG->MSDC_INT, interrupt_status, loop_count);
...
mtk_sdio_status_t sdio_wait_data_ready(void)
{
    ...
    if (count >= MSDC_TIMEOUT_PERIOD_DATA) {
        log_hal_info("wait sdio data ready timeout.\r\n");
...
mtk_sdio_status_t sdio_send_command(uint32_t msdc_command, uint32_t argument)
{
    ...
    if (status != NO_ERROR) {
        log_hal_info("send command error, command = %x, arg = %x \r\n", (unsigned int)msdc_command, (unsigned int)argument);
        return status;
    }
