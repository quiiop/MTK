...
mtk_sd_status_t sd_wait_data_ready(void)
{
    ...
    if (count >= MSDC_TIMEOUT_PERIOD_DATA) {
        log_hal_info("wait sd data ready timeout.\r\n");
    ...
    if (interrupt_status & MSDC_INT_DATTMO) {
        ...
        log_hal_info("interrupt_status = %x\r\n", (unsigned int)interrupt_status);
...
mtk_sd_status_t sd_wait_command_ready(void)
{
    ...
    if (count >= MSDC_TIMEOUT_PERIOD_COMMAND) {
        log_hal_info("wait sd command ready timeout.\r\n");
...
mtk_sd_status_t sd_send_command(uint32_t msdc_command, uint32_t argument)
{
    ...
    if (status != NO_ERROR) {
        log_hal_info("send command error, cmd = 0x%08x, opcode = %d, arg = 0x%08x error= %d\n",
                      (unsigned int)msdc_command, (unsigned int)(msdc_command & 0x3F), (unsigned int)argument, status);
        return status;
    }
...
mtk_sd_status_t sd_latch_read_blocks(uint32_t *read_buffer,  uint32_t read_address, uint32_t block_number)
{
    mtk_sd_status_t status;
...
static void sd_dma_interrupt_error_handle(mtk_sd_status_t status)
{
    sd_information.error_status = status;
    msdc_dma_disable();
...
{
    mtk_sd_status_t status = NO_ERROR;
    uint32_t response = 0;
