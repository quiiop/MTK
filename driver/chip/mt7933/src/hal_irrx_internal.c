...
{
    P_IOT_IRRX_TypeDef pIRRXTypeDef = (P_IOT_IRRX_TypeDef)(IRRX_BASE + 0x00);

    (pIRRXTypeDef->IRCLR) |= BIT(IR_IRCLR_OFFSET);
    (pIRRXTypeDef->INTCLR) |= BIT(IR_INTCLR_OFFSET);
}
...
ENUM_HAL_RET_T halIrRxRegisterCallback(low_hal_irrx_mode_t mode,
                                       low_hal_irrx_callback_t callback, void *parameter)
{
    ...
    hal_nvic_disable_irq(IRRX_IRQn);
    hal_nvic_register_isr_handler(IRRX_IRQn, halIrRxIsr);
    hal_nvic_enable_irq(IRRX_IRQn);

    return ret;
}


void halIrRxRC5Read(uint8_t *pucBitNum, uint32_t *pu4DW0, uint32_t *pu4DW1)
{
    uint8_t ucBitNum = 0;
    uint8_t format = 0;
    P_IOT_IRRX_TypeDef pIRRXTypeDef = (P_IOT_IRRX_TypeDef)(IRRX_BASE + 0x00);
...
void halIrRxPWDRead(uint8_t *pucPWNum, uint8_t *pucPWBuf, uint8_t ucBufLen)
{
    P_IOT_IRRX_TypeDef pIRRXTypeDef = (P_IOT_IRRX_TypeDef)(IRRX_BASE + 0x00);
    uint8_t   ucNum = 0;
    uint32_t  u4RegVal = 0;
    uint32_t  u4RegAddr = (uint32_t)(&(pIRRXTypeDef->CHK_DATA0));

    log_hal_info("\n");

    do {
        u4RegVal = readl(u4RegAddr);
...
ENUM_HAL_RET_T halIrRxRC5(uint8_t ucInverse, uint8_t ucBitReverse, uint8_t ucIgnoreEmpty, uint16_t u2SamplePeriod)
{
    // IR RX Sequence for RC-5
    P_IOT_IRRX_TypeDef pIRRXTypeDef = (P_IOT_IRRX_TypeDef)(IRRX_BASE + 0x00);
    uint32_t u4Val = 0;
    uint32_t u4DeGlitchCnt = 0x1F;
    ENUM_HAL_RET_T ret = HAL_RET_SUCCESS;
    ...
    // [5]    IGN_1ST  1'b0    Ignore synchronization pulse
    u4Val &= (BIT(IRCFGH_IGN1ST_OFFSET));
...
#if 0 // debug 
    {
        uint32_t  u4CalTherminateThresholdUs = 0;
        uint32_t  u4CalPrecesionUs = 0;
...
#endif /* #ifdef HAL_IRRX_MODULE_ENABLED */
