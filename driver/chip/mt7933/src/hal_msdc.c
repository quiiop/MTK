...
{
    if (is_power_on) {
        hal_clock_enable(HAL_CLOCK_CG_SDIOM);
    } else {
        hal_clock_disable(HAL_CLOCK_CG_SDIOM);
    }
}
...
void msdc_set_output_clock(uint32_t clock)
{
    ...
    /*enable msdc clock*/
    hal_clock_enable(HAL_CLOCK_CG_SDIOM);
    log_hal_info("set clk final:%d, request: %d, sclk: %d, MSDC_CFG = 0x%08x MSDC_PS: 0x%08x\n",
                 msdc_config.output_clock, clock, MSDC_CLOCK_SOURCE, MSDC_REG->MSDC_CFG, MSDC_REG->MSDC_PS);
}
...
void msdc_nvic_set(bool enable)
{
    ...
    if (enable) {
        hal_nvic_enable_irq(MSDC_IRQn);
    } else {
        hal_nvic_disable_irq(MSDC_IRQn);
    }
}
...
void msdc_interrupt_init(void)
{
    ...
    hal_nvic_register_isr_handler(MSDC_IRQn, msdc_isr);
...
{
    hal_pinmux_set_function(HAL_GPIO_6, 2);// MC0_CK
    hal_pinmux_set_function(HAL_GPIO_7, 2);// MC0_CM0
    hal_pinmux_set_function(HAL_GPIO_8, 2);// MC0_DA0
    hal_pinmux_set_function(HAL_GPIO_9, 2);// MC0_DA1
    hal_pinmux_set_function(HAL_GPIO_10, 2);// MC0_DA2
    hal_pinmux_set_function(HAL_GPIO_11, 2);// MC0_DA3

    /*PU/PD setting.*/
    /* pupd 1 indicates pull up */
    hal_gpio_set_pupd_register(HAL_GPIO_6, 0, 0, 1);
    hal_gpio_set_pupd_register(HAL_GPIO_7, 1, 1, 0);
    hal_gpio_set_pupd_register(HAL_GPIO_8, 1, 1, 0);
    hal_gpio_set_pupd_register(HAL_GPIO_9, 1, 1, 0);
    hal_gpio_set_pupd_register(HAL_GPIO_10, 1, 1, 0);
    hal_gpio_set_pupd_register(HAL_GPIO_11, 1, 1, 0);

    /*IO driving setting.*/
    hal_gpio_set_driving_current(HAL_GPIO_6, (hal_gpio_driving_current_t)MSDC_DATA_LINE_DRIVING_CAPABILITY);
    hal_gpio_set_driving_current(HAL_GPIO_7, (hal_gpio_driving_current_t)MSDC_DATA_LINE_DRIVING_CAPABILITY);
    hal_gpio_set_driving_current(HAL_GPIO_8, (hal_gpio_driving_current_t)MSDC_DATA_LINE_DRIVING_CAPABILITY);
    hal_gpio_set_driving_current(HAL_GPIO_9, (hal_gpio_driving_current_t)MSDC_DATA_LINE_DRIVING_CAPABILITY);
    hal_gpio_set_driving_current(HAL_GPIO_10, (hal_gpio_driving_current_t)MSDC_DATA_LINE_DRIVING_CAPABILITY);
...
msdc_status_t msdc_init(msdc_bus_width_t bus_width)
{
    ...
    //TODO    hal_clock_mux_select(CLK_SDIOMST_SEL, 2); /*0:xo_ck(20M), 1:bbpll2_d5_d8(24M), 2:bbpll2_d5_d4(48M)*/

    hal_clock_mux_select(HAL_CLOCK_SEL_SDIOM, CLK_SDIOM_CLKSEL_50M);
    ...
    MSDC_REG->MSDC_CFG = MSDC_REG->MSDC_CFG | MSDC_CFG_MODE | MSDC_CFG_CKPDN;
    ...
    log_hal_info("init hardware done cfg:0x%08x\r\n", MSDC_REG->MSDC_CFG);
