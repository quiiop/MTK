    ...
    *socklen = (offsetof(struct sockaddr_un, sun_path) + strlen(sockaddr->sun_path) + 1);
    ...
    if (!client) {
        ...
        if (setsockopt(*sk, SOL_SOCKET, SO_RCVBUF, &memsize, sizeof(memsize)) < 0)
            BPRINT_E("Set socket buffer size failed: %s(%d), memsize %d",
                strerror(errno), errno, memsize);
...
//---------------------------------------------------------------------------
#ifdef OS_FREERTOS
ssize_t boots_sk_send(int s, const void * buf, size_t len, struct sockaddr * to,
        char * name)
#else
