...
#define SCRIPT_HCIUSB   0x10
#define SCRIPT_HCI      0x11
#define SCRIPT_END      0x12    // script end

/** Script setting */
typedef struct script_set {
    unsigned char *script;           /** pointer to script */
    int loop;               /** loop counter */
    unsigned char *loop_pos;         /** record loop header */
    int timo;               /** timeout */
    int wait;               /** sleep */
} script_set_s;
...
typedef struct pkt_list {
    ...
    union {
        ...
        char        *hci;   /** for multiple chip interface setting */
        rssi_set_s  *rssi_s;/** for rssi setting */
    } u_cnt;
    size_t      len;
    uint8_t     xfer_idx;   /** transfer index for script TX/RX to multiple interface */
    uint8_t     *q_mask;
...
void boots_pkt_node_pop(pkt_list_s **list, void *buf, size_t *len);
bool boots_pkt_node_swap(pkt_list_s **list, uint8_t *buf);
void boots_pkt_list_destroy(pkt_list_s *list);
size_t boots_pkt_list_amount(pkt_list_s *list);

int boots_pkt_handler(boots_buf_s *buf, /*size_t *len,*/ pkt_list_s **pList,
        uint8_t isRX, int ifidx);
