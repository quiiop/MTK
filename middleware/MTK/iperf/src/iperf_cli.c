...
static uint8_t _cli_iperf_server(uint8_t len, char *param[])
{
    ...
    for (i = 0; i < 13 && i < len; i++) {
        if (strcmp(param[i], "-u") == 0) {
            ...
            if (pdPASS != xTaskCreate((TaskFunction_t)iperf_udp_run_server, "iperf_udp_s",
                                      IPERF_TASK_STACKSIZE / sizeof(portSTACK_TYPE), g_iperf_param, IPERF_TASK_PRIO, NULL))
                IPERF_LOGI("Iperf UDP Server: Failed!");
            is_create_task = 1;
            break;
        }
    ...
    if (0 == is_create_task) {
        ...
        if (pdPASS != xTaskCreate((TaskFunction_t)iperf_tcp_run_server, "iperf_tcp_s",
                                  IPERF_TASK_STACKSIZE / sizeof(portSTACK_TYPE), g_iperf_param, IPERF_TASK_PRIO, NULL))
            IPERF_LOGI("Iperf TCP Server: Failed!");
    }
...
static uint8_t _cli_iperf_client(uint8_t len, char *param[])
{
    ...
    for (i = 0; i < 18 && i < len; i++) {
        if (strcmp(param[i], "-u") == 0) {
            ...
            if (pdPASS != xTaskCreate((TaskFunction_t)iperf_udp_run_client, "iperf_udp_c",
                                      IPERF_TASK_STACKSIZE / sizeof(portSTACK_TYPE), g_iperf_param, IPERF_TASK_PRIO, NULL))
                IPERF_LOGI("Iperf UDP Client: Failed!");
            is_create_task = 1;
            break;
        }
    }
    ...
    if (0 == is_create_task) {
        ...
        if (pdPASS != xTaskCreate((TaskFunction_t)iperf_tcp_run_client, IPERF_TASK_NAME,
                                  IPERF_TASK_STACKSIZE / sizeof(portSTACK_TYPE), g_iperf_param, IPERF_TASK_PRIO,
                                  (TaskHandle_t *)&g_balance_ctr.tx_handle))
            IPERF_LOGI("Iperf TCP Client: Failed!");
#else /* #if (CFG_CONNSYS_TRX_BALANCE_EN == 1) */
        if (pdPASS != xTaskCreate((TaskFunction_t)iperf_tcp_run_client, "iperf_tcp_c",
                                  IPERF_TASK_STACKSIZE / sizeof(portSTACK_TYPE), g_iperf_param, IPERF_TASK_PRIO, NULL))
            IPERF_LOGI("Iperf TCP Client: Failed!");
#endif /* #if (CFG_CONNSYS_TRX_BALANCE_EN == 1) */
    }
...
#endif /* #ifdef MTK_DEBUG_MODE_ENABLE */
    IPERF_LOGI("Example:");
