...
#else /* #ifdef IPERF_MODULE_PRINTF */
...
#endif /* #ifdef IPERF_MODULE_PRINTF */

#define UDPUL_RTY_LIMIT 1000000 /* us */
...
static void iperf_get_current_time(uint32_t *s, uint32_t *ms, uint32_t start_count);
...
void iperf_udp_run_server(char *parameters[])
{
    ...
    int tmp = 0;
#endif /* #if defined(MTK_IPERF_DEBUG_ENABLE) */
    char *buffer = NULL;
...
#else /* #ifdef MTK_TCPIP_FOR_NB_MODULE_ENABLE */
    timeout.tv_sec = 20; //set recvive timeout = 20(sec)
#endif /* #ifdef MTK_TCPIP_FOR_NB_MODULE_ENABLE */
    timeout.tv_usec = 0;
    ...
    if (g_iperf_is_tradeoff_test_client == 0) {
        for (i = 0; i < 13; i++) {
            if (strcmp((char *)&parameters[i * offset], "-p") == 0) {
                ...
                } else if (data_size < (int)(sizeof(UDP_datagram) + sizeof(client_hdr))) {
...
#endif /* #if LWIP_IPV6 */
    buffer = SYS_MALLOC(IPERF_TEST_BUFFER_SIZE);
    if (buffer == NULL) {
    ...
    do {
        // Handles request
        do {
            iperf_get_current_time(&offset_t1, 0, start_count);

            SLA_CustomLogging("UDL", SA_START);
            SLA_CustomLogging("UDL", SA_LABEL);
            if (ipv6_flag == 0) {
...
#endif /* #if LWIP_IPV6 */
            SLA_CustomLogging("UDL", SA_STOP);

            iperf_get_current_time(&offset_t2, 0, start_count);
...
#endif /* #if defined(IPERF_DEBUG_INTERNAL) */
            ...
            if (pkt_count.times == 1) {
                iperf_get_current_time(&t1, &t1_h_ms, start_count);
                t1_h_ms = (t1_h_ms / 100) % 10;
            }

            // Report by second
            if (pkt_count.times >= 1) {
                iperf_get_current_time(&curr_t, &curr_h_ms, start_count);
            ...
            if ((is_test_started == 0) && (udp_h_id > 0) && (nbytes > 0)) {
                ...
                if (pkt_count.times >= 1) {
                    ...
                    if (interval_tag > 0) {
                        ...
                        iperf_get_current_time(&t2, &t2_h_ms, start_count);
                ...
                client_h = (client_hdr *)&buffer[12];
                client_h_trans.flags = (int32_t)(ntohl(client_h->flags));
#if 0   /* maskedout redirect traffic based on iperf version keep it simple */
                // Tradeoff mode
...
#endif /* #if 0   ( maskedout redirect traffic based on iperf version keep it simple ) */
    ...
    do {
        ...
        do {
            ...
            if (connfd != -1) {
                ...
                //Connection
                do {
                    SLA_CustomLogging("TDL", SA_START);
                    SLA_CustomLogging("TDL", SA_LABEL);
                    nbytes = recv(connfd, buffer, IPERF_TEST_BUFFER_SIZE, 0);
                    SLA_CustomLogging("TDL", SA_STOP);

                    iperf_calculate_result(nbytes, &pkt_count);
                    if (pkt_count.times == 1) {
                        iperf_get_current_time(&t1, 0, start_count);
                    }
#if defined(MTK_IPERF_DEBUG_ENABLE)
                    if (tmp != nbytes) {
                        DBGPRINT_IPERF(IPERF_DEBUG_RECEIVE, ("\n[%s:%d] nbytes=%d \n", __FUNCTION__, __LINE__, nbytes));
                    ...
                    if (pkt_count.times >= 1) {
                        iperf_get_current_time(&curr_t, 0, start_count);
                ...
                if (pkt_count.times >= 1) {
                    iperf_get_current_time(&t2, 0, start_count);
                }
...
void iperf_tcp_run_client(char *parameters[])
{
    ...
    do {
        SLA_CustomLogging("TUL", SA_START);
        SLA_CustomLogging("TUL", SA_LABEL);
        nbytes = send(sockfd, str, win_size, 0);
        SLA_CustomLogging("TUL", SA_STOP);

        iperf_calculate_result(nbytes, &pkt_count);
#if defined(MTK_IPERF_DEBUG_ENABLE)
        DBGPRINT_IPERF(IPERF_DEBUG_SEND, ("\n[%s:%d] nbytes=%d \n", __FUNCTION__, __LINE__, nbytes));
#endif /* #if defined(MTK_IPERF_DEBUG_ENABLE) */
        /* in case disconnect in between */
        if (nbytes < 0) {
            IPERF_LOGI("\n[%s:%d] nbytes=%d \n", __FUNCTION__, __LINE__, nbytes);
            break;
        }
...
void iperf_udp_run_client(char *parameters[])
{
    ...
    int offset = IPERF_COMMAND_BUFFER_SIZE / sizeof(char *);
    int ret = 0;
    int last_datagram_retry = 0;
    uint32_t __start, __now, __dur;
    uint32_t start_count = 0;
    char *endptr;
    ...
    if (g_iperf_is_tradeoff_test_server == 0) {
        ...
        for (i = 1; i < 18; i++) {
            if (strcmp((char *)&parameters[i * offset], "-l") == 0) {
                ...
                i++;
                tos = strtol((char *)&parameters[i * offset], &endptr, 0);
                /* prevent coverity issue */
                if (tos < 0) {
                    IPERF_LOGE("TOS is set to an invalid value\n");
                    return;
                }
                IPERF_LOGI("Set TOS = %d \n", tos);
            } else if (strcmp((char *)&parameters[i * offset], "-b") == 0) {
...
#if 0 /* why we need to limit bw under 20Mbps */
                if (bw > 2621440 || bw <= 0) {
                    bw = 2621440;
                    IPERF_LOGI("Upper limit of bandwith setting = 20Mbits/sec\n");
                }
#endif /* #if 0 ( why we need to limit bw under 20Mbps ) */
    ...
    iperf_get_current_time(&t1, &t1_ms, start_count);
    last_tick = t1_ms;
    last_sleep = 0;
    wifi_config_update_tx_policy(CFG_TX_POLICY_DROP_RET_FAIL, 0xff);
    ...
    do {
        ...
        SLA_CustomLogging("UUL", SA_START);
        SLA_CustomLogging("UUL", SA_LABEL);
        nbytes = send(sockfd, str, data_size, 0);
        SLA_CustomLogging("UUL", SA_STOP);

        if (nbytes == -1 && errno == err_to_errno(ERR_MEM)) {
            ...
            do {
                /* retry if out of memory */
                nbytes = send(sockfd, str, data_size, 0);
                hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &__now);
                hal_gpt_get_duration_count(__start, __now, &__dur);
            } while (nbytes == -1 && errno == err_to_errno(ERR_MEM) &&
                     __dur < UDPUL_RTY_LIMIT);
        }

        iperf_calculate_result(nbytes, &pkt_count);

        iperf_get_current_time(&curr_t, &current_tick, start_count);

        if (pkt_delay_offset && (udp_h_id % pkt_delay_offset) == 0) {
        ...
        iperf_get_current_time(&curr_t, &current_tick, start_count);
    } while ((int)(current_tick + pkt_delay - t1_ms) < send_time * 1000);
    ...
    nbytes = 0;
    nbytes = send(sockfd, str, data_size, 0);
    while ((nbytes != data_size) && last_datagram_retry < 10) {
        vTaskDelay(10 / portTICK_PERIOD_MS);
        last_datagram_retry++;
        nbytes = send(sockfd, str, data_size, 0);
    }
    IPERF_LOGI(
        "sockfd send the last datagram [%d] bytes, retry [%d]...",
        nbytes, last_datagram_retry);
    wifi_config_update_tx_policy(CFG_TX_POLICY_DROP_RET_SUCCESS, 0xff);
...
static void iperf_display_report(char *report_title, unsigned time, unsigned h_ms_time, count_t *pkt_count)
{
    ...
    char s[9] = {0};
    double tput = 0.0;
    int conv = 0, resultIdx = 0;

    if (tmp_time == 0) {
        tmp_time = 1;
    }

    if (strlen(report_title) < IPERF_REPORT_SIZE)
    ...
    resultIdx = snprintf(g_iperf_context.result_t.total_len,
                         strlen(g_iperf_context.result_t.total_len),
                         "%s %s", s, kLabel_Byte[conv]);

    if (resultIdx < 0 || resultIdx >= (int)(sizeof(g_iperf_context.result_t.total_len))) {
        printf("get total length error");
    }
    ...
    resultIdx = snprintf(g_iperf_context.result_t.result,
                         strlen(g_iperf_context.result_t.result),
                         "%s %s/sec.", s, kLabel_bit[conv]);

    if (resultIdx < 0 || resultIdx >= (int)(sizeof(g_iperf_context.result_t.result))) {
