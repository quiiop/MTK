...
#if defined(CONFIG_MTK_COMMON) && defined(ANDROID)
...
#define DPRINT_ERR(_file, _format, arg...)  \
    LOG_E(common, _format, ##arg);
#define DPRINT_INFO(_file, _format, arg...)  \
    if(wfa_defined_debug & WFA_DEBUG_INFO) \
        LOG_I(common, _format, ##arg);
#define DPRINT_WARNING(_file, _format, arg...) \
    if(wfa_defined_debug & WFA_DEBUG_WARNING) \
        LOG_W(common, _format, ##arg);

#define DPRINT_ERR_HDR()  \
	LOG_E(common, "%s is not supported in File %s, line %d.", __func__, __FILE__, __LINE__);
#else
