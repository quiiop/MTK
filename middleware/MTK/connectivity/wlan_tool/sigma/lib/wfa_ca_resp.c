...
#endif
{
    ...
    /* P2P */
    wfaStaGetP2pDevAddressResp,          /* (WFA_STA_DISCONNECT_RESP_TLV - (50) */
    ...
    wfaStaWpsReadPinResp,                /* (WFA_STA_SET_WPS_PBC_RESP_TLV - (60) */
    ...
    wfaStaGenericResp,                   /* (WFA_STA_DISCONNECT_RESP_TLV - (70) */
...
#ifdef CONFIG_MTK_HE
    wfaStaGenericResp,                   /* WFA_STA_POWER_SAVE_RESP_TLV              */
#endif
};
...
int wfaTrafficAgentSendResp(BYTE *cmdBuf)
{
    ...
    int len = ptlv->len;
#ifdef CONFIG_MTK_COMMON
    int numStreams = len/sizeof(dutTgResponse_t);
    dutTgResponse_t *statResp = (dutTgResponse_t *)(cmdBuf + 4);
#else
...
int wfaTrafficAgentRecvStopResp(BYTE *cmdBuf)
{
...
#ifdef CONFIG_MTK_COMMON
    int numStreams = len/sizeof(dutTgResponse_t);
    dutTgResponse_t statResp[WFA_MAX_TRAFFIC_STREAMS];
#else
    int numStreams = len/sizeof(dutCmdResponse_t);
    ...
    {
#ifdef CONFIG_MTK_COMMON
        dutRsp = startRsp + i * sizeof(dutTgResponse_t);
        memcpy(&statResp[i], dutRsp, sizeof(dutTgResponse_t));
#else
...
int wfaStaScanResp(BYTE *cmdBuf)
{
    ...
    DPRINT_INFO(WFA_OUT, "Entry wfaStaScanResp... %d %d \n", setwmmResp->status, strlen(setwmmResp->cmdru.scanResult));
    switch(setwmmResp->status)
    {
    case STATUS_RUNNING:
        done = 1;
    ...
    case STATUS_COMPLETE:
        if(strlen(setwmmResp->cmdru.scanResult)!=0)
            sprintf(gRespStr, "status,COMPLETE%s\r\n",setwmmResp->cmdru.scanResult);
        else
            sprintf(gRespStr, "status,COMPLETE\r\n");
