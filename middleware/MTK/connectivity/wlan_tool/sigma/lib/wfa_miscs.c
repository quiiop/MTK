...
#else
#ifdef MTK_MINISUPP_ENABLE
inline double wfa_ftime_diff(struct os_time *t1, struct os_time *t2)
{
    double dtime;
...
#else
#ifdef MTK_MINISUPP_ENABLE
int wfa_itime_diff(struct os_time *t1, struct os_time *t2)
{
    int dtime;
...
int wfa_estimate_timer_latency()
{
    ...
    return latency = (t2.sec - tp2.sec) * 1000000 + (t2.usec - tp2.usec);
}
#endif
#endif
