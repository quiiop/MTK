...
#ifdef _FREERTOS
/* send response */
int send_response(unsigned char *resBuf, int respLen)
{
	uint32_t write_cnt = 0;
	uint32_t nbytes = 0;
	int send_done = 0;
	int i;
	while((write_cnt < respLen) && (send_done == 0)) {
		nbytes = hal_uart_send_dma(DUT_CMD_UART_PORT, resBuf + write_cnt, respLen - write_cnt);
		write_cnt += nbytes;
...
void wfaSetThreadPrio(int tid, int userPriority)
{
    int task_priority = DUT_TG_TASK_PRI;
    ...
    case TG_WMM_AC_VO:
        ...
        break;
    case TG_WMM_AC_VI:
        task_priority = TASK_PRIORITY_ABOVE_NORMAL;
        break;
    case TG_WMM_AC_BE:
        task_priority = TASK_PRIORITY_NORMAL;
        break;
    case TG_WMM_AC_BK:
        task_priority = TASK_PRIORITY_BELOW_NORMAL;
        break;
    default:
        /* default */
        ;
...
void  wfaSentStatsResp(int sock, BYTE *buf)
{
    ...
    dutTgResponse_t *sendStatsResp = (dutTgResponse_t *)buf, *first;
    int resp_size = sizeof(dutTgResponse_t);
#else
    dutCmdResponse_t *sendStatsResp = (dutCmdResponse_t *)buf, *first;
    int resp_size = sizeof(dutCmdResponse_t);
#endif
    ...
    wfaEncodeTLV(WFA_TRAFFIC_AGENT_SEND_RESP_TLV, total*resp_size,
                 (BYTE *)first, (BYTE *)buff);

    pkLen = WFA_TLV_HDR_LEN + total*resp_size;
    printf("pkLen %i\n", pkLen);
...
#else
    if (send_response((BYTE *)buff, pkLen) != pkLen)
#endif
    {
...
void
#endif
wfa_wmm_thread(void *thr_param)
{
#ifndef _FREERTOS
    int myId = ((tgThrData_t *)thr_param)->tid;
#else
    int myId = (int)thr_param;
#endif
...
//#endif
    ...
    while(1)
    {
        ...
        my_wmm->thr_flag=0;
#else
        xEventGroupWaitBits(my_wmm->thr_flag_wait, WFA_TG_BIT, pdTRUE,
...
#else
                                if (send_response(respBuf, respLen) != respLen)
#endif
                                {
...
#else
                        if (send_response((BYTE *)respBuf, respLen) != respLen)
#endif
                        {
                            DPRINT_WARNING(WFA_WNG, "wfaCtrlSend Error\n");
                        }
...
#ifdef _FREERTOS
        vTaskPrioritySet(NULL, DUT_TG_TASK_PRI); /* Reset task priority to default */
#endif
    }
#ifdef _FREERTOS
    vTaskDelete(NULL);
