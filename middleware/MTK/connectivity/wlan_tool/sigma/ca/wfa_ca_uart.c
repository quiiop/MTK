...
int gUartfd = -1, tmsockfd, gCaSockfd = -1, xcSockfd, btSockfd;
...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		sprintf(respStr, "%s/getipconfig.sh %s/.lan_ip %s", BIN_DIR, BIN_DIR, argv[1]);
		DPRINT_INFO(WFA_OUT, "%s", respStr);
		system(respStr);

		tmpfile = fopen(BIN_DIR"/.lan_ip", "r+");
		if(tmpfile == NULL)
		{
			DPRINT_ERR(WFA_ERR, "Can't open %s/.lan_ip\n",BIN_DIR);
			exit(1);
		}

		for (;;) {
			if(fgets(respStr, WFA_BUFF_32, tmpfile) == NULL) {
				return WFA_FAILURE;
			}
			if (strncmp(respStr, "ipaddr", 6) == 0) {
				tstr = strtok(respStr, "=");
				tstr = strtok(NULL, "=");
				/* get ipaddr */
				if(tstr) {
		...
		servIP= tstr;
		servPort = 8000;
		DPRINT_INFO(WFA_OUT, "servIP = %s, servPort = %d\n", servIP, servPort);
#else
	...
	if((tstr = getenv("WFA_ENV_UART_PORT")) == NULL)
	{
		DPRINT_ERR(WFA_ERR, "Environment variable WFA_ENV_UART_PORT not set or specify /dev/ttyUSB?\n");
		exit(1);
	}
	...
	if(gUartfd == -1)
	{
		gUartfd = open(uartPort, O_RDWR | O_NOCTTY | O_SYNC);
		if (gUartfd < 0) {
			DPRINT_ERR(WFA_ERR, "Error opening %s: %s\n", uartPort, strerror(errno));
			exit(1);
		}

		set_interface_attribs(gUartfd, B921600);
	}
	...
	for(;;)
	{
		...
		if(gCaSockfd > 0 && FD_ISSET(gCaSockfd, &sockSet))
		{
			 ...
			 * send to DUT.
			 */
			if (write(gUartfd, pcmdBuf, cmdLen) != cmdLen) {
				DPRINT_WARNING(WFA_WNG, "Incorrect sending ...\n");
				continue;
			}
			tcdrain(gUartfd);    /* delay for output */
		...
		if(gUartfd > 0 && FD_ISSET(gUartfd, &sockSet))
		{
			DPRINT_INFO(WFA_OUT, "received from DUT\n");
			sleep(1);
			memset(respStr, 0, WFA_BUFF_512);
			memset(caCmdBuf, 0, WFA_BUFF_4K);
			if ((bytesRcvd = read(gUartfd, caCmdBuf + caCmdBufPos, WFA_BUFF_4K)) <= 0)
			{
				DPRINT_WARNING(WFA_WNG, "receive failed\n");
				caCmdBufPos = 0;
				continue;
			}
...
#if DEBUG
			DPRINT_INFO(WFA_OUT, "receive data: %d\n", bytesRcvd);
