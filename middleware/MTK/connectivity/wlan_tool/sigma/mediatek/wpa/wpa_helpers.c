...
#if defined(_FREERTOS) && !defined(CONFIG_CTRL_IFACE)
...
int wpa_command(const char *ifname, const char *cmd)
{
    ...
    snprintf(buf, sizeof(buf), "wpa_cli %s", cmd);
    printf("Test: %s\n", buf);
    *token = strtok(buf, " ");
    //printf("%s ", *token);
...
#else
	if (wpa_ctrl_request(ctrl, "STATUS", 16, buf, &len, NULL) < 0) {
#endif /* ANDROID */
		wpa_ctrl_close(ctrl);
		return -1;
	}
