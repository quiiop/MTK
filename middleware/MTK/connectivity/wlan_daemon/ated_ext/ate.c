...
static void SanityCheck(UINT16 command_id, UINT16 sequence, UINT16 len);
static void SanityCheckATE(UINT16 command_id, UINT16 sequence, UINT16 len);
...
static void NetReceive(UINT8 *inpkt, int len)
{
	...
	UINT16					Command_Id;
	UINT16					Sequence;
	UINT16				Len;	/*Data Length in Command*/
	struct				iwreq pwrq;
	int				s;
	pid_t               pid;
	...
	if (Command_Id == RACFG_CMD_ATE_STOP) {
		...
		os_memcpy(&p_racfgh->data[0], (UINT8 *)&pid, sizeof(pid));
		/* We have content now. */
		Len += sizeof(pid);
