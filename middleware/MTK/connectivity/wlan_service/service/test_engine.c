...
/* phy rates comes from NSS = 1, short GI */
	...
	struct test_ru_info *ru_info)
{
	...
	if (tx_mode == TEST_MODE_CCK) {
		...
		} else if (bw == TEST_BW_10) {	/* 7915 occupy 0x4 as 160DBW */
...
static s_int32 mt_engine_store_tx_info(
	...
	struct test_tx_info *tx_info)
{
	s_int32 ret = SERV_STATUS_ENGINE_FAIL, sta_idx;
	...
	u_char band_idx)
{
	...
	u_int32 op_mode, tx_cnt, ipg, pkt_tx_time;
	/* u_int32 mcs, duty_cycle; */
	u_char *test_pkt, stack_idx = 0;
	u_char ctrl_ch, ch, dbw;
	/* u_char bw; */
