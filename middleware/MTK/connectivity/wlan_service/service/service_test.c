...
#endif
	ops->op_get_rx_ok = mt_op_get_rx_ok;
	ops->op_set_security = mt_op_set_security;
	ops->op_set_LowPower_mode = mt_op_set_LowPower_mode;
...
s_int32 mt_serv_set_security(struct service_test *serv_test)
{
	...
	/* set security */
	ret = ops->op_set_security(winfos, configs);
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE, ("%s\n", __func__));
...
s_int32 mt_serv_set_channel(struct service_test *serv_test)
{
	...
	const s_int8 bw40_sel[] = { -2, 2, 6, 10, 14, 18, 22, 26};
	const s_int8 bw80_sel[] = { -6, -2, 2, 6, 10, 14, 18, 22};
...
s_int32 mt_serv_get_rx_ok_data(
	struct service_test *serv_test)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	...
	ret = serv_test->test_op->op_get_rx_ok(
		serv_test->test_winfo,
		test_configs_band0);
	...
	struct test_rx_stat_u *st)
{
	...
	case TEST_RX_STAT_BAND:
		/* blk_idx no use in case TEST_RX_STAT_BAND */
		...
		sys_ad_move_mem(st, stat->rx_st_band,
	...
	struct service_test *serv_test)
{
	...
	ops = serv_test->test_op;
	if (serv_test->test_bstat.band_mode == TEST_BAND_MODE_DUAL)
	...
	if (!serv_test->engine_offload) {
		...
		SET_TEST_DBDC(serv_test->test_winfo, dbdc_enb);
	...
	struct service_test *serv_test, u_char pfmu_idx, u_char nc)
{
	...
	s_int32 ret = SERV_STATUS_SUCCESS;
	u_char wcid;
	u_char nr, ndp_nss, pfmu_mem_row[4], pfmu_mem_col[4];
	...
	/* Configure WTBL */
	addr1 = (u_char *)CONFIG_GET_PADDR(serv_test,
