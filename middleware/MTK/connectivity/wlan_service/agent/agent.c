...
u_char *agent_trtok(u_char *s, const u_char *ct)
{
	...
	sbegin += strspn((char *)sbegin, (const char *)ct);
	...
	send = (u_char *)strpbrk((char *)sbegin, (const char *)ct);
				  ...
				  s_int32 *argc, s_int8 *argv[])
{
	...
	if (strnlen((char *)cmd_line, 512) >= 512) {
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
					("cmd_line >= 512\n"));
...
static s_int32 hqa_get_rx_ok_data(
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	u_int32 rx_cnt0 = 0;
	...
	ret = mt_serv_get_rx_ok_data(serv_test);
	/* Get parameters */
	rx_cnt0 = CONFIG_GET_PARAM(serv_test, rx_ok_count,
				TEST_DBDC_BAND0);

	rx_cnt0 = SERV_OS_HTONL(rx_cnt0);

	/* Update hqa_frame with response: status (2 bytes) */
	sys_ad_move_mem((hqa_frame->data + 2),
			&rx_cnt0, sizeof(rx_cnt0));
	update_hqa_frame(hqa_frame,
			2 + sizeof(rx_cnt0), ret);
	return ret;
}
...
static s_int32 hqa_set_buffer_bin(
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	struct test_eeprom *test_eprms = &serv_test->test_eprm;
	...
	test_eprms->bufferBinVal = (u_char)val;

	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE,
		("%s: buffer bin mode = %d\n", __func__, test_eprms->bufferBinVal));
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	/* Set parameters */
#ifndef DBDC_MODE
	SERV_SET_PARAM(serv_test, ctrl_band_idx, TEST_DBDC_BAND0);
#endif
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	u_int32 *content = NULL, *ori_addr = NULL;
	...
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE,
		("%s: band_idx: %d, rxv_cnt: %d, rxv_dw_num: %d, ret: %d\n",
		__func__, band_idx, recal_cnt, recal_dw_num, ret));
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE,
		("%s: band_idx: %d, rxv_cnt: %d, rxv_dw_num: %d, ret: %d\n",
		__func__, band_idx, rxv_cnt, rxv_dw_num, ret));
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE,
		("%s: band_idx: %d, pulse number: %d, rdd buffer size: %d, ret: %d\n",
		__func__, band_idx, rdd_cnt, rdd_dw_num, ret));
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	switch (action) {
	case HQA_TXBF_INIT:
		snprintf(cmd, sizeof(char) * (HQA_BF_STR_SIZE),
		"%d", (u_char)arg[0]);
	...
	case HQA_IBF_TX:
		...
		/* fgBf:WLAN idx:Txcnt */
		snprintf(cmd, sizeof(char) * (HQA_BF_STR_SIZE),
		"%02x:%02x:%02x:%02x", arg[0], arg[1], arg[2], arg[3]);
	...
	struct service_test *serv_test, u_char *arg)
{
	...
	addr = (u_char *)CONFIG_GET_PADDR(serv_test,
						addr1[wcid-1], ctrl_band_idx);
...
static s_int32 hqa_set_security(
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	u_char *data = hqa_frame->data;
	u_int32 extid, para_num, bandsel, cipher_mode, key_id, key_len;

	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE, ("%s\n", __func__));

	get_param_and_shift_buf(TRUE, sizeof(extid),
				&data, (u_char *)&extid);
	get_param_and_shift_buf(TRUE, sizeof(para_num),
				&data, (u_char *)&para_num);
	get_param_and_shift_buf(TRUE, sizeof(bandsel),
				&data, (u_char *)&bandsel);
	get_param_and_shift_buf(TRUE, sizeof(cipher_mode),
				&data, (u_char *)&cipher_mode);
	get_param_and_shift_buf(TRUE, sizeof(key_id),
				&data, (u_char *)&key_id);
	get_param_and_shift_buf(TRUE, sizeof(key_len),
				&data, (u_char *)&key_len);

	do
	{
		if (bandsel >= TEST_DBDC_BAND_NUM)
		{
			ret = SERV_STATUS_AGENT_INVALID_BANDIDX;
			SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
				("%s: bandsel: %d",
		...
		/* Set parameters */
		SERV_SET_PARAM(serv_test, ctrl_band_idx, (u_char)bandsel);
		CONFIG_SET_PARAM(serv_test, tc_cipher_mode,
				(u_char)cipher_mode, bandsel);
		CONFIG_SET_PARAM(serv_test, tc_key_id,
				(u_char)key_id, bandsel);
		CONFIG_SET_PARAM(serv_test, tc_key_len,
				(u_int16)key_len, bandsel);

		ret = mt_serv_set_security(serv_test);

		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_OFF,
			("%s: band_idx: %d, cipher_mode: %d, key_id: %d, key_len: %d, ret: %d\n",
			__func__, bandsel, cipher_mode, key_id,
			key_len, ret));
	} while(0);

	/* Update hqa_frame with response: status (2 bytes) */
	sys_ad_move_mem(hqa_frame->data + 2, (u_char *) &extid, 4);
	update_hqa_frame(hqa_frame, 6, ret);

	return ret;
}

static struct hqa_cmd_entry CMD_SET7[] = {
	/* cmd id start from 0x1700 */
	{0x1,	hqa_set_security}
};
	...
	s_int8 *hqa_frame_string, struct hqa_frame *hqa_frame)
{
	...
	s_int32 i4argc = 0;
	char *apc_argv[AGENT_CFG_ARGV_MAX] = { 0 };
	u_char tmpdata[100] = { 0 };
	s_int8 tmp_mac[SERV_MAC_ADDR_LEN] = { 0 };
	u_int16 tmp_length = 0;
	u_int32 tmp_value32 = 0;
	u_int16 tmp_value16 = 0;
	u_int8 tmp_value8 = 0;
	u_char *data = NULL;
	s_int32 ret = 0;
	u_int8 parasize = 0;
	...
	for (i = 0; i < sizeof(priv_hqa_cmd_mapping) / sizeof(struct
		...
		if (strncasecmp((char *)hqa_frame_string,
			(char *)priv_hqa_cmd_mapping[i].cmd_str,
			strlen((char *)priv_hqa_cmd_mapping[i].cmd_str)) == 0) {
		...
		if (i4argc) {
			...
			tmp_length += parasize;
				...
				if (parasize == SERV_MAC_ADDR_LEN) {
					...
					kstrtou32(apc_argv[j], 0, &tmp_value32);
					if(tmp_value32 >= 0)
						set_param_and_shift_buf(TRUE, parasize,
							(u_char *)&tmp_value32, &data);
				} else if (parasize == 2) {
					kstrtou16(apc_argv[j], 0, &tmp_value16);
					if (tmp_value16 >= 0)
						set_param_and_shift_buf(TRUE, parasize,
							(u_char *)&tmp_value16, &data);
				} else {
					kstrtou8(apc_argv[j], 0, &tmp_value8);
					if (tmp_value8 >= 0)
						set_param_and_shift_buf(TRUE, parasize,
							(u_char *)&tmp_value8, &data);
				}
			}
		}
	...
	struct service *serv, struct hqa_frame_ctrl *hqa_frame_ctrl)
{
	...
	while (table_idx < (sizeof(CMD_TABLES) / sizeof(struct hqa_cmd_table))) {
		if ((cmd_id & 0xff00) == CMD_TABLES[table_idx].cmd_offset) {
			...
			if (CMD_TABLES[table_idx].cmd_offset == 0x1700) {
				sys_ad_move_mem(&ext_cmd_id,
						hqa_frame->data,
						sizeof(ext_cmd_id));
				cmd_id = SERV_OS_NTOHL(ext_cmd_id);
...
s_int32 mt_agent_set_bw(struct service_test *serv_test, u_char *arg)
{
	...
	for (i = 0, value = agent_trtok(arg, (u_char *)":");
		value;
		value = agent_trtok(NULL, (u_char *)":"), i++) {
		if (i == 2)
			break;
	...
	struct service_test *serv_test, u_char *arg)
{
	...
	for (i = 0, tok = agent_trtok(arg, (u_char *)":");
		tok;
		tok = agent_trtok(NULL, (u_char *)":"), i++) {
		if (i == 4)
			break;
...
{
	s_int32 ret = SERV_STATUS_AGENT_INVALID_PARAM, input_cnt = 0;
	u_char i = 0, band_idx;
	char *value = NULL;
	struct test_ru_info *ru_info = NULL;
	...
	if (strlen((char *)arg) > 0) {
		...
		for (i = 0, value = (char *)agent_trtok(arg, (u_char *)":");
			value;
			value = (char *)agent_trtok(NULL, (u_char *)":"), i++) {
			...
			if (strlen((char *)value) > 0 && input_cnt == 9) {
