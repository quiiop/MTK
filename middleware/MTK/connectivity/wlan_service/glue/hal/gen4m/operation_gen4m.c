	...
	struct test_configuration *configs)
{
	...
	if (ret != SERV_STATUS_SUCCESS)
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_WARN,
			 ("%s: RF_AT_FUNCID_SET_DBDC_BAND_IDX:%d\n",
			  __func__, ret));
	...
	struct test_configuration *configs)
{
	...
	if (configs->tx_mode == TEST_MODE_HE_MU) {
		...
		if (ret != SERV_STATUS_SUCCESS)
			SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_WARN,
				 ("%s: RF_AT_FUNCID_SET_RX_MU_AID:%d\n",
				  __func__, ret));
	}
	...
	struct test_configuration *test_configs_band0)
{
	...
	rf_at_info.func_idx = RF_AT_FUNCID_RXOK_COUNT;
	rf_at_info.func_data = 0;
	ret = tm_rftest_query_auto_test(winfos,
			&rf_at_info, &buf_len);
	if (ret == SERV_STATUS_SUCCESS)
		test_configs_band0->rx_ok_count = rf_at_info.func_data;

	// now we dont use band1
	return ret;
}
	...
	struct test_configuration *configs)
{
	...
	tc_cipher_mode = configs->tc_cipher_mode;
	tc_key_id = configs->tc_key_id;
	tc_key_len = configs->tc_key_len;
	u4Data = tc_key_len << 16 | tc_key_id << 8 | tc_cipher_mode;

	tm_rftest_set_auto_test(winfos,
		RF_AT_FUNCID_SET_SECURITY_MODE, u4Data);
	...
	u_int32 Control)
{
	...
	if(Control == 1)
		...
		u4Data = 0x00640001;    //u2 bcn interval, u1 mode, u1 enable

	ret =   tm_rftest_set_auto_test(winfos,
		RF_AT_FUNCID_SET_LP_MODE, u4Data);
	...
	u_int8 *data)
{
	...
	pr_rbist_info->emi_start_addr = winfos->icap_emi_start_addr;
	pr_rbist_info->emi_end_addr = winfos->icap_emi_end_addr;
	pr_rbist_info->emi_msb_addr = winfos->icap_emi_msb_addr;
	...
	u_int32 iq_type)
{
	...
	r_dump_iq.picap_data = (u_int8 *)icap_data;
