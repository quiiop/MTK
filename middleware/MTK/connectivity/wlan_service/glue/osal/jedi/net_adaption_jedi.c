...
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	struct serv_os_task *task = (struct serv_os_task *) context;
	...
	u_char band_idx)
{
	...
	if (IS_AXE(ad) || IS_MT7915(ad) || IS_MT7986(ad))
	...
	struct test_configuration *configs)
{
	...
	u_int8 ac_num, aifsn;
#if defined(WIFI_UNIFIED_COMMAND) || defined(CONNAC3_DRV)
	struct wifi_dev *wdev = (struct wifi_dev *)configs->stack.virtual_device[0];
#endif /* WIFI_UNIFIED_COMMAND || CONNAC3_DRV */
	...
	u_char mode)
{
	...
	GET_PAD_FROM_NET_DEV(ad, winfos->net_dev);
	if (ad == NULL)
		ret = SERV_STATUS_HAL_MAC_INVALID_PAD;
	else {
	...
	struct test_tmr_info *tmr_info)
{
	...
	snprintf(tmr_setting, sizeof(tmr_setting),
	"%d", tmr_info->setting);
	snprintf(tmr_hw_version, sizeof(tmr_hw_version),
	"%d", tmr_info->version);
