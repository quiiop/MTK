...
log_create_module(mesh_bearer, PRINT_LEVEL_ERROR);
log_create_module(mesh_bearer_gatt, PRINT_LEVEL_WARNING);
log_create_module(mesh_network, PRINT_LEVEL_WARNING);
log_create_module(mesh_transport, PRINT_LEVEL_WARNING);
log_create_module(mesh_access, PRINT_LEVEL_WARNING);
log_create_module(mesh_model, PRINT_LEVEL_INFO);
log_create_module(mesh_provision, PRINT_LEVEL_INFO);
log_create_module(mesh_beacon, PRINT_LEVEL_ERROR);
log_create_module(mesh_proxy, PRINT_LEVEL_WARNING);
log_create_module(mesh_config, PRINT_LEVEL_WARNING);
log_create_module(mesh_utils, PRINT_LEVEL_ERROR);
...
void bt_mesh_debug_log(bt_mesh_debug_log_level_t level, bt_mesh_debug_group_t group, const char *format, ...)
{
    ...
    va_start(arg, format);
    retval = vsnprintf(mesh_debug_buff, 150, format, arg);
    va_end(arg);
    if (retval < 0) {
        LOG_E(mesh_utils, "%s, vsnprintf fail, ret = %d", __func__, retval);
        return;
    }
    ...
    if (level == BT_MESH_DEBUG_LOG_LEVEL_INFO) {
        ...
        switch (group) {
            case BT_MESH_DEBUG_MUST:
                LOG_D(mesh_must, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_BEARER:
                LOG_D(mesh_bearer, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_BEARER_GATT:
                LOG_D(mesh_bearer_gatt, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_NETWORK:
                LOG_D(mesh_network, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_TRANSPORT:
                LOG_D(mesh_transport, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_ACCESS:
                LOG_D(mesh_access, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_MODEL:
                LOG_D(mesh_model, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_PROVISION:
                LOG_D(mesh_provision, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_BEACON:
                LOG_D(mesh_beacon, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_PROXY:
                LOG_D(mesh_proxy, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_CONFIG:
                LOG_D(mesh_config, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_MIDDLEWARE:
                LOG_D(mesh_middleware, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_FRIEND:
                LOG_D(mesh_friend, "%s", mesh_debug_buff);
                break;
            case BT_MESH_DEBUG_UTILS:
                LOG_D(mesh_utils, "%s", mesh_debug_buff);
                break;
            default:
                break;
        }
...
void bt_mesh_dump_hex_log(bt_mesh_debug_log_level_t level, bt_mesh_debug_group_t group, void *data, uint16_t len, const char *format,  ...)
{
    ...
    if (level == BT_MESH_DEBUG_LOG_LEVEL_ERROR) {
        switch (group) {
            case BT_MESH_DEBUG_MUST:
                LOG_HEXDUMP_E(mesh_must, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEARER:
                LOG_HEXDUMP_E(mesh_bearer, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEARER_GATT:
                LOG_HEXDUMP_E(mesh_bearer_gatt, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_NETWORK:
                LOG_HEXDUMP_E(mesh_network, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_TRANSPORT:
                LOG_HEXDUMP_E(mesh_transport, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_ACCESS:
                LOG_HEXDUMP_E(mesh_access, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_MODEL:
                LOG_HEXDUMP_E(mesh_model, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_PROVISION:
                LOG_HEXDUMP_E(mesh_provision, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEACON:
                LOG_HEXDUMP_E(mesh_beacon, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_PROXY:
                LOG_HEXDUMP_E(mesh_proxy, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_CONFIG:
                LOG_HEXDUMP_E(mesh_config, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_MIDDLEWARE:
                LOG_HEXDUMP_E(mesh_middleware, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_FRIEND:
                LOG_HEXDUMP_E(mesh_friend, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_UTILS:
                LOG_HEXDUMP_E(mesh_utils, mesh_debug_buff, data, len);
                break;
            default:
                break;
        }
        ...
        switch (group) {
            case BT_MESH_DEBUG_MUST:
                LOG_HEXDUMP_W(mesh_must, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEARER:
                LOG_HEXDUMP_W(mesh_bearer, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEARER_GATT:
                LOG_HEXDUMP_W(mesh_bearer_gatt, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_NETWORK:
                LOG_HEXDUMP_W(mesh_network, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_TRANSPORT:
                LOG_HEXDUMP_W(mesh_transport, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_ACCESS:
                LOG_HEXDUMP_W(mesh_access, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_MODEL:
                LOG_HEXDUMP_W(mesh_model, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_PROVISION:
                LOG_HEXDUMP_W(mesh_provision, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEACON:
                LOG_HEXDUMP_W(mesh_beacon, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_PROXY:
                LOG_HEXDUMP_W(mesh_proxy, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_CONFIG:
                LOG_HEXDUMP_W(mesh_config, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_MIDDLEWARE:
                LOG_HEXDUMP_W(mesh_middleware, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_FRIEND:
                LOG_HEXDUMP_W(mesh_friend, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_UTILS:
                LOG_HEXDUMP_W(mesh_utils, mesh_debug_buff, data, len);
                break;
            default:
                break;
        }
    } else if (level == BT_MESH_DEBUG_LOG_LEVEL_INFO) {
        switch (group) {
            case BT_MESH_DEBUG_MUST:
        ...
        switch (group) {
            case BT_MESH_DEBUG_MUST:
                LOG_HEXDUMP_D(mesh_must, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEARER:
                LOG_HEXDUMP_D(mesh_bearer, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEARER_GATT:
                LOG_HEXDUMP_D(mesh_bearer_gatt, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_NETWORK:
                LOG_HEXDUMP_D(mesh_network, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_TRANSPORT:
                LOG_HEXDUMP_D(mesh_transport, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_ACCESS:
                LOG_HEXDUMP_D(mesh_access, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_MODEL:
                LOG_HEXDUMP_D(mesh_model, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_PROVISION:
                LOG_HEXDUMP_D(mesh_provision, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_BEACON:
                LOG_HEXDUMP_D(mesh_beacon, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_PROXY:
                LOG_HEXDUMP_D(mesh_proxy, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_CONFIG:
                LOG_HEXDUMP_D(mesh_config, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_MIDDLEWARE:
                LOG_HEXDUMP_D(mesh_middleware, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_FRIEND:
                LOG_HEXDUMP_D(mesh_friend, mesh_debug_buff, data, len);
                break;
            case BT_MESH_DEBUG_UTILS:
                LOG_HEXDUMP_D(mesh_utils, mesh_debug_buff, data, len);
                break;
            default:
                break;
        }
...
void bt_mesh_set_log_level(bt_mesh_debug_group_t group, bt_mesh_debug_log_level_t level)
{
    switch (group) {
        case BT_MESH_DEBUG_MUST:
            log_config_print_level(mesh_must, level);
            break;
        case BT_MESH_DEBUG_BEARER:
            log_config_print_level(mesh_bearer, level);
            break;
        case BT_MESH_DEBUG_BEARER_GATT:
            log_config_print_level(mesh_bearer_gatt, level);
            break;
        case BT_MESH_DEBUG_NETWORK:
            log_config_print_level(mesh_network, level);
            break;
        case BT_MESH_DEBUG_TRANSPORT:
            log_config_print_level(mesh_transport, level);
            break;
        case BT_MESH_DEBUG_ACCESS:
            log_config_print_level(mesh_access, level);
            break;
        case BT_MESH_DEBUG_MODEL:
            log_config_print_level(mesh_model, level);
            break;
        case BT_MESH_DEBUG_PROVISION:
            log_config_print_level(mesh_provision, level);
            break;
        case BT_MESH_DEBUG_BEACON:
            log_config_print_level(mesh_beacon, level);
            break;
        case BT_MESH_DEBUG_PROXY:
            log_config_print_level(mesh_proxy, level);
            break;
        case BT_MESH_DEBUG_CONFIG:
            log_config_print_level(mesh_config, level);
            break;
        case BT_MESH_DEBUG_MIDDLEWARE:
            log_config_print_level(mesh_middleware, level);
            break;
        case BT_MESH_DEBUG_FRIEND:
            log_config_print_level(mesh_friend, level);
            break;
        case BT_MESH_DEBUG_UTILS:
            log_config_print_level(mesh_utils, level);
            break;
        default:
            break;
    }
}
#endif /* #ifdef MESH_DEBUG */
