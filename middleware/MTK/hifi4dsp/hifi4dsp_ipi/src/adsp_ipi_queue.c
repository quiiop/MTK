...
#ifdef scp_debug
#undef scp_debug
#endif /* #ifdef scp_debug */

/* debug only. might make performace degrade */
//#define ENABLE_SCP_DEBUG_LOG

#ifdef ENABLE_SCP_DEBUG_LOG
#define scp_debug(x...) if (!is_in_isr()) printf(x)
#else /* #ifdef ENABLE_SCP_DEBUG_LOG */
#define scp_debug(x...)
#endif /* #ifdef ENABLE_SCP_DEBUG_LOG */
...
bool scp_check_idx_msg_valid(
    const struct scp_msg_queue_t *msg_queue,
    const uint8_t idx_msg);

bool scp_check_queue_empty(
    const struct scp_msg_queue_t *msg_queue);

bool scp_check_queue_to_be_full(
    const struct scp_msg_queue_t *msg_queue);

uint8_t scp_get_num_messages_in_queue(
    const struct scp_msg_queue_t *msg_queue);
...
{
    if (msg_queue == NULL) {
        printf("%s(), msg_queue == NULL!! return\n", __func__);
        return false;
    }
...
bool scp_check_queue_empty(const struct scp_msg_queue_t *msg_queue)
{
    if (msg_queue == NULL) {
        printf("%s(), msg_queue == NULL!! return\n", __func__);
        return false;
...
bool scp_check_queue_to_be_full(const struct scp_msg_queue_t *msg_queue)
{
...
static void scp_dump_msg_in_queue(struct scp_msg_queue_t *msg_queue)
{
    ...
    printf("%s(), idx_r: %d, idx_w: %d, queue(%d/%d)\n",
           __func__,
           msg_queue->idx_r,
           msg_queue->idx_w,
    ...
    uint8_t *p_idx_msg)
{
    ...
    if (msg_queue == NULL || buf == NULL || p_idx_msg == NULL) {
        printf("%s(), NULL!! msg_queue: %p, buf: %p, p_idx_msg: %p\n",
    ...
    if (scp_check_queue_to_be_full(msg_queue) == true) {
        printf("ipi_id: %ld, queue overflow, idx_r: %d, idx_w: %d, drop it\n",
    ...
    if (scp_check_idx_msg_valid(msg_queue, msg_queue->idx_w) == false) {
        printf("%s(), idx_w %d is invalid!! return\n",
    ...
    *p_idx_msg = msg_queue->idx_w;
    ...
    scp_debug("%s(), scp_path: %ld, ipi_id: %ld, len: %ld, idx_r: %d, idx_w: %d, queue(%d/%d), *p_idx_msg: %d\n",
              __func__,
              msg_queue->scp_path,
              p_scp_msg->ipi_id,
              p_scp_msg->len,
...
static int scp_pop_msg(struct scp_msg_queue_t *msg_queue)
{
    ...
    struct scp_msg_t *p_scp_msg = NULL;
#endif /* #ifdef ENABLE_SCP_DEBUG_LOG ( for unused var build warning ) */

    if (msg_queue == NULL) {
        printf("%s(), NULL!! msg_queue: %p\n", __func__, msg_queue);
        return -EFAULT;
    }

    /* check queue empty */
    if (scp_check_queue_empty(msg_queue) == true) {
        printf("%s(), queue is empty, idx_r: %d, idx_w: %d\n",
               __func__,
               msg_queue->idx_r,
               msg_queue->idx_w);
        return -1;
    ...
    scp_debug("%s(), scp_path: %ld, ipi_id: %ld, len: %ld, idx_r: %d, idx_w: %d, queue(%d/%d)\n",
              __func__,
              msg_queue->scp_path,
              p_scp_msg->ipi_id,
              p_scp_msg->len,
    ...
    uint8_t *p_idx_msg)
{
    if (msg_queue == NULL || pp_scp_msg == NULL || p_idx_msg == NULL) {
        printf("%s(), NULL!! msg_queue: %p, pp_scp_msg: %p, p_idx_msg: %p\n",
    ...
    *p_idx_msg = 0xFF;
    ...
    /* check queue empty */
    if (scp_check_queue_empty(msg_queue) == true) {
        printf("%s(), queue empty, idx_r: %d, idx_w: %d\n",
    ...
    if (scp_check_idx_msg_valid(msg_queue, msg_queue->idx_r) == false) {
        printf("%s(), idx_r %d is invalid!! return\n",
...
int scp_ipi_queue_init(uint32_t target_id)
{
    ...
    for (scp_path = 0; scp_path < SCP_NUM_PATH; scp_path++) {
        ...
        ret = snprintf(task_name, sizeof(task_name), task_name_tmpl[scp_path], curr_target_idx);
        if (ret < 0) {
            printf("%s(), snprintf: return false!!\n", __func__);
            return -1;
        }
        ...
        /* check double init */
        if (msg_queue->init) {
            printf("%s(), scp_path: %ld already init!!\n", __func__, scp_path);
            continue;
        }
        msg_queue->init = true;
    ...
    int (*ipi_handler)(int id, void *data, unsigned int len))
{
    ...
    scp_debug("%s(+), ipi_id: %ld, buf: %p, len: %ld, ipi_handler: %p\n",
...
static void scp_process_msg_thread(void *data)
{
    ...
    if (msg_queue->queue == NULL) {
        printf("%s(), queue == NULL!! return\n", __func__);
        configASSERT(0);
        return;
    }

    while (1) {
        ret = xQueueReceive(msg_queue->queue, &idx_msg_from_queue, portMAX_DELAY);
        if (ret == pdFALSE) {
            printf("xQueueReceive fail!! ret == pdFALSE\n");
            continue;
        }

        retval = scp_front_msg(msg_queue, &p_scp_msg, &idx_msg);
        if (retval != 0 || idx_msg != idx_msg_from_queue) {
            printf("front fail!! idx_msg %d, idx_msg_from_queue %d, retval %d\n",
...
{
    if (p_scp_msg == NULL) {
        printf("%s(), NULL!! p_scp_msg: %p\n",
               __func__, p_scp_msg);
        return -EFAULT;
    }

    if (p_scp_msg->ipi_handler == NULL) {
        printf("%s(), NULL!! p_scp_msg->ipi_handler: %p\n",
               __func__, p_scp_msg->ipi_handler);
        return -EFAULT;
    }

    if (p_scp_msg->buf == NULL || p_scp_msg->len == 0) {
        printf("%s(), p_scp_msg->buf: %p, p_scp_msg->len: %ld\n",
