 ...
 */
typedef enum {
    BT_CM_PROFILE_SERVICE_UNKNOWN = -1, /**< Profile service type: unknown. */
    BT_CM_PROFILE_SERVICE_NONE = 0,     /**< Profile service type: none. */
    ...
    BT_CM_PROFILE_SERVICE_MAX      /**< Profile service type maximum, should not large than 31. */
 ...
 */
#define BT_MODULE_CUSTOM_CM 0xF8200000
...
typedef struct {
    ...
    bool                    connectivity_state;     /**< The current bt connectable state. */
...
typedef struct {
    ...
    bt_cm_profile_service_t         update_service;         /**< The profile service updated this time. */
 ...
 */
/**
* @brief   This function is used for show the current connected profile of current connected device.
* @return  The string that describe the connected profile.
*/
char *bt_cm_profile_parser(bt_cm_profile_service_t profile);

/**
* @brief   This function is used for show the current device status like scanmode and current connected device.
* @return                       void.
*/
...
/**
 * @brief   This function used to disconnect ACL link.
 *          If use NULL as parameter, it would diconnect all ACL Links.
 * @param[in] remote_dev    address of remote device.
 * @return                  #BT_STATUS_SUCCESS , the operation success.
 *                          #BT_STATUS_FAIL, the disconnect procodure failed.
 */
...
/**
 * @brief   This function used to set bt scan mode for visibility and accept connection ability.
 *          If the scan state change the #BT_CM_EVENT_VISIBILITY_STATE_UPDATE event while send to user.
 * @param[in] visible        open or close the visibility.
 * @param[in] connectable    open or close the accept connection ability.
 * @return                    void.
 */
void bt_cm_scan_mode(bool visible, bool connectable);

/**
 * @brief   This function used to set role of the connection.
 * @param[in] addr  is the addr of connection.
 * @param[in] role  the role you want to set. 0:Master 1:Slave
 * @return                    void.
 */
void bt_cm_switch_role(bt_bd_addr_t address, bt_role_t role);

/**
 * @brief   This function used to check role of the connection.
 * @param[in] addr  is the addr of connection.
 * @return                    void.
 */
...
#endif /* #ifndef __BT_CONNECTION_MANAGER_H__ */
