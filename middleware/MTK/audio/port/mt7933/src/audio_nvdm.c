...
#define LOGMSGIDE(fmt,arg...)   LOG_E(audio_nvdm, "[audio_nvdm]"fmt,##arg)
#define LOGMSGIDW(fmt,arg...)   LOG_W(audio_nvdm, "[audio_nvdm]"fmt,##arg)
#define LOGMSGIDI(fmt,arg...)   LOG_I(audio_nvdm, "[audio_nvdm]"fmt,##arg)
...
#endif /* #if defined(__AFE_HS_DC_CALIBRATION__) //HP can be added in the future */
...
void audio_nvdm_init()
{
    ...
    UNUSED(size);
    return;
#else /* #ifndef MTK_AUDIO_TUNING_ENABLED */
    ...
    LOGMSGIDI("audio nvdm init --\n", 0);
    UNUSED(size);
#endif /* #ifndef MTK_AUDIO_TUNING_ENABLED */
}
