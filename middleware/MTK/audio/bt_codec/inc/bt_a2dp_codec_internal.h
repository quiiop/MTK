...
#else
#define TASK_LOG_MSGID_E(fmt,arg...)      LOG_E(BT_A2DP_CODEC, "[A2DP CODEC]: "fmt,##arg)
#define TASK_LOG_MSGID_W(fmt,arg...)      /* LOG_MSGID_W(BT_A2DP_CODEC, "[A2DP CODEC]: "fmt,##arg) */
#define TASK_LOG_MSGID_I(fmt,arg...)      LOG_I(BT_A2DP_CODEC, "[A2DP CODEC]: "fmt,##arg)
#define TASK_LOG_MSGID_CTRL(fmt,arg...)   LOG_I(BT_A2DP_CODEC, "[A2DP CODEC]: "fmt,##arg)
#define LISR_LOG_MSGID_E(fmt,arg...)      LOG_E(BT_A2DP_CODEC, "[A2DP CODEC]: "fmt,##arg)
#define LISR_LOG_MSGID_W(fmt,arg...)      /* LOG_MSGID_W(BT_A2DP_CODEC, "[A2DP CODEC]: "fmt,##arg) */
#define LISR_LOG_MSGID_I(fmt,arg...)      LOG_I(BT_A2DP_CODEC, "[A2DP CODEC]: "fmt,##arg)
...
#define MAX_SBC_DECODE_FUNCTIONS  2
#define SHARE_BUFFER_LENGTH      (1024 * 2)
#define TEMP_BUFFER_LENGTH       (1024 * 2)
#define PCM_BUFFER_LENGTH        (1024 * 4)

typedef enum {
    SBC_DECODE_QUEUE_EVENT_NONE = 0,
    SBC_DECODE_QUEUE_EVENT_DECODE
} sbc_decode_queue_event_id_t;

typedef struct {
    sbc_decode_queue_event_id_t id;
    void *parameter;
} sbc_decode_queue_event_t;

typedef struct {
    uint32_t buffer_total_length;//buffer length
    uint32_t buffer_data_byte_count;//how many data in the buffer
    uint8_t *buffer_base_pointer;
} linear_buffer_information_t;

typedef struct {
    uint32_t buffer_total_length;//buffer length
    uint32_t buffer_data_byte_count;//how many data in the buffer
    uint16_t *buffer_base_pointer;
} linear_buffer_16bit_information_t;
...
typedef struct {
    ...
    linear_buffer_information_t share_buffer_info;//getting data from bt for parser input
    linear_buffer_information_t temp_buffer_info;//parser output & decoder input
    linear_buffer_16bit_information_t pcm_buffer_info;//decoder output & i2s tx input
    uint8_t *working_buffer1;//internal buffer for parser
    uint8_t *working_buffer2;//internal buffer for decoder
    uint32_t working_buffer1_size;
    uint32_t working_buffer2_size;
    void *working_buffer1_handle;//handle for parser
    void *working_buffer2_handle;//handle for decoder

} bt_a2dp_audio_internal_handle_t;

typedef void (*sbc_decode_internal_callback_t)(void *parameter);
