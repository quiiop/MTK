...
#else 
    extern BaseType_t xTaskGenericCreate(TaskFunction_t pxTaskCode, const char *const pcName, const uint16_t usStackDepth, void *const pvParameters, UBaseType_t uxPriority, TaskHandle_t *const pxCreatedTask, StackType_t *const puxStackBuffer, const MemoryRegion_t *const xRegions) PRIVILEGED_FUNCTION;    
    (void)tcb_buffer;
...
SemaPhoreHandle_t ui_shell_al_semaphore_create_binary(void)
{
...
SemaPhoreHandle_t ui_shell_al_create_mutex(void)
{
