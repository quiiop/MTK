...
static void MessageHandler_threadMethod(void* argc)
{
    ...
    ui_shell_msg_t *dalayMessage = NULL;
...
#ifdef WIN32
    if (p_handler != NULL && p_handler->m_thread_run_state == THREAD_STATE_STARTING)
#else
    if (p_handler != NULL)
#endif
    {
        ...
        while (queue) {
            ...
            if (msgToHandle) {
                if (p_handler->handleMessage != NULL) {
            ...
            if (0xFFFFFFFF == next_delay_ms) {
                ...
                dalayMessage = &(p_handler->m_DelayMessageQueue->removeTimeoutMessages(p_handler->m_DelayMessageQueue)->basic_msg);
                while (dalayMessage) {
                    queue->pushMessage(queue, false, dalayMessage);
                    last_delayMessage = dalayMessage;
                    dalayMessage = dalayMessage->next_msg;
