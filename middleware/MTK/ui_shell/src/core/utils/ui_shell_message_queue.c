        ...
        ui_shell_msg_t *input_msg)
{
    ...
    if (message_queue->mTop == NULL) {
        ...
        _current = message_queue->mTop;
        while(1) {
            if (_current == NULL || _current->msg_priority < msg->msg_priority) {
                msg->next_msg = _current;
                if (_last) {
                    _last->next_msg = msg;
                } else {
                    message_queue->mTop = msg;
...
static void removeMessages(ui_shell_msg_queue_t *message_queue, uint32_t group, uint32_t id)
{
    ...
    if (message_queue) {
        ui_shell_al_synchronized(&sync_mask);
        for (_current = message_queue->mTop; _current; ) {
            if (_current->msg_group == group && _current->msg_id == id) {
                if (_last) {
                    _last->next_msg = _current->next_msg;
                } else {
                    message_queue->mTop = _current->next_msg;
                }
