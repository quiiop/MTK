...
bt_status_t bt_sink_srv_le_music_action_handler(uint32_t action, void *param)
{
    ...
    if (NULL == srv_cntx || handle == BT_HANDLE_INVALID || !(srv_cntx->conn_mask & BT_SINK_SRV_PROFILE_A2DP_SINK)) {
        ret = BT_STATUS_FAIL;
        bt_sink_srv_report_id("[Sink]send le music action fail, handle:0x%x, srv_cntx:%p", 2, handle, srv_cntx);
        return ret;
    }
    ...
    switch (action) {
        ...
        case BT_SINK_SRV_ACTION_LE_GET_CTR_OPCODES: {
            bt_le_audio_sink_send_action(handle, BT_LE_AUDIO_SINK_ACTION_MEDIA_READ_MEDIA_CONTROL_OPCODES_SUPPORTED, &le_param);
            break;
        }
        case BT_SINK_SRV_ACTION_LE_GET_PLAYER_NAME: {
            ble_mcp_read_media_player_name_req(handle, 0xFF);
            break;
        }
        case BT_SINK_SRV_ACTION_LE_GET_TRACK_TITLE: {
            ble_mcp_read_track_title_req(handle, 0xFF);
            break;
        }
        case BT_SINK_SRV_ACTION_LE_GET_TRACK_DURATION: {
            ble_mcp_read_track_duration_req(handle, 0xFF);
            break;
        }
        case BT_SINK_SRV_ACTION_LE_GET_TRACK_POSITION: {
            ble_mcp_read_track_position_req(handle, 0xFF);
            break;
        }
#endif
    ...
    //(MTK_COMMON:Reduce log)bt_sink_srv_report_id("[Sink]le_sink_srv_music_action_handler:ret:%x", 1, ret);
    return ret;
}
...
bt_status_t bt_sink_srv_le_music_mcp_callback(bt_msg_type_t msg, bt_status_t status, void *buffer)
{
...
#ifdef MTK_COMMON_LE_MCP_EVT
    le_sink_srv_mcp_event_ind_notify(msg, status, buffer);
#endif
    ...
    switch (msg) {
        ...
        case BLE_MCP_SET_MEDIA_CONTROL_OPCODES_SUPPORTED_NOTIFICATION_CNF: {
            ...
            bt_sink_srv_report_id("[MCP][sink_music] SET_NOTIFICATION_CNF, msg:%x status:%x handle:%x srv_idx:%x", 5, msg, status, cnf->handle, cnf->service_idx);
            break;
        }
        ...
        case BLE_MCP_READ_MEDIA_PLAYER_NAME_CNF: {
            ...
            bt_sink_srv_report_id("[MCP][sink_music] READ_MEDIA_PLAYER_NAME_CNF, status:%x handle:%x srv_idx:%x len:%x", 4, status, cnf->handle, cnf->service_idx, cnf->media_player_name_length);
            break;
        }
        case BLE_MCP_READ_MEDIA_PLAYER_ICON_URL_CNF: {
            ble_mcp_read_media_player_icon_url_cnf_t *cnf = (ble_mcp_read_media_player_icon_url_cnf_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] READ_MEDIA_PLAYER_ICON_URL_CNF, status:%x handle:%x srv_idx:%x len:%x", 4, status, cnf->handle, cnf->service_idx, cnf->media_player_icon_url_length);
            break;
        }
        case BLE_MCP_READ_TRACK_TITLE_CNF: {
            ble_mcp_read_track_title_cnf_t *cnf = (ble_mcp_read_track_title_cnf_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] READ_TRACK_TITLE_CNF, status:%x handle:%x srv_idx:%x len:%x", 4, status, cnf->handle, cnf->service_idx, cnf->track_title_length);
            break;
        }
        case BLE_MCP_READ_TRACK_DURATION_CNF: {
            ble_mcp_read_track_duration_cnf_t *cnf = (ble_mcp_read_track_duration_cnf_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] READ_TRACK_DURATION_CNF, status:%x handle:%x srv_idx:%x track_duration:%x", 4, status, cnf->handle, cnf->service_idx, cnf->track_duration);
            break;
        }
        case BLE_MCP_READ_TRACK_POSITION_CNF: {
            ble_mcp_read_track_position_cnf_t *cnf = (ble_mcp_read_track_position_cnf_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] READ_TRACK_POSITION_CNF, status:%x handle:%x srv_idx:%x track_position:%x", 4, status, cnf->handle, cnf->service_idx, cnf->track_position);
            break;
        }
        case BLE_MCP_READ_PLAYBACK_SPEED_CNF: {
            ble_mcp_read_playback_speed_cnf_t *cnf = (ble_mcp_read_playback_speed_cnf_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] READ_PLAYBACK_SPEED_CNF, status:%x handle:%x srv_idx:%x playback_speed:%x", 4, status, cnf->handle, cnf->service_idx, cnf->playback_speed);
            break;
        }
        case BLE_MCP_READ_SEEKING_SPEED_CNF: {
            ble_mcp_read_seeking_speed_cnf_t *cnf = (ble_mcp_read_seeking_speed_cnf_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] READ_PLAYBACK_SPEED_CNF, status:%x handle:%x srv_idx:%x seeking_speed:%x", 4, status, cnf->handle, cnf->service_idx, cnf->seeking_speed);
            break;
        }
        case BLE_MCP_READ_PLAYING_ORDER_CNF: {
            ble_mcp_read_playing_order_cnf_t *cnf = (ble_mcp_read_playing_order_cnf_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] READ_PLAYING_ORDER_CNF, status:%x handle:%x srv_idx:%x playing_order:%x", 4, status, cnf->handle, cnf->service_idx, cnf->playing_order);
            break;
        }
        case BLE_MCP_READ_PLAYING_ORDERS_SUPPORTED_CNF: {
            ...
            bt_sink_srv_report_id("[MCP][sink_music] READ_PLAYING_ORDERS_SUPPORTED_CNF, status:%x handle:%x srv_idx:%x playing_orders_supported:%x", 4, status, cnf->handle, cnf->service_idx, cnf->playing_order_supported);
            break;
        }
        case BLE_MCP_READ_MEDIA_STATE_CNF: {
            ble_mcp_read_media_state_cnf_t *cnf = (ble_mcp_read_media_state_cnf_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] READ_MEDIA_STATE_CNF, status:%x handle:%x srv_idx:%x media_state:%x", 4, status, cnf->handle, cnf->service_idx, cnf->media_state);
            break;
        }
        case BLE_MCP_READ_MEDIA_CONTROL_OPCODES_SUPPORTED_CNF:{
            ...
            bt_sink_srv_report_id("[MCP][sink_music] READ_MEDIA_CONTROL_OPCODES_SUPPORTED_CNF, status:%x handle:%x srv_idx:%x media_control_opcodes_supported:%x", 4, status, cnf->handle, cnf->service_idx, cnf->media_control_opcodes_supported);
            break;
        }
        case BLE_MCP_READ_CONTENT_CONTROL_ID_CNF: {
            ...
            bt_sink_srv_report_id("[MCP][sink_music] READ_CONTENT_CONTROL_ID_CNF, status:%x handle:%x srv_idx:%x content_control_id:%x", 4, status, cnf->handle, cnf->service_idx, cnf->content_control_id);
            break;
        }
        case BLE_MCP_MEDIA_PLAYER_NAME_IND: {
            ble_mcp_media_player_name_ind_t *ind = (ble_mcp_media_player_name_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] MEDIA_PLAYER_NAME_IND, status:%x handle:%x srv_idx:%x len:%x", 4, status, ind->handle, ind->service_idx, ind->media_player_name_length);
            break;
        }
        case BLE_MCP_TRACK_CHANGED_IND: {
            ble_mcp_track_changed_ind_t *ind = (ble_mcp_track_changed_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] TRACK_CHANGED_IND, status:%x handle:%x srv_idx:%x", 3, status, ind->handle, ind->service_idx);
            break;
        }
        case BLE_MCP_TRACK_TITLE_IND: {
            ble_mcp_track_title_ind_t *ind = (ble_mcp_track_title_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] TRACK_TITLE_IND, status:%x handle:%x srv_idx:%x len:%x", 4, status, ind->handle, ind->service_idx, ind->track_title_length);
            break;
        }
        case BLE_MCP_TRACK_DURATION_IND: {
            ble_mcp_track_duration_ind_t *ind = (ble_mcp_track_duration_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] TRACK_DURATION_IND, status:%x handle:%x srv_idx:%x track_duration:%x", 4, status, ind->handle, ind->service_idx, ind->track_duration);
            break;
        }
        case BLE_MCP_TRACK_POSITION_IND: {
            ble_mcp_track_position_ind_t *ind = (ble_mcp_track_position_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] TRACK_POSITION_IND, status:%x handle:%x srv_idx:%x track_position:%x", 4, status, ind->handle, ind->service_idx, ind->track_position);
            break;
        }
        case BLE_MCP_PLAYBACK_SPEED_IND: {
            ble_mcp_playback_speed_ind_t *ind = (ble_mcp_playback_speed_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] PLAYBACK_SPEED_IND, status:%x handle:%x srv_idx:%x playback_speed:%x", 4, status, ind->handle, ind->service_idx, ind->playback_speed);
            break;
        }
        case BLE_MCP_SEEKING_SPEED_IND: {
            ble_mcp_seeking_speed_ind_t *ind = (ble_mcp_seeking_speed_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] SEEKING_SPEED_IND, status:%x handle:%x srv_idx:%x seeking_speed:%x", 4, status, ind->handle, ind->service_idx, ind->seeking_speed);
            break;
        }
        case BLE_MCP_PLAYING_ORDER_IND: {
            ble_mcp_playing_order_ind_t *ind = (ble_mcp_playing_order_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] PLAYING_ORDER_IND, status:%x handle:%x srv_idx:%x playing_order:%x", 4, status, ind->handle, ind->service_idx, ind->playing_order);
            break;
        }
        case BLE_MCP_MEDIA_STATE_IND: {
            ble_mcp_media_state_ind_t *ind = (ble_mcp_media_state_ind_t *)buffer;
            bt_sink_srv_report_id("[MCP][sink_music] MEDIA_STATE_IND, status:%x handle:%x srv_idx:%x media_state:%x", 4, status, ind->handle, ind->service_idx, ind->media_state);
            break;
        }
        case BLE_MCP_MEDIA_CONTROL_OPCODES_SUPPORTED_IND: {
            ...
            bt_sink_srv_report_id("[MCP][sink_music] MEDIA_CONTROL_OPCODES_SUPPORTED_IND, status:%x handle:%x srv_idx:%x media_state:%x", 4, status, ind->handle, ind->service_idx, ind->media_control_opcodes_supported);
            break;
        }
        default:
            break;
