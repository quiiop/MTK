...
static void bt_sink_srv_vcp_event_notify(ble_vcs_volume_t volume, ble_vcs_mute_t mute)
{
    ble_vcs_volume_state_t ind;

    ind.volume = volume;
    ind.mute = mute;

    bt_sink_srv_event_callback(BT_SINK_SRV_EVENT_LE_VCP_VOLUME_STATE_CHANGE,
                               &ind, sizeof(ble_vcs_volume_state_t));
}
#endif
...
#ifdef BT_VCS_VOLUME_CONSISTENT_ENABLE
...
#ifdef MTK_COMMON_LE_VCP_EVT
    /* MTK_David: Notify APP for volume change */
    bt_sink_srv_vcp_event_notify(p_vol->volume, p_vol->mute);
#endif
...
**************************************************************************************************/
#ifdef MTK_COMMON_LE_VCP_EVT
bt_status_t default_bt_sink_srv_le_volume_get_init_setting(uint8_t *volume, uint8_t *mute, uint8_t *flags)
{
    *volume = BT_SINK_LE_VOLUME_VALUE_DEFAULT;
    *mute = BLE_VCS_UNMUTE;
    *flags = BLE_VCS_VOLUME_FLAGS_SETTING_NOT_PERSISTED;

    return BT_STATUS_SUCCESS;
}

#if _MSC_VER >= 1500
#pragma comment(linker, "/alternatename:bt_sink_srv_le_volume_get_init_setting=default_bt_sink_srv_le_volume_get_init_setting")
#elif defined(__GNUC__) || defined(__ICCARM__) || defined(__CC_ARM)
#pragma weak bt_sink_srv_le_volume_get_init_setting = default_bt_sink_srv_le_volume_get_init_setting
...
bt_status_t bt_sink_srv_le_volume_init(void)
{
    ...
    bt_sink_srv_le_volume_get_init_setting(&volume, &mute, &flags);

    if (mute > BLE_VCS_MUTE)
        mute = BLE_VCS_UNMUTE;

    if (flags > BLE_VCS_VOLUME_FLAGS_SETTING_PERSISTED)
        flags = BLE_VCS_VOLUME_FLAGS_SETTING_NOT_PERSISTED;
#endif
...
{
    bool set_mute = false;
    bool set_volume = true;

    /* Set volume setting to AM */
    if (g_sink_le_volume_state[0].mute)
        set_mute = true;

    bt_sink_srv_le_adjust_am_volume_state(set_mute, g_sink_le_volume_state[0].mute,
                                          set_volume, g_sink_le_volume_state[0].volume);
