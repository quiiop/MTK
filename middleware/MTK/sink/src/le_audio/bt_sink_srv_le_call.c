...
//(MTK_COMMON:why extern from APP)extern bool bt_sink_srv_aird_support_call_mode(bt_handle_t handle);
...
#if _MSC_VER >= 1500
#pragma comment(linker, "/alternatename:bt_sink_srv_aird_support_call_mode=default_bt_sink_srv_aird_support_call_mode")
        ...
        case BT_SINK_SRV_ACTION_CALL_SET_VOLUME: {
            ...
            if (param) {
                ...
                if (BT_SINK_SRV_DEVICE_LE == sink_srv_dev->type && (sink_srv_dev->address)) {
                    bt_handle_t active_handle = bt_device_manager_le_get_hdl_by_addr(&sink_srv_dev->address);
                    if (active_handle) {
