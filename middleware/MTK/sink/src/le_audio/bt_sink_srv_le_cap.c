...
#include "bt_sink_srv_utils.h" //MTK_COMMON: Use this .h to replace bt_utils.h
...
static bt_status_t bt_sink_srv_cap_event_callback(bt_msg_type_t msg, bt_status_t status, void *buffer)
{
    ...
    switch (module) {
        ...
        case BT_MODULE_LE_AUDIO: {
            //le_audio_log("[sink cap] event callback, msg:%x, status:%x", 2, msg, status); //MTK_COMMON: David - reduce log
...
bt_status_t bt_sink_srv_cap_init(bt_sink_srv_cap_callback_t callback, uint8_t max_link_num)
{
    ...
    ble_tmap_role_t role = BLE_TMAP_ROLE_MASK_UMR;
#ifdef MTK_LE_AUDIO_CT
    role |= BLE_TMAP_ROLE_MASK_CT;
#endif
#ifdef MTK_LE_AUDIO_BMR
    role |= BLE_TMAP_ROLE_MASK_BMR;
#endif

    if (BT_STATUS_SUCCESS != bt_le_audio_sink_init(role, bt_sink_srv_cap_le_audio_callback, max_link_num)) {
        return BT_STATUS_FAIL;
    }
    ...
    //MTK_Titan: Added to reset this struct
    memset(g_sink_srv_cap_link, 0x0, sizeof(bt_sink_srv_cap_link_info_t) * max_link_num);
#endif
...
bt_status_t bt_sink_srv_cap_set_link(bt_handle_t connect_handle)
{
    ...
    //(MTK ReduceLog)le_audio_log("[sink cap] cap set link, max link num:%d", 1, g_sink_srv_cap_ctl.max_link_num);

    for (link_index = 0; link_index < g_sink_srv_cap_ctl.max_link_num; link_index++) {
        //(MTK ReduceLog)le_audio_log("[sink cap] cap set link, idx:%d, handle1:0x%x, handle2:0x%x", 3, link_index, g_sink_srv_cap_link[link_index].connect_handle, connect_handle);
        if (g_sink_srv_cap_link[link_index].connect_handle != connect_handle &&
            g_sink_srv_cap_link[link_index].connect_handle == BT_HANDLE_INVALID) {
            //MTK_COMMON: remove this dummy log
            ...
            //MTK_COMMON: change log formate due to rm above log
            le_audio_log("[sink cap] set link, index = %d, connHdl= 0x%x, addr(%x):%x-%x-%x-%x-%x-%x",
                         9, link_index, connect_handle, address.type,
...
#if 1 //MTK_COMMON: To support reason code update
bt_status_t bt_sink_srv_cap_clear_link(bt_handle_t connect_handle, bt_status_t reason)
#else
...
#if 1 //MTK_COMMON: To support reason code update
            bt_sink_srv_cap_set_state(connect_handle, BT_SINK_SRV_CAP_STATE_IDLE, reason);
#else
...
//MTK_Titan: add to show link info
void bt_sink_srv_cap_show_link_info(void)
{
    uint8_t i;
    bt_sink_srv_cap_link_info_t *p_linfo;

    if (!g_sink_srv_cap_link) {
        le_audio_log("link info did not created", 0);
        return;
    }

    le_audio_log("==== [sink cap] link info (total: %d) =====", 1, g_sink_srv_cap_ctl.max_link_num);

    for(i = 0; i < g_sink_srv_cap_ctl.max_link_num; i++)
    {
        p_linfo = &g_sink_srv_cap_link[i];
        le_audio_log("[sink cap] link[%d], hdr:0x%x addr(%d):%02x-%02x-%02x-%02x-%02x-%02x",
                     9, i, p_linfo->connect_handle, p_linfo->peer_addr.type,
                     p_linfo->peer_addr.addr[5], p_linfo->peer_addr.addr[4], p_linfo->peer_addr.addr[3],
                     p_linfo->peer_addr.addr[2], p_linfo->peer_addr.addr[1], p_linfo->peer_addr.addr[0]);
        le_audio_log("[sink cap] link[%d], state: 0x%x, sub-state: 0x%x",
                     3, i, p_linfo->state, p_linfo->sub_state);
    }

}
//(end)MTK_Titan

//MTK_David: add for disconnection procedure
bt_handle_t bt_sink_srv_cap_get_hdl_by_addr(const bt_addr_t *bt_addr)
{
    uint8_t i;
    bt_sink_srv_cap_link_info_t *p_linfo;
    ...
    for (i = 0; i < g_sink_srv_cap_ctl.max_link_num; i++) {
        p_linfo = &g_sink_srv_cap_link[i];
        if (memcmp(&p_linfo->peer_addr, bt_addr, sizeof(bt_addr_t)) == 0) {
            return p_linfo->connect_handle;
        }
    }
    return BT_HANDLE_INVALID;
}

bt_status_t bt_sink_srv_cap_disconnect_all(void)
{
    uint8_t i;
    ...
    for (i = 0; i < g_sink_srv_cap_ctl.max_link_num; i++) {
        if (g_sink_srv_cap_link[i].connect_handle != BT_HANDLE_INVALID) {
            param.connection_handle = g_sink_srv_cap_link[i].connect_handle;
            le_audio_log("[sink cap] disconnect hdl : 0x%x", 0, param.connection_handle);
            if (bt_gap_le_disconnect(&param) == BT_STATUS_SUCCESS)
                return BT_STATUS_SUCCESS;
        }
    }
...
uint8_t bt_sink_srv_cap_get_link_index(bt_handle_t connect_handle)
{
    ...
    //(MTK_COMM:rm)le_audio_log("[sink cap] get ble link index:%d, connect handle:%d", 2, index, connect_handle);
...
void bt_sink_srv_cap_set_sub_state(bt_handle_t connect_handle, uint8_t sub_state)
{
    ...
    le_audio_log("[sink cap] set sink srv sub state conn_hd:%04x, sub state:%d -> %d", 3,
...
#if 1 //MTK_COMMON: To support reason code update
void bt_sink_srv_cap_set_state(bt_handle_t connect_handle, bt_sink_srv_cap_state state, bt_status_t reason)
#else
...
#endif
{
    ...
    //MTK_COMMON: David - shrink log
    le_audio_log("[sink cap] set sink srv state, conn_hd:%04x, current state:%d, next state:%d, sub_state:%d", 4,
