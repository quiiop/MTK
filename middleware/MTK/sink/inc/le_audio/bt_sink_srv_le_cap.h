...
#if 1
//MTK_COMMON: Remove ARH's enum declaration due to it's not a proper design, and use below method to instead.
#define CAP_AM_BROADCAST_MUSIC_MODE          (CAP_AM_BROADCAST_MUSIC_MODE_START)
#define CAP_AM_MODE_NUM                      (CAP_MAX_DEVICE_NUM)
#else
enum
{
...
typedef struct {
    ...
    bt_status_t reason;                     /**< reason code. */
#endif
...
//MTK_Titan
/**
 * @brief                       This function is to show link information.
 * @return                      none.
 */
...
//MTK_David
/**
 * @brief                       This function is to get connection handle for LE Audio link.
 * @param[in] bt_addr           is address of remote device.
 * @return                      BLE connection handle.
 */
bt_handle_t bt_sink_srv_cap_get_hdl_by_addr(const bt_addr_t *bt_addr);

/**
 * @brief                       This function is used for disconnect all LE Audio links.
 * @return                      None.
 */
