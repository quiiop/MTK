...
#define DHCPD_SOCKET_RECV_TIMEOUT_MS     200
#define DHCPD_STOP_DELAY_MS              100
#define DHCPD_MS_IN_SECOND               1000
...
static bool dhcpd_mutex_new(void)
{
    if (dhcpd_mutex == NULL) {
    ...
    if (dhcpd_mutex == NULL) {
        DHCPD_PRINTF("##############################");
        DHCPD_PRINTF("dhcpd, dhcpd_mutex_new fail");
        DHCPD_PRINTF("##############################");
    }
    return (dhcpd_mutex != NULL);
}
...
int dhcpd_start(dhcpd_settings_t *dhcpd_settings)
{
    ...
    bool ret = dhcpd_mutex_new();
    if (!ret) {
        DHCPD_ERR("DHCPD dhcpd_start return");
        return -1;
    }
    dhcpd_mutex_lock();
    ...
    if (dhcpd_running == 0 && dhcpd_task_handle == 0) {
        ...
        if (pdFALSE == xTaskCreate(dhcpd_prepare, DHCPD_TASK_NAME,
                DHCPD_TASK_STACKSIZE / sizeof(portSTACK_TYPE), NULL,
                TASK_PRIORITY_NORMAL, &dhcpd_task_handle)) {
...
static int dhcpd_do_request(struct sockaddr *source_addr)
{
    ...
    if (ret != -1) {
        ...
        option_ptr = (char *)dhcpd_msg->options;
        // security patch
        size_t options_buf_len = sizeof(dhcpd_msg->options);
        unsigned int options_cur_pos = 0;
        ...
        while (*option_ptr != DHCPD_OPT_END) {
            ...
            // Make sure len is not negative number and falls within the options buffer limit
            if ((len < 0) || (options_cur_pos + len + 2) > options_buf_len) {
                DHCPD_PRINTF("Security warning - Invalid options length: %d\r\n", len);
                break;
            } else {
                option_ptr += (len + 2);
                options_cur_pos += (len + 2);
            }
        }
...
static int dhcpd_task_loop(void *arg)
{
    ...
    while (dhcpd_running == 1) {
        // security patch
        memset((char *)dhcpd_msg, 0, sizeof(dhcpd_message_t));
    ...
    DHCPD_PRINTF("dhcpd_task_loop exit, dhcpd_running=%d", dhcpd_running);
    return ret;
}
...
static void dhcpd_prepare(void *param)
{
    ...
    struct timeval timeout = {
        .tv_sec = DHCPD_SOCKET_RECV_TIMEOUT_MS / DHCPD_MS_IN_SECOND,
        .tv_usec = (DHCPD_SOCKET_RECV_TIMEOUT_MS % DHCPD_MS_IN_SECOND) * DHCPD_MS_IN_SECOND
    };
    if (setsockopt(dhcpd_socket, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout, sizeof(timeout)) < 0) {
        DHCPD_ERR("socket Setsockopt failed");
        close(dhcpd_socket);
        dhcpd_socket = -1;
        dhcpd_task_handle = 0;
        vTaskDelete(NULL);
...
{
    DHCPD_PRINTF("dhcpd_stop task_handle=%d dhcpd_running=%d", (int)dhcpd_task_handle, dhcpd_running);
    bool ret = dhcpd_mutex_new();
    if (!ret) {
        DHCPD_ERR("DHCPD dhcpd_stop return");
        return;
    }
    dhcpd_mutex_lock();
    if (dhcpd_running == 1) {
        ...
        int count = 0;
        while (dhcpd_task_handle != 0) {
            vTaskDelay(DHCPD_STOP_DELAY_MS);
            count++;
            if (count % 5 == 0) {
                DHCPD_PRINTF("stopping %d", count);
            }
        }
