...
#define BT_CLI_CMD_PARAM_LEN 200
...
{
    uint8_t i;
    uint8_t cmd[BT_CLI_CMD_PARAM_LEN];
    uint8_t tt = 0;

    for (i = 0; i < len && tt < BT_CLI_CMD_PARAM_LEN - 1; i++) {
        if (strlen(param[i]) < (unsigned int)BT_CLI_CMD_PARAM_LEN - 1 - tt) {
            memcpy(cmd + tt, param[i], strlen(param[i]));
        } else {
            printf("param[%d] is larger then max array length", i);
            break;
        }
        tt += strlen(param[i]);
        *(cmd + tt) = ' ';
