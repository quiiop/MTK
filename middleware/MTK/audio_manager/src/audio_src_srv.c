...
audio_src_srv_handle_t *audio_src_srv_construct_handle(audio_src_srv_pseudo_device_t type)
{
    ...
    if (!hd) {
        ...
        audio_src_srv_report("construct_handle--error, type(%d)\n", 1, type);
    } else {
        //(reduce log)audio_src_srv_report("construct_handle i(%d) type(%d)\n", 2, i, type);
    }
...
{
    //audio_src_srv_report("destruct type(%d)\n", 1, handle->type);
    // mutex lock
    audio_src_srv_mutex_lock();
...
void audio_src_srv_add_waiting_list(audio_src_srv_handle_t *handle)
{
    ...
    if (handle) {
        if ((ctx == NULL) ? true : ((ctx->running == handle) ? false : true)) {
            audio_src_srv_report("hd: 0x%x, type: %d, state: %d\n", 3, handle, handle->type, handle->state);
            AUDIO_SRC_SRV_SET_FLAG(handle, AUDIO_SRC_SRV_FLAG_WAITING);
        } else {
            audio_src_srv_report("add_waiting_list(ignore)--hd = running: 0x%x, type: %d, state: %d\n", 3, handle, handle->type, handle->state);
        }
    } else {
        audio_src_srv_report("add_waiting_list(err)\n", 0);
    }
    // mutex unlock
    audio_src_srv_mutex_unlock();
}
...
void audio_src_srv_del_waiting_list(audio_src_srv_handle_t *handle)
{
    ...
    if (handle) {
        ...
        audio_src_srv_report("del_waiting_list(err)\n", 0);
    }
    // mutex unlock
    audio_src_srv_mutex_unlock();
}
...
void audio_src_srv_update_state(audio_src_srv_handle_t *handle, audio_src_srv_event_t evt_id)
{
    ...
    audio_src_srv_report("Change Request handle(0x%x) type(%d) msg_ID(%d)", 3, handle, handle->type, evt_id);
#endif
    if (handle) {
        audio_src_srv_process_psedev_event(handle, evt_id);
    } else {
        audio_src_srv_report("update_state(err)\n", 0);
    }
    // mutex unlock
    audio_src_srv_mutex_unlock();
}
...
void audio_src_srv_set_substate(audio_src_srv_handle_t *handle, uint32_t substate)
{
    ...
    audio_src_srv_mutex_lock();
#if defined(MTK_AUDIO_MANAGER_DEBUG)
    audio_src_srv_report("Set_substate  handle(0x%x) type(%d) msg_ID(%d)", 3, handle, handle->type, substate);
#endif
    if (handle) {
        handle->substate = substate;
    } else {
        audio_src_srv_report("set_substate(err)\n", 0);
    }
    // mutex unlock
    audio_src_srv_mutex_unlock();
}
