...
static bool audio_src_srv_check_psedev_param(audio_src_srv_handle_t *handle)
{
    if (handle->stop && handle->play && handle->suspend && handle->reject && handle->exception_handle) {
        ...
        audio_src_srv_report("check_param(err)--empty func ptr\n", 0);
        return false;
    }
...
static void default_audio_src_srv_mutex_lock(void)
{
    ...
    //(reduce log)audio_src_srv_report("Take am_state ++ (%d)", 1, g_xSemaphore_cnt);
}
...
static void default_audio_src_srv_mutex_unlock(void)
{
    ...
    if (is_current_task) {
        if (g_xSemaphore_cnt == 0) {
            //(reduce log)audio_src_srv_report("Give am_state --", 0);
...
static void audio_src_srv_transfer_state(audio_src_srv_state_t state)
{
    ...
    audio_src_srv_report("ori: 0x%x, cur: 0x%x\n", 2, ctx->state, state);
    ctx->state = state;
}
...
static void audio_src_srv_update_running_psedev(audio_src_srv_handle_t *handle)
{
    ...
    if (handle) {
        audio_src_srv_report("hd: 0x%x, type: %d, state: 0x%x\n", 3,
                             handle, handle->type, handle->state);
    } else {
        audio_src_srv_report("reset running\n", 0);
    }

    if (ctx->running) {
        audio_src_srv_report("update_running(cur)--hd: 0x%x, type: %d, state: 0x%x\n", 3,
...
static void audio_src_srv_psedev_interrupt_handle(audio_src_srv_handle_t *handle)
{
    ...
    audio_src_srv_report("psedev_interrupt--cur~~hd: 0x%x, type: %d, state: 0x%x, pri: %d, cnt: %d, coming~~hd: 0x%x, type: %d, state: 0x%x, pri: %d, cnt: %d\n", 10,
    ...
    if (preempt) {
        ...
        audio_src_srv_report("psedev_interrupt handle(0x%x) type(%d). suspend", 2, ctx->running, ctx->running->type);
#endif
        ctx->running->suspend(ctx->running, handle);
    } else {
        /* reject play request */
#if defined(MTK_AUDIO_MANAGER_DEBUG)
        audio_src_srv_report("psedev_interrupt handle(0x%x) type(%d). reject", 2, handle, handle->type);
#endif
...
static audio_src_srv_handle_t *audio_src_srv_get_waiting_psedev(void)
{
    ...
    if (psedev == NULL) {
        audio_src_srv_report("Get waiting list NULL.", 0);
    } else {
        audio_src_srv_report("Get waiting list handle(0x%x), type(%d).", 2, psedev, psedev->type);
    }
    return psedev;
}
...
#if defined(MTK_AUDIO_MANAGER_DEBUG)
                            audio_src_srv_report("state_ready handle(0x%x) type(%d). play", 2, ctx->running, ctx->running->type);
#endif
                            ctx->running->play(ctx->running);
                        }
                    } else {
                        audio_src_srv_report("state_ready(err_ready)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                    }
                } else {
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_READY);
                }
        ...
        case AUDIO_SRC_SRV_MSG_PLAY: {
                if (ctx->running) {
                    if ((ctx->running->state & AUDIO_SRC_SRV_STATE_MASK) == AUDIO_SRC_SRV_STATE_PREPARE_PLAY) {
                        ...
                        audio_src_srv_report("state_ready(err_play)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                    }
                    audio_src_srv_report("state_ready(msg_play)--hd: 0x%08x, type: %d, state: 0x%x\n", 3,
                                         ctx->running, ctx->running->type, ctx->running->state);
                } else {
                    /* update pseudo device transient state */
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_PREPARE_PLAY);
...
#if defined(MTK_AUDIO_MANAGER_DEBUG)
                    audio_src_srv_report("state_ready handle(0x%x) type(%d). play handle", 2, handle, handle->type);
#endif
                    handle->play(handle);
                }
                break;
    ...
    switch (msg_id) {
        case AUDIO_SRC_SRV_MSG_STOP: {
                if (ctx->running == handle) {
                    if ((ctx->running->state & AUDIO_SRC_SRV_STATE_MASK) == AUDIO_SRC_SRV_STATE_PREPARE_PLAY) {
                        ...
                        audio_src_srv_transfer_state(AUDIO_SRC_SRV_STATE_PREPARE_STOP);
#if defined(MTK_AUDIO_MANAGER_DEBUG)
                        audio_src_srv_report("state_prepare_play handle(0x%x) type(%d). stop", 2, ctx->running, ctx->running->type);
#endif
                        ctx->running->stop(ctx->running);
                    } else {
                        audio_src_srv_report("state_ready(err_stop)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                    }
                } else {
...
#if defined(MTK_AUDIO_MANAGER_DEBUG)
                    audio_src_srv_report("state_prepare_play handle(0x%x) type(%d). stop handle", 2, handle, handle->type);
#endif
                    handle->stop(handle);
                }
                break;
        ...
        case AUDIO_SRC_SRV_MSG_PLAYING: {
                if (ctx->running == handle) {
                    ...
                    audio_src_srv_report("state_ready(err_playing)--hd: 0x%x, type: %d, state: 0x%x, coming~~hd: 0x%x, type: %d, state: 0x%x\n", 6,
                                         ctx->running, ctx->running->type, ctx->running->state,
                                         handle, handle->type, handle->state);
                }
                break;
        ...
        case AUDIO_SRC_SRV_MSG_READY: {
                if ((ctx->running) && (ctx->running == handle)) {
                    audio_src_srv_report("state_prepare_play(err_ready)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                } else {
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_READY);
                }
                break;
        ...
        case AUDIO_SRC_SRV_MSG_PLAY: {
                if (ctx->running) {
                    if ((ctx->running->state & AUDIO_SRC_SRV_STATE_MASK) == AUDIO_SRC_SRV_STATE_PREPARE_PLAY) {
                        ...
                        audio_src_srv_report("state_prepare_play(err_play)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                    }
                    audio_src_srv_report("state_prepare_play(msg_play)--hd: 0x%08x, type: %d, state: 0x%x\n", 3,
                                         ctx->running, ctx->running->type, ctx->running->state);
                }
                break;
            }
        ...
        case AUDIO_SRC_SRV_MSG_UNAVAILABLE: {
                if (ctx->running == handle) {
                    ...
                    if (ctx->running->state == AUDIO_SRC_SRV_STATE_PREPARE_PLAY) {
                        ...
                        audio_src_srv_report("state_prepare_play(err2)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                    }
                } else {
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_NONE);
                }
...
static void audio_src_srv_state_prepare_stop_handle(audio_src_srv_handle_t *handle, audio_src_srv_message_t msg_id, void *param)
{
    ...
    switch (msg_id) {
                ...
                if ((ctx->running) && (ctx->running == handle)) {
                    if ((ctx->running->state & AUDIO_SRC_SRV_STATE_MASK) == AUDIO_SRC_SRV_STATE_PREPARE_STOP) {
                        ...
                        if (ctx->running) {
                            ...
                            audio_src_srv_transfer_state(AUDIO_SRC_SRV_STATE_PREPARE_PLAY);
#if defined(MTK_AUDIO_MANAGER_DEBUG)
                            audio_src_srv_report("state_prepare_stop handle(0x%x) type(%d). play", 2, ctx->running, ctx->running->type);
#endif
                            ctx->running->play(ctx->running);
                        }
                    } else {
                        audio_src_srv_report("state_prepare_stop(err_ready)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                    }
                } else {
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_READY);
                }
        ...
        case AUDIO_SRC_SRV_MSG_STOP: {
                if ((ctx->running) && (ctx->running == handle)) {
#if defined(MTK_AUDIO_MANAGER_DEBUG)
                    audio_src_srv_report("state_prepare_stop handle(0x%x) type(%d). stop", 2, ctx->running, ctx->running->type);
#endif
        ...
        case AUDIO_SRC_SRV_MSG_PLAY: {
                if ((ctx->running) && (ctx->running == handle)) {
                    ...
                    audio_src_srv_report("state_prepare_stop handle(0x%x) type(%d). stop", 2, ctx->running, ctx->running->type);
#endif
                    audio_src_srv_report("state_prepare_stop handle: unexpected msg_id!", 0);
                } else {
                    /* push waiting list & wait next schedule */
                    audio_src_srv_add_waiting_list(handle);
                }
        ...
        case AUDIO_SRC_SRV_MSG_UNAVAILABLE: {
                if (ctx->running == handle) {
                    ...
                    if (ctx->running->state == AUDIO_SRC_SRV_STATE_PREPARE_STOP) {
                        ...
                        audio_src_srv_report("state_prepare_stop(err2)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                    }
                } else {
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_NONE);
                }
    ...
    switch (msg_id) {
        case AUDIO_SRC_SRV_MSG_PLAY: {
                if ((ctx->running->state & AUDIO_SRC_SRV_STATE_MASK) == AUDIO_SRC_SRV_STATE_PLAYING) {
                    ...
                    /* push waiting list & wait next schedule */
                    audio_src_srv_add_waiting_list(handle);
                    audio_src_srv_report("state_playing(wait)--hd: 0x%08x, type: %d, state: 0x%x\n", 3,
                                         ctx->running, ctx->running->type, ctx->running->state);
                } else {
                    audio_src_srv_report("state_playing(err1)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                }
                break;
            }
        ...
        case AUDIO_SRC_SRV_MSG_UNAVAILABLE: {
                if (ctx->running == handle) {
                    ...
                    if (ctx->running->state == AUDIO_SRC_SRV_STATE_PLAYING) {
                        ...
                        audio_src_srv_transfer_state(AUDIO_SRC_SRV_STATE_PREPARE_STOP);
#if defined(MTK_AUDIO_MANAGER_DEBUG)
                        audio_src_srv_report("state_playing handle(0x%x) type(%d). stop", 2, ctx->running, ctx->running->type);
#endif
                        ctx->running->stop(ctx->running);
                    } else {
                        audio_src_srv_report("state_playing(err2)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                    }
                } else {
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_NONE);
                }
        ...
        case AUDIO_SRC_SRV_MSG_STOP: {
                if (ctx->running == handle) {
                    ...
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_PREPARE_STOP);
#if defined(MTK_AUDIO_MANAGER_DEBUG)
                    audio_src_srv_report("state_playing handle(0x%x) type(%d). stop handle", 2, handle, handle->type);
#endif
                    handle->stop(handle);
                }
                break;
        ...
        case AUDIO_SRC_SRV_MSG_READY: {
                if ((ctx->running) && (ctx->running == handle)) {
                    audio_src_srv_report("state_playing(err_ready)--type: %d, state: 0x%x\n", 2, ctx->running->type, ctx->running->state);
                } else {
                    audio_src_srv_update_psedev_state(handle, AUDIO_SRC_SRV_STATE_READY);
                }
                break;
...
{
    audio_src_srv_report("type: %d, ori: 0x%x, cur: 0x%x\n", 3, handle->type, handle->state, state);
    handle->state = state;
}
...
void audio_src_srv_state_machine_handle(audio_src_srv_handle_t *handle, audio_src_srv_message_t msg_id, void *param)
{
    ...
    audio_src_srv_report("state: 0x%x, msg: %d, hd: 0x%08x, type: %d, dev_state: 0x%x\n", 5,
...
void audio_src_srv_process_psedev_event(audio_src_srv_handle_t *handle, audio_src_srv_event_t evt_id)
{
    ...
    if (msg_id > AUDIO_SRC_SRV_MSG_START) {
        ...
        if (handle) {
            audio_src_srv_report("update_state(err)--type: %d, msg_id: %d, evt_id: %d\n", 3, handle->type, msg_id, evt_id);
        } else {
            audio_src_srv_report("update_state(err)--handle is NULL\n", 0);
        }
    }
}
